...
$sub_menu_select = 'work_order_manager';
...
switch ($action) {
    ...
    case 'print':
        ...
        if (!$error) {
            if ($id) { // if id is present only allow update of select fields
                ...
                if ($allocate && $result->fields['qty'] <> $qty)
                    allocation_adjustment($sku_id, $qty, $result->fields['qty']);
                // if closing, remove allocation
                if ($allocate && $closed)
                    allocation_adjustment($sku_id, 0, $result->fields['qty']);
                ...
                if (!$error) {
                    ...
                    $allocate = $temp->fields['allocate'];
                    if ($allocate)
                        allocation_adjustment($sku_id, $qty); // add allocation
                }
            }
        }
    ...
    case 'save_step':
        ...
        if (!$error && sizeof($sql_data_array) > 0) {
            ...
            if (!$success)
                $error = $messageStack->add(WO_DB_UPDATE_ERROR, 'error');
        }
        if ($notes) { // update any notes
            $db->Execute("update " . TABLE_WO_JOURNAL_MAIN . " set notes = '" . $notes . "' where id = " . $id);
        }
        // check to see if the step is complete
        $result = $db->Execute("select * from " . TABLE_WO_JOURNAL_ITEM . " where ref_id = " . $id . " and step = " . $step);
        $task_id = $result->fields['task_id'];
        $set_close = true;
        if ($result->fields['mfg'] && $result->fields['mfg_id'] == '0')
            $set_close = false;
        if ($result->fields['qa'] && $result->fields['qa_id'] == '0')
            $set_close = false;
        if ($result->fields['data_entry'] && $result->fields['data_value'] == '')
            $set_close = false;
        if ($set_close) {
...
switch ($action) {
    ...
    default:
        ...
        $ajax_source_datatable = html_href_link(FILENAME_DEFAULT, 'module=work_orders&page=ajax&op=load_work_order_manager', 'SSL');
