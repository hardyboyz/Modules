...
$sub_menu_select = 'work_order_builder';
...
switch ($action) {
    ...
    case 'save':
        ...
        if (!$error) {
            ...
            if ($id) {
                if (!$success = db_perform(TABLE_WO_MAIN, $sql_data_array, 'update', 'id = ' . $id))
                    $error = true;
            } else {
                if (!$success = db_perform(TABLE_WO_MAIN, $sql_data_array, 'insert'))
                    $error = true;
                $id = db_insert_id();
                if (!$error && $bump_rev) {
        ...
        if (!$error) {
            ...
            $action = 'edit';//gen_redirect(html_href_link(FILENAME_DEFAULT, gen_get_all_get_params(array('action')), 'SSL'));
        } else {
...
switch ($action) {
    ...
    default:
        ...
        $ajax_source_datatable = html_href_link(FILENAME_DEFAULT, 'module=work_orders&page=ajax&op=load_work_order_builder', 'SSL');
...
//        }
//        if (!$f0)
//            $criteria[] = "m.inactive = '0'"; // inactive flag
//            
