...
$menu_select = 'company';
$sub_menu_select = 'asset_management';
...
switch ($action) {
    ...
    case 'create':
        ...
        switch ($asset_type) {
            case 'vh': $search_text = TEXT_VEHICLE;
                break;
            case 'bd': $search_text = TEXT_BUILDING;
                break;
            case 'fn': $search_text = TEXT_FURNITURE;
                break;
            case 'pc': $search_text = TEXT_COMPUTER;
                break;
            case 'ld': $search_text = TEXT_LAND;
                break;
            case 'sw': $search_text = TEXT_SOFTWARE;
                break;
        }
    ...
    case 'delete':
        ...
        if ($image_with_path) { // delete image
            ...
            if (file_exists($file_path . $result->fields['image_with_path']))
                unlink($file_path . $result->fields['image_with_path']);
        }
        foreach (glob(ASSETS_DIR_ATTACHMENTS . 'assets_' . $id . '_*.zip') as $filename)
            unlink($filename); // remove attachments
    ...
    case 'save':
        ...
        if (substr($asset_path, 0, 1) == '/')
            $asset_path = substr($asset_path, 1); // remove leading '/' if there
        if (substr($asset_path, -1, 1) == '/')
            $asset_path = substr($asset_path, 0, strlen($asset_path) - 1); // remove trailing '/' if there
        $asset_type = db_prepare_input($_POST['asset_type']);
        $sql_data_array = array();
...
switch ($action) {
    ...
    default:
        ...
        $ajax_source_datatable = html_href_link(FILENAME_DEFAULT, 'module=assets&page=ajax&op=load_asset_management', 'SSL');
