...
// +-----------------------------------------------------------------+
//  Path: /modules/bank_transfer/classes/gen_ledger.php
//
                    ...
                    case 2:
                        ...
                        if (sizeof($this->journal_rows) > 0)
                            foreach ($this->journal_rows as $value) {
                                ...
                                if (round($credit_amount, $this->currencies[DEFAULT_CURRENCY]['decimal_places'] + 2) <> 0
                                        || round($debit_amount, $this->currencies[DEFAULT_CURRENCY]['decimal_places'] + 2) <> 0) {
                                    if ($value['gl_type'] == 'tax') {
                                        $accounts[$value['gl_account']]['credit'] += $credit_amount;
                                        $accounts[$value['gl_account']]['debit'] += $debit_amount;
                                        $total_tax += empty($credit_amount) ? $debit_amount : $credit_amount;
                                    } else {
...
//                        if ($this->tax_inclusive != 0)
//                            if (ROUND_TAX_BY_AUTH)
//                                $accounts[$this->journal_rows[0]['gl_account']]['debit'] = $total_credit;
//                            else
//                                $accounts[$this->journal_rows[0]['gl_account']]['debit'] -= $total_tax;
                        if (sizeof($accounts) > 0)
                            foreach ($accounts as $gl_acct => $values) {
                                if ($values['credit'] <> 0 || $values['debit'] <> 0) {
                                    $sql = "update " . TABLE_CHART_OF_ACCOUNTS_HISTORY . " set 
                    ...
                    case 3:
                        ...
                        if (sizeof($this->journal_rows) > 0)
                            foreach ($this->journal_rows as $value) {
                                ...
                                if (round($credit_amount, $this->currencies[DEFAULT_CURRENCY]['decimal_places'] + 2) <> 0
                                        || round($debit_amount, $this->currencies[DEFAULT_CURRENCY]['decimal_places'] + 2) <> 0) {
                                    if ($value['gl_type'] == 'tax') { // for testing purpose only
                        ...
                        //  if ($this->tax_inclusive != 0) // test purpose only
                        //    if (ROUND_TAX_BY_AUTH)
                        //      $accounts[$this->journal_rows[0]['gl_account']]['credit'] = $total_debit;
                        //else
                        //  $accounts[$this->journal_rows[0]['gl_account']]['credit'] -= $total_tax;
                        if (sizeof($accounts) > 0)
                            foreach ($accounts as $gl_acct => $values) {
                                if ($values['credit'] <> 0 || $values['debit'] <> 0) {
                                    $sql = "update " . TABLE_CHART_OF_ACCOUNTS_HISTORY . " set 
            ...
            case 20: // Cash Distribution Journal
            case 21: // Inventory Direct Purchase Journal
            case 34: // Inventory Direct Purchase Journal
            case 35: // Inventory Direct Purchase Journal
                $accounts = array();
                if (sizeof($this->journal_rows) > 0)
                    foreach ($this->journal_rows as $value) {
                        $credit_amount = ($value['credit_amount']) ? $value['credit_amount'] : '0';
                        ...
                        if (round($credit_amount, $this->currencies[DEFAULT_CURRENCY]['decimal_places'] + 2) <> 0
                                || round($debit_amount, $this->currencies[DEFAULT_CURRENCY]['decimal_places'] + 2) <> 0) {
                            if ($value['gl_type'] == 'tax') {
                                $accounts[$value['gl_account']]['credit'] += empty($credit_amount) ? $debit_amount : $credit_amount;
                                $accounts[$value['gl_account']]['debit'] += empty($credit_amount) ? $debit_amount : $credit_amount;
                            } else {
                                $accounts[$value['gl_account']]['credit'] += $credit_amount;
                                $accounts[$value['gl_account']]['debit'] += $debit_amount;
                            }
        ...
        switch ($this->journal_id) {
			...
			where account_id = '" . $this->journal_rows[$i]['gl_account'] . "' and period = " . $this->period;
                    ...
                    case 2:
                        ...
                        $total_credit = 0;
                        for ($i = 0; $i < count($this->journal_rows); $i++) {
                            if ($this->journal_rows[$i]['credit_amount'] > 0) {
                                $total_credit += $this->journal_rows[$i]['credit_amount'];
                            }
                        }
                        for ($i = 0; $i < count($this->journal_rows); $i++) {
                            // Update chart of accounts history 

//                            if ($this->journal_rows[$i]['gl_type'] == 'tax') {
//                                $tax_amount = empty($this->journal_rows[$i]['credit_amount']) ? $this->journal_rows[$i]['debit_amount'] : $this->journal_rows[$i]['credit_amount'];
//                                $tax_amount_total += $tax_amount;
//                                $sql = "update " . TABLE_CHART_OF_ACCOUNTS_HISTORY . " set 
//			credit_amount = credit_amount - " . $tax_amount . ", 
//			debit_amount = debit_amount - " . $tax_amount . " 
//			where account_id = '" . $this->journal_rows[$i]['gl_account'] . "' and period = " . $this->period;
                      //      } else {
//                                if ($this->journal_rows[$i]['debit_amount'] > 0) {
//                                    if ($this->tax_inclusive != 0)
//                                        if (ROUND_TAX_BY_AUTH)
//                                            $this->journal_rows[$i]['debit_amount'] = $total_credit;
//                                        else
//                                            $this->journal_rows[$i]['debit_amount'] = $this->journal_rows[$i]['debit_amount'] - $this->sales_tax;
//                             //   }
                                $sql = "update " . TABLE_CHART_OF_ACCOUNTS_HISTORY . " set 
			credit_amount = credit_amount - " . $this->journal_rows[$i]['credit_amount'] . ", 
			debit_amount = debit_amount - " . $this->journal_rows[$i]['debit_amount'] . " 
			where account_id = '" . $this->journal_rows[$i]['gl_account'] . "' and period = " . $this->period;
                    ...
                    case 3 :
                        ...
                        for ($i = 0; $i < count($this->journal_rows); $i++) {
                            if ($this->journal_rows[$i]['debit_amount'] > 0) {
                                $total_debit += $this->journal_rows[$i]['debit_amount'];
                            }
                        }
                        for ($i = 0; $i < count($this->journal_rows); $i++) {
                            // Update chart of accounts history 
//                            if ($this->journal_rows[$i]['gl_type'] == 'tax') { // for testing purpose only
//                                $tax_amount = empty($this->journal_rows[$i]['credit_amount']) ? $this->journal_rows[$i]['debit_amount'] : $this->journal_rows[$i]['credit_amount'];
//                                $tax_amount_total += $tax_amount;
//                                $sql = "update " . TABLE_CHART_OF_ACCOUNTS_HISTORY . " set 
//			credit_amount = credit_amount - " . $tax_amount . ", 
...
//			where account_id = '" . $this->journal_rows[$i]['gl_account'] . "' and period = " . $this->period;
//                            } else {
                            //   if ($this->journal_rows[$i]['credit_amount'] > 0) { //Test purpose only
                            //  if ($this->tax_inclusive != 0)
                            //    if (ROUND_TAX_BY_AUTH)
                            //      $this->journal_rows[$i]['credit_amount'] = $total_debit;
                            // else
                            //   $this->journal_rows[$i]['credit_amount'] = $this->journal_rows[$i]['credit_amount'] - $this->sales_tax;
                            // }
                            $sql = "update " . TABLE_CHART_OF_ACCOUNTS_HISTORY . " set 
			credit_amount = credit_amount - " . $this->journal_rows[$i]['credit_amount'] . ", 
			debit_amount = debit_amount - " . $this->journal_rows[$i]['debit_amount'] . " 
            ...
            case 19: // POS Journal
            case 20: // Cash Distribution Journal
            case 34: // Cash Distribution Journal
            case 35: // Cash Distribution Journal
            case 21: // Inventory Direct Purchase Journal
                $tax_amount_total = 0;
                for ($i = 0; $i < count($this->journal_rows); $i++) {
                    // Update chart of accounts history 
		...
		where purchase_invoice_id = '" . $this->purchase_invoice_id . "' and journal_id = '" . $this->journal_id . "'";
            ...
            switch ($this->journal_id) { // select the field to fetch the next number
                ...
                case 4: $str_field = 'next_po_num';
                    break;
                case 6: $str_field = 'next_pur_num';
                    break; //adding by 4axiz(apu) date:28-sep-2013
                ...
                case 33: $str_field = 'bank_transfer';
                    break;
                case 34: $str_field = 'received_money';
                    break;
                case 35: $str_field = 'spend_money';
                    break;
            }
            if ($str_field) {
