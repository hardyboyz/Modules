...
// +-----------------------------------------------------------------+
//  Path: /modules/bank_transfer/ajax/load_order.php
//
/* * ************   Check user security   **************************** */
$xml = NULL;
$debug = NULL;
...
require_once(DIR_FS_MODULES . 'bank_transfer/defaults.php');
require_once(DIR_FS_MODULES . 'bank_transfer/functions/bank_transfer.php');
...
$rec_id = db_prepare_input($_GET['rec_id']);
...
if ($rec_id) {
    $qurey = "SELECT * FROM invoice_recurring WHERE id = " . $rec_id;
    $query_res = mysql_query($qurey);
    $query_res = mysql_fetch_assoc($query_res);
    //$inv_data = unserialize($query_res['inv_info']);
    $order->fields = unserialize($query_res['inv_info']);
    //$inv_item = unserialize($query_res['inv_detail_info']);
    $ordr_items->fields = unserialize($query_res['inv_detail_info']);
    $cID = $order->fields['bill_acct_id'];
    $sID = $order->fields['ship_acct_id'];
...
if (sizeof($order->fields) > 0) {
    ...
    $order->fields['attach_exist'] = file_exists(bank_transfer_DIR_MY_ORDERS . 'order_' . $oID . '.zip') ? 1 : 0;
...
//    }
    if ($id) {
        ...
        switch (JOURNAL_ID) { // determine where to put value, qty or pstd
            ...
            case 4:
            case 9:
            case 10: $qty_pstd = 'qty';
                break;
            case 6:
            case 7:
            case 12:
            case 13: $qty_pstd = 'pstd';
                break;
            default:
        }
//        while (!$ordr_items->EOF) {
...
foreach ($ordr_items->fields as $order_item) {
    $inv_details = $db->Execute("select inventory_type, inactive, item_weight, quantity_on_hand, lead_time 
		    from " . TABLE_INVENTORY . " where sku = '" . $order_item['sku'] . "'");
    if (!in_array($inv_details->fields['inventory_type'], $cog_types))
        $inv_details->fields['quantity_on_hand'] = 'NA';
    $total = $order_item['credit_amount'] + $order_item['debit_amount'];
    $item_list[] = array(
        'id' => $order_item['id'],
        'item_cnt' => $order_item['item_cnt'],
        'gl_type' => $order_item['gl_type'],
        'pstd' => $order_item['qty'],
        'sku' => $order_item['sku'],
        'description' => $order_item['description'],
        'gl_account' => $order_item['gl_account'],
        'taxable' => $order_item['taxable'],
        'serialize' => $order_item['serialize_number'],
        'proj_id' => $order_item['project_id'],
        'date_1' => substr($order_item['date_1'], 0, 10),
        'unit_price' => $currencies->precise($order_item['qty'] ? ($total / $order_item['qty']) : '0', true, $currencies_code, $currencies_value),
        'total' => $currencies->format($total, true, $currencies_code, $currencies_value),
        'full_price' => $currencies->format($order_item['full_price'], true, $currencies_code, $currencies_value),
        'inactive' => $inv_details->fields['inactive'],
        'weight' => $inv_details->fields['item_weight'],
        'stock' => $inv_details->fields['quantity_on_hand'],
        'lead' => $inv_details->fields['lead_time'],
