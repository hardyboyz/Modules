...
switch ($_GET['sub_jID']) {
    case 1 :
        $sWhere = "WHERE journal_id=33 or (sub_journal_id=" . $_GET['sub_jID'] . " and journal_id=" . $_GET['jID'] . ")";
        break;
    case 2 :
        $sWhere = "WHERE journal_id=34 or (sub_journal_id=" . $_GET['sub_jID'] . " and journal_id=" . $_GET['jID'] . ")";
        break;
    case 3:
        $sWhere = "WHERE journal_id=35 or (sub_journal_id=" . $_GET['sub_jID'] . " and journal_id=" . $_GET['jID'] . ")";
        break;
    default:
        $sWhere = "WHERE journal_id=" . $_GET['jID'] . ' and sub_journal_id = 0';
}

if (!isset($_GET['sub_jID'])) {
    $sWhere = "WHERE journal_id=" . $_GET['jID'] . ' and sub_journal_id = 0';
}
        ...
        if ($aColumns[$i] == "id") {
            ...
            switch ($_GET['jID']) {
                case 2:
                    switch ($_GET['sub_jID']) {
                        case 1:
                            $link_page = html_href_link(FILENAME_DEFAULT, 'module=bank_transfer&amp;page=journal&amp;oID=' . $id . '&amp;jID=' . $_GET['jID'] . '&sub_jID=1&amp;action=edit', 'SSL');
                            break;
                        case 2 :
                            $link_page = html_href_link(FILENAME_DEFAULT, 'module=bank_transfer&amp;page=spend_receive_money&amp;oID=' . $id . '&amp;jID=' . $_GET['jID'] . '&sub_jID=2&amp;action=edit', 'SSL');
                            break;
                        case 3 :
                            $link_page = html_href_link(FILENAME_DEFAULT, 'module=bank_transfer&amp;page=receive_spend_money&amp;oID=' . $id . '&amp;jID=' . $_GET['jID'] . '&sub_jID=3&amp;action=edit', 'SSL');
                            break;
                        default :
