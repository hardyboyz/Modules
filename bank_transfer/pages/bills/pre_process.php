...
// +-----------------------------------------------------------------+
//  Path: /modules/bank_transfer/pages/bills/pre_process.php
//
    ...
    case 'print':
        ...
        if (!$error && $post_success = $order->post_ordr($action)) { // Post the order class to the db
            if ($action == 'save') {
                ...
                gen_redirect(html_href_link(FILENAME_DEFAULT, 'module=bank_transfer&amp;page=bills&amp;oID=' . $order->id . '&amp;jID=' . $_GET['jID'] . '&amp;type='.$_GET['type'].'&amp;action=edit', 'SSL'));
