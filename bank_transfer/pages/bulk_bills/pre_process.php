...
// +-----------------------------------------------------------------+
//  Path: /modules/bank_transfer/pages/bulk_bills/pre_process.php
//

//this is for menu select coding by 4aixz(apu) date:20_04_2013
...
$purchase_invoice_id     = db_prepare_input($_POST['purchase_invoice_id']);	// bank_transfer starting check number
...
switch ($action) {
  case 'print':
	...
	foreach ($payment_list as $account => $values) {
	  ...
	  $order->purchase_invoice_id = $purchase_invoice_id;	// bank_transfer payment number
	  $order->shipper_code        = '';
