...
} else if ($_GET['page'] == "orders" && $_GET['jID'] == "12") {
    $menu_select = 'customers';
...
} else if ($_GET['page'] == "orders" && $_GET['jID'] == "6") {
    $menu_select = 'vendors';
...
switch ($action) {
    ...
    case 'post_next':
        ...
        $order->purchase_invoice_id = db_prepare_input($_POST['purchase_invoice_id']); // bank_transfer order/invoice ID
...
//	$order->sales_tax_auths     = db_prepare_input($_POST['sales_tax_auths']);
        ...
        if (!$error) { // Post the order
            ...
            if ($post_success = $order->post_ordr($action)) { // Post the order class to the db
                if ($order->rm_attach)
                    @unlink(bank_transfer_DIR_MY_ORDERS . 'order_' . $order->id . '.zip');
                if (is_uploaded_file($_FILES['file_name']['tmp_name']))
                    saveUploadZip('file_name', bank_transfer_DIR_MY_ORDERS, 'order_' . $order->id . '.zip');
                ...
                if (!empty($_POST['is_recurring']) && intval($_POST['recurring_interval']) > 0) {
                    ...
                    $recurring_info['parent_inv_id'] = $recurring_info['inv_info']['id'];
                    $recurring_info['inv_type'] = 'invoice';
                    $recurring_info['inv_date'] = $recurring_info['inv_info']['post_date'];
                    ...
                    if (!$recurring_info_is_set) {
                        $query = "INSERT INTO invoice_recurring(journal_id, parent_inv_id ,inv_info, inv_detail_info,inv_date,create_date,recurring_interval ) VALUES (" . $_GET[jID] . "," . $order->id . ",'" . $recurring_info['inv_info'] . "','" . $recurring_info['inv_detail_info'] . "','" . $recurring_info['inv_date'] . "','" . $recurring_info['create_date'] . "'," . $recurring_info['recurring_interval'] . ")";
                        ...
                        if (!empty($_POST['update_recurring_info'])) {
                            $query = "UPDATE invoice_recurring SET inv_info = '" . $recurring_info['inv_info'] . "' , inv_detail_info = '" . $recurring_info['inv_detail_info'] . "', update_date = '" . date("Y-m-d g:i:s") . "' , recurring_interval = " . $recurring_info['recurring_interval'] . " WHERE parent_inv_id=" . $order->id;
                            mysql_query($query);
                        } else {
                            $query = "UPDATE invoice_recurring SET recurring_interval = " . $recurring_info['recurring_interval'] . " WHERE parent_inv_id=" . $order->id;
                            mysql_query($query);
                        }
                    }
                } else {
...
//                    mysql_query($query);
                ...
                if ($action == 'save') {
                    ...
                    gen_redirect(html_href_link(FILENAME_DEFAULT, 'module=bank_transfer&amp;page=orders&amp;oID=' . $order->id . '&amp;jID=' . $_GET['jID'] . '&amp;action=edit', 'SSL'));
                    ...
                    gen_redirect(html_href_link(FILENAME_DEFAULT, 'module=bank_transfer&amp;page=bills&amp;jID=' . $jID . '&amp;type=' . $account_type . '&amp;oID=' . $order->id . '&amp;action=pmt', 'SSL'));
