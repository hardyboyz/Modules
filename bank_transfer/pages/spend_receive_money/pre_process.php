        ...
        for ($i = 1; $i < count($journal_item); $i++) {
            if (isset($_POST['tax_inclusive'])) {
                if ($journal_item[$i]['taxable'] != 0) {
                    ...
                    if ($journal_item[$i]['debit_amount'] > 0) {
...
//        }
        ...
        $glEntry->journal_main_array = array(
            ...
            'sales_tax' => $total_tax,
            //'total_amount' => $total_amount,
            'total_amount' => $currencies->clean_value($_POST['debit_0']),
        ...
        if (!$error) {
            ...
            if (!$error) {
                ...
                gen_redirect(html_href_link(FILENAME_DEFAULT, 'module=bank_transfer&page=spend_receive_money&oID=' . $glEntry->id . '&jID=' . JOURNAL_ID . '&sub_jID=' . SUB_JOURNAL_ID . '&action=edit', 'SSL'));
            }
            // *************** END TRANSACTION *************************
        }
        $db->transRollback();
