...
// +-----------------------------------------------------------------+
//  Path: /modules/bank_transfer/pages/orders/js_include.php
//
...
<?php 
  ...
  if (!$error && $action == 'email') {
    echo '  force_clear = true;' . "\n";
	echo '  ClearForm();' . "\n";
	echo '  var printWin = window.open("index.php?module=bank_transfer&page=popup_email&oID=' . $order->id . '","forms","width=1150px,height=350px,resizable=1,scrollbars=1,top=150px,left=100px");' . "\n";
    echo '  printWin.focus()' . "\n";
  }
  if ($action == 'edit')   echo '  ajaxOrderData(0, ' . $_GET['oID'] . ', ' . $_GET['jID'] . ', false, false,'.$_GET['rec_id'].');' . "\n";
  if ($action == 'prc_so') echo '  ajaxOrderData(0, ' . $_GET['oID'] . ', ' . $_GET['jID'] . ', true, false);' . "\n";
...
<script type="text/javascript" src="modules/bank_transfer/javascript/orders.js"></script>
