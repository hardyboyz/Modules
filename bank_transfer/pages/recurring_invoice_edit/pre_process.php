...
//	$order->sales_tax_auths     = db_prepare_input($_POST['sales_tax_auths']);
        ...
        if (!$error) { // Post the order
            if ($post_success = $order->post_ordr($action, "recurring")) { // Post the order class to the db
                
                
            $recurring_info['recurring_interval'] = intval($_POST['recurring_interval']);
            $recurring_info['update_date'] = date("Y-m-d g:i:s");
            $recurring_info['inv_info']= mysql_escape_string(serialize($post_success['journal_main']));
            $recurring_info['inv_detail_info']= mysql_escape_string(serialize($post_success['journal_main_item']));
            
            $query = "UPDATE invoice_recurring SET inv_info = '".$recurring_info['inv_info']."', inv_detail_info = '". $recurring_info['inv_detail_info'] ."', update_date = '".$recurring_info['update_date']."', recurring_interval = " .$recurring_info['recurring_interval'] ." WHERE id = " . $_GET['rec_id'];
            
            if(mysql_query($query)){
                 gen_redirect(html_href_link(FILENAME_DEFAULT, 'module=bank_transfer&amp;page=recurring_invoice_edit&amp;oID=' . $_GET['oID']. '&amp;jID=' . $_GET['jID'] . '&amp;action=edit&rec_id=' . $_GET['rec_id'], 'SSL'));
            }
