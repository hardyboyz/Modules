...
// +-----------------------------------------------------------------+
//  Path: /modules/bank_transfer/pages/journal/js_include.php
//
?>
...
<?php if ($action == 'edit') echo '  EditJournal(' . $oID . ');' . chr(10); ?>
    ...
    function check_form() {
        ...
        function OpenGLList() {
            window.open("index.php?module=bank_transfer&page=popup_journal&list=1&form=journal","gl_open","width=1150,height=550,resizable=1,scrollbars=1,top=150,left=100");
        }
        ...
        function EditJournal(rID) {
            $.ajax({
                type: "GET",
                url: 'index.php?module=bank_transfer&page=ajax&op=load_record&rID='+rID,
        ...
        function addGLRow(debit, credit, description) {
            ...
            // End hidden fields
            cell[2] = '<td><input style="width:500px !important;" type="text" name="desc_'+rowCnt+'" id="desc_'+rowCnt+'"  value="'+description+'"><\/td>';
            if(journalID=='34' || sub_journalID==2 || journalID=='35' || sub_journalID==3){
                if(document.getElementById('item_table').rows.length==1){
                    cell[3] = '<td><input type="text" name="credit_'+rowCnt+'" id="credit_'+rowCnt+'" style="text-align:right" size="13" maxlength="12" value="'+credit+'"onchange="formatRow('+rowCnt+', \'c\')"><\/td>';
                }else{
        ...
        function updateBalanceRP(){
            ...
            for (var i = 1; i <= ((document.getElementById('item_table').rows.length) / 2); i++) {
                ...
                if(document.getElementById('item_table').rows.length==2){
                    ...
                    temp = parseFloat(cleanCurrency(document.getElementById('credit_1').value));
                    if (!isNaN(temp)) credit_total += temp;
                    if(i!=1){
                        temp = parseFloat(cleanCurrency(document.getElementById('debit_'+i).value));
                        if (!isNaN(temp)) debit_total += temp;
