        ...
        if (!$error) {
            ...
            if (!$error) {
               ...
               // gen_redirect(html_href_link(FILENAME_DEFAULT, gen_get_all_get_params(array('action')), 'SSL'));
                gen_redirect(html_href_link(FILENAME_DEFAULT, 'module=bank_transfer&page=journal&oID='.$glEntry->id.'&jID='.JOURNAL_ID.'&sub_jID='.SUB_JOURNAL_ID.'&action=edit', 'SSL'));
            }
            // *************** END TRANSACTION *************************
        }
        $db->transRollback();
...
$gl_array_list = gen_coa_pull_down_bank();
$i = 0;
...
$next_bt_num = $db->Execute('select '.$next_running_num.' from ' . TABLE_CURRENT_STATUS . ' limit 1');
$include_header = true;
$include_footer = true;
$include_tabs = false;
