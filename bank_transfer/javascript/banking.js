...
// +-----------------------------------------------------------------+
//  Path: /modules/bank_transfer/javascript/banking.js
//
...
//  if (!xml) return;
  if ($(xml).find("result").text() == 'success') {
    ...
    window.open('index.php?module=bank_transfer&page=popup_bills_accts&list=1&jID='+journalID+"&type="+account_type+"&search_text="+search_text,"invoices","width=1150px,height=550px,resizable=1,scrollbars=1,top=150,left=100");
  }
}

function OpenOrdrList(currObj) {
  window.open('index.php?module=bank_transfer&page=popup_bills&list=1&jID='+journalID+"&type="+account_type,"invoices","width=1150px,height=550px,resizable=1,scrollbars=1,top=150,left=100");
}

function popupWindowCvv() {
  window.open('index.php?module=bank_transfer&page=popup_cvv',"popup_payment_cvv","width=1150,height=550,resizable=1,scrollbars=1,top=150,left=100");
}

function ajaxBillData(cID, bID, jID) {
  $.ajax({
    type: "GET",
	url: 'index.php?module=bank_transfer&page=ajax&op=load_bill&cID='+cID+'&bID='+bID+'&jID='+jID,
    dataType: ($.browser.msie) ? "text" : "xml",
    error: function(XMLHttpRequest, textStatus, errorThrown) {
      alert ("Ajax Error: " + XMLHttpRequest.responseText + "\nTextStatus: " + textStatus + "\nErrorThrown: " + errorThrown);
    },
...
function loadNewBalance() { // request funtion
  ...
  $.ajax({
    type: "GET",
    url: 'index.php?module=bank_transfer&page=ajax&op=acct_balance&gl_acct_id='+gl_acct+'&post_date='+post_date,
    dataType: ($.browser.msie) ? "text" : "xml",
    error: function(XMLHttpRequest, textStatus, errorThrown) {
      alert ("Ajax Error: " + XMLHttpRequest.responseText + "\nTextStatus: " + textStatus + "\nErrorThrown: " + errorThrown);
    },
