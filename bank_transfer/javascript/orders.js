...
// +-----------------------------------------------------------------+
//  Path: /modules/bank_transfer/javascript/orders.js
//
...
function ajaxOrderData(cID, oID, jID, open_order, ship_only, recurring_invoice ) {
    
    var open_so_po = (open_order) ? '1' : '0';
    var only_ship  = (ship_only)  ? '1' : '0';
    if(recurring_invoice != undefined){
        var urls = 'index.php?module=bank_transfer&page=ajax&op=load_recurring_invoice_order&cID='+cID+'&oID='+oID+'&jID='+jID+'&so_po='+open_so_po+'&ship_only='+only_ship+'&rec_id='+recurring_invoice;
    }else{
        var urls =  'index.php?module=bank_transfer&page=ajax&op=load_order&cID='+cID+'&oID='+oID+'&jID='+jID+'&so_po='+open_so_po+'&ship_only='+only_ship;
    }
    ...
    $(xml).find(cTag).each(function() {
        ...
        switch (type) {
            ...
            case 'bill':
                ...
                if (show_status == '1') {
                    window.open("index.php?module=bank_transfer&page=popup_status&form=orders&id="+id,"contact_status","width=1150px,height=400px,resizable=0,scrollbars=1,top=150,left=100");
                }
                break;
            case 'ship':
                ship_add = this;
...
function AccountList() {
    ...
    if (guess != text_search && guess != '' && !override) {
        $.ajax({
            type: "GET",
            url: 'index.php?module=bank_transfer&page=ajax&op=load_searches&jID='+journalID+'&type='+account_type+'&guess='+guess,
            dataType: ($.browser.msie) ? "text" : "xml",
            error: function(XMLHttpRequest, textStatus, errorThrown) {
                alert ("Ajax Error: " + XMLHttpRequest.responseText + "\nTextStatus: " + textStatus + "\nErrorThrown: " + errorThrown);
            },
...
function AccountListResp(sXml) {
    ...
    if ($(xml).find("result").text() == 'success') {
        ...
        window.open("index.php?module=contacts&page=popup_accts&type="+account_type+"&fill="+fill+"&jID="+journalID+"&search_text="+document.getElementById('search').value,"accounts","width=1120px,height=800px,resizable=1,scrollbars=1,top=150,left=100");
    }
}
...
function OpenOrdrList(currObj) {
    window.open("index.php?module=bank_transfer&page=popup_orders&jID="+journalID,"search_po","width=1150px,height=550px,resizable=1,scrollbars=1,top=150,left=100");
}

function OpenRecurList(currObj) {
    window.open("index.php?module=bank_transfer&page=popup_recur&jID="+journalID,"recur","width=1150px,height=300px,resizable=1,scrollbars=1,top=150,left=100");
}
...
function InventoryList(rowCnt) {
    ...
    window.open("index.php?module=inventory&page=popup_inv&type="+account_type+"&rowID="+rowCnt+"&storeID="+storeID+"&cID="+cID+"&search_text="+sku,"inventory","width=1150px,height=800px,resizable=1,scrollbars=1,top=150,left=100");
}
...
function convertQuote() {
    ...
    if (id != '') {
        window.open("index.php?module=bank_transfer&page=popup_convert&oID="+id,"popup_convert","width=1150px,height=300px,resizable=1,scrollbars=1,top=150,left=100");
    } else {
...
function convertSO() {
    var id = document.getElementById('id').value;
    if (id != '') {
        window.open("index.php?module=bank_transfer&page=popup_convert_po&oID="+id,"popup_convert_po","width=1150px,height=300px,resizable=1,scrollbars=1,top=150,left=100");
    } else {
...
function openBarCode() {
    window.open("index.php?module=bank_transfer&page=popup_bar_code&jID="+journalID,"bar_code","width=1150px,height=150px,resizable=1,scrollbars=1,top=110,left=100");
}
...
function addInvRow() {
    ...
    cell = '<input style="width:200px !important;" value="" readonly="readonly" onclick=\'window.open("index.php?count='+rowCnt+'&module=bank_transfer&page=disc_msg","Description","height=500,width=750")\' name="desc_'+rowCnt+'" id="desc_'+rowCnt+'" size="'+((single_line_list=='1')?50:75)+'" maxlength="255" />';
    newCell = newRow.insertCell(-1);
    newCell.innerHTML = cell;
    ...
    cell    = '<input style="width:30px !important" type="text" name="qty_'+rowCnt+'" id="qty_'+rowCnt+'"'+(item_col_1_enable == '1' ? " " : " readonly=\"readonly\"")+' size="7" maxlength="6" onchange="updateRowTotal('+rowCnt+', true)" style="text-align:right" />';
    newCell = newRow.insertCell(-1);
    newCell.innerHTML = cell;
    newCell.align  = 'center';
    newCell.style.whiteSpace = 'nowrap';
    ...
    cell    = '<input type="text" style="width:30px !important" name="pstd_'+rowCnt+'" id="pstd_'+rowCnt+'"'+(item_col_2_enable == '1' ? " " : " readonly=\"readonly\"")+' size="7" maxlength="6" onchange="updateRowTotal('+rowCnt+', true)" style="text-align:right" />';
    switch (journalID) {
        case  '6':
        case  '7':
        case '12':
...
//backup function addInvRow backup by 4axiz(apu) date:21/05/2013
//function addInvRow() {
//  var newCell = '';
//  var cell    = '';
//  var rowCnt  = 0;
...
function PreProcessLowStock() {
    ...
    $.ajax({
        type: "GET",
        url: 'index.php?module=bank_transfer&page=ajax&op=low_stock&cID='+acct+'&sID='+store+'&rID='+rowCnt,
        dataType: ($.browser.msie) ? "text" : "xml",
        error: function(XMLHttpRequest, textStatus, errorThrown) {
            alert ("Ajax Error: " + XMLHttpRequest.responseText + "\nTextStatus: " + textStatus + "\nErrorThrown: " + errorThrown);
        },
