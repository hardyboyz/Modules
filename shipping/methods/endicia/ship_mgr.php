...
<table class="ui-widget" style="border-style:none;width:100%">
  <tr>
	<td><?php echo ($security_level < 2) ? '&nbsp;' : html_button_field('ship_'    .$method_id, SHIPPING_SHIP_PACKAGE, 'onclick="window.open(\'index.php?module=shipping&amp;page=popup_label_mgr&amp;method='.$method_id.'\',\'popup_label_mgr\',\'width=1150,height=700,resizable=1,scrollbars=1,top=50,left=100\')"'); ?></td>
	<td><?php echo ($security_level < 2) ? '&nbsp;' : html_button_field('ship_log_'.$method_id, SHIPPING_CREATE_ENTRY, 'onclick="window.open(\'index.php?module=shipping&amp;page=popup_tracking&amp;method=' .$method_id.'&amp;action=new\',\'popup_tracking\',\'width=1150,height=350,resizable=1,scrollbars=1,top=150,left=100\')"'); ?></td>
	...
	if ($result->RecordCount() > 0) {
	  ...
	  while(!$result->EOF) {
		...
		if ($result->fields['actual_date'] == '0000-00-00 00:00:00') // not tracked yet, show the tracking icon 
		  echo html_icon('ucbooks/truck-icon.png',  TEXT_TRACK_CONFIRM,'small', 'onclick="submitShipSequence(\'' . $method_id . '\', ' . $result->fields['id'] . ', \'track\')"') . chr(10);
		echo html_icon('ucbooks/stock_id.png',      TEXT_VIEW_SHIP_LOG,'small', 'onclick="loadPopUp(\'' . $method_id . '\', \'edit\', ' . $result->fields['id'] . ')"') . chr(10);
		echo html_icon('actions/document-print.png',   TEXT_PRINT,        'small', 'onclick="window.open(\'index.php?module=shipping&amp;page=popup_label_mgr&amp;action=view&amp;method=' . $method_id . '&amp;date=' . $date . '&amp;labels=' . $result->fields['tracking_id'] . '\',\'label_mgr\',\'width=1150,height=700,resizable=1,scrollbars=1,top=50,left=100\')"') . chr(10);
		echo html_icon('emblems/emblem-unreadable.png',TEXT_DELETE,       'small', 'onclick="if (confirm(\'' . SHIPPING_DELETE_CONFIRM . '\')) window.open(\'index.php?module=shipping&amp;page=popup_label_mgr&amp;method=' . $method_id . '&amp;sID=' . $result->fields['shipment_id'] . '&amp;action=delete\',\'popup_label_mgr\',\'width=1150,height=700,resizable=1,scrollbars=1,top=50,left=100\')"') . chr(10);
		echo '    </td>';
		echo '  </tr>' . chr(10);
		$result->MoveNext();
		$odd = !$odd;
