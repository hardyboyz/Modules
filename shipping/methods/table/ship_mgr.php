  ...
  <?php 
	...
	if ($result->RecordCount() > 0) {
		while(!$result->EOF) {
			...
			echo '    <td align="right">';
	  		echo html_icon('ucbooks/stock_id.png', TEXT_VIEW_SHIP_LOG, 'small', 'onclick="loadPopUp(\'table\', \'edit\', ' . $result->fields['id'] . ')"') . chr(10);
//	  		echo html_icon('actions/document-print.png', TEXT_PRINT, 'small', 'onclick="window.open(\'' . html_href_link(FILENAME_DEFAULT, gen_get_all_get_params(array('method', 'labels', 'date')) . 'method=table&date=' . $date . '&labels=' . $result->fields['tracking_id'], 'SSL') . '\',\'label_mgr\',\'width=800,height=700,resizable=1,scrollbars=1,top=50,left=50\')"') . chr(10);
	  		echo html_icon('emblems/emblem-unreadable.png', TEXT_DELETE, 'small', 'onclick="if (confirm(\'' . SHIPPING_DELETE_CONFIRM . '\')) window.open(\'index.php?module=shipping&page=popup_label_mgr&method=table&sID=' . $result->fields['shipment_id'] . '&action=delete\',\'popup_label_mgr\',\'width=1150,height=700,resizable=1,scrollbars=1,top=50,left=100\')"') . chr(10);
			echo '    </td>';
			echo '  </tr>' . chr(10);
			$result->MoveNext();
		}
