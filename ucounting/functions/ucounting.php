...
// +-----------------------------------------------------------------+
//  Path: /modules/ucounting/functions/ucounting.php
//
...
function load_company_dropdown($include_select = false) {
    ...
    foreach ($contents as $file) {
        if (strpos($file, '.') !== 0 && is_dir(DIR_FS_MY_FILES . $file)) {
            if (file_exists(DIR_FS_MY_FILES . $file . '/config.txt'))
                convert_cfg($file);
    ...
    if ($include_select)
        $output[0] = array('text' => TEXT_NONE, 'file' => 'none');
    $the_list = array();
...
function load_language_dropdown($language_directory = 'modules/ucounting/language/') {
    $output = array();
...
function convert_cfg($company) {
    ...
    $filename = DIR_FS_ADMIN . PATH_TO_MY_FILES . $company . '/config';
    if (!$handle = @fopen($filename . '.php', 'w'))
        die('Cannot open file (' . $filename . '.php) for writing, check your permissions. This directory and file needs access from the web server for upgrading UcBooks to the latest version.');
    if (!fwrite($handle, $lines))
        die('Cannot write to file (' . $filename . '.php), check your permissions.');
    fclose($handle);
    if (!unlink($filename . '.txt'))
        die('Cannot delete file (' . $filename . '.txt). This file needs to be deleted for security reasons.');
}

function gen_pull_db_config_info($database, $key) {
    $filename = DIR_FS_ADMIN . PATH_TO_MY_FILES . $database . '/config.txt';
    $lines = file($filename);
        ...
        foreach ($dirlist as $dir) {
            if (!file_exists($path . $dir)) {
                if (!@mkdir($path . $dir, 0755, true))
                    $error = $messageStack->add(sprintf(ERROR_CANNOT_CREATE_MODULE_DIR, $path . $dir), 'error');
            }
        }
    return $error;
}
...
function admin_remove_dirs($dirlist, $path = DIR_FS_MY_FILES) {
    ...
    if (is_array($dirlist)) {
        ...
        foreach ($temp as $dir) {
            if (!@rmdir($path . $dir))
                $error = $messageStack->add(sprintf(ERROR_CANNOT_REMOVE_MODULE_DIR, $path . $dir), 'error');
        }
    }
    return $error;
}
...
function admin_install_tables($tables) {
    ...
    foreach ($tables as $table => $create_table_sql) {
        ...
        $fp = fopen(DIR_FS_ADMIN . "user_company/" . $_POST['company_name'] . "/sql_backup/" . $table . '.txt', "wb");
        fwrite($fp, $content);
        fclose($fp);
...
function admin_add_report_heading($doc_title, $doc_group) {
    ...
    if ($result->RecordCount() < 1) {
        $db->Execute("INSERT INTO " . TABLE_UCFORM . " (parent_id, doc_type, doc_title, doc_group, doc_ext, security, create_date) VALUES 
...
function admin_add_report_folder($parent_id, $doc_title, $doc_group, $doc_ext) {
    ...
    if ($parent_id == '')
        return true;
    $result = $db->Execute("select id from " . TABLE_UCFORM . " where doc_group = '$doc_group' and doc_ext = '$doc_ext'");
    if ($result->RecordCount() < 1) {
        $db->Execute("INSERT INTO " . TABLE_UCFORM . " (parent_id, doc_type, doc_title, doc_group, doc_ext, security, create_date) VALUES 
...
function install_build_co_config_file($company, $key, $value) {
    global $messageStack;
    $filename = DIR_FS_ADMIN . PATH_TO_MY_FILES . $company . '/config.php';
...
function load_module_xml($module) {
    ...
    if (!$output = xml_to_object($result))
        return false;
    // fix some special cases, multi elements with single entries convert to arrays
    if (is_object($output->Module->Table))
        $output->Module->Table = array($output->Module->Table);
    return $output;
}
                            ...
                            for ($i = 1; $i <= MAX_IMPORT_CSV_ITEMS; $i++) {
                                foreach ($working->Field as $field) {
                                    if ($field->CanImport) {
                                        ...
                                        if ($i == 1)
                                            $legend .= '"' . $field->TagName . '_X: (' . $field->Type . ') - ' . $req . $field->Description . '"' . chr(10);
                                    }
                                }
                            }
                        }
    ...
    foreach ($data as $line) {
        ...
        foreach ($sql_array as $table => $count) {
            foreach ($count as $cnt => $table_array) {
                if ($table == 'contacts') {
                    if (empty($table_array['short_name']) || empty($table_array['type'])) {
                        $unaffected_row++;
                        continue;
                    }
                } else if ($table == 'address_book') {
                    if (empty($table_array['primary_name']) || empty($table_array['type'])) {
                        //$unaffected_row++;
                        continue;
                    }
                } else if ($table == 'inventory') {
                    if (empty($table_array['sku']) || preg_match("/[']/i", $table_array['sku'])) {
                        $unaffected_row++;
                        continue;
                    }
                } else if ($table == 'inventory_assy_list') {
                    if (empty($table_array['sku']) || preg_match("/[']/i", $table_array['sku'])) {
                        $unaffected_row++;
                        continue;
                    }
...
//echo 'inserting data: '; print_r($table_array); echo '<br>';
                // codeing by shaheen for checking duplicate inventry
                if ($table == 'contacts') {
                    $Fsql = mysql_query("SELECT * FROM " . TABLE_CONTACTS . " where short_name='" . db_input($table_array['short_name']) . "'");
                    if ($table_array['id']) {
                        $result = $db->Execute("SELECT * FROM " . TABLE_CONTACTS . " where id='" . $table_array['id'] . "'");
                        if ($result->RecordCount() > 0) {
                            $table_array['id'] = '';
                        }
                    }
                } else if ($table == 'inventory') {
                    $Fsql = mysql_query("SELECT * FROM " . TABLE_INVENTORY . " where sku='" . db_input($table_array['sku']) . "'");
                    if ($table_array['id']) {
                        $result = $db->Execute("SELECT * FROM " . TABLE_INVENTORY . " where id='" . $table_array['id'] . "'");
                        if ($result->RecordCount() > 0) {
                            $table_array['id'] = '';
                        }
                    }
                } else if ($table == 'address_book') {
                    $Fsql = mysql_query("SELECT * FROM " . TABLE_ADDRESS_BOOK . " where primary_name='" . db_input($table_array['primary_name']) . "'");

                    if ($table_array['address_id']) {
                        $result = $db->Execute("SELECT * FROM " . TABLE_ADDRESS_BOOK . " where address_id='" . db_input($table_array['address_id']) . "'");
                        if ($result->RecordCount() > 0) {
                            $table_array['address_id'] = '';
                        }
                    }
                    while ($row = mysql_fetch_array($Fsql)) {
                        ...
                        $table_array['address_id'] = $address_id;
                        $table_array['ref_id'] = $ref_id;
                        $Fsql = mysql_query("SELECT * FROM " . TABLE_ADDRESS_BOOK . " where address_id='" . $address_id . "'");
                        while ($row = mysql_fetch_array($Fsql)) {

                            $check_address = $row['address_id'];
                        }
                    }
                }
                ...
                if ($cnt == 0) { // main record, fetch id afterwards
                    if ($skuID) {
                        if (!empty($check_address) && $table == 'address_book') {
                            //$table_array['id'] = $skuID;
                            db_perform(DB_PREFIX . $table, $table_array, 'update', "address_id = " . $address_id . "");

                            $id = $address_id;
                        } else {
                            $table_array['id'] = $skuID;
                            db_perform(DB_PREFIX . $table, $table_array, 'update', "id = " . $skuID . "");
    ...
    return $affected_row . '_' . $unaffected_row;
}
...
function table_export_xml($structure, $db_table) {
    ...
    foreach ($structure->Module->Table as $table)
        if ($table->Name == $db_table) {
            $tag_map = array();
            foreach ($table->Field as $field)
                $tag_map[$field->Name] = $field->TagName;
    ...
    if ($needs_sync) {
        ...
        if (isset($add_list)) {
            foreach ($add_list as $value) { // find the field attributes and copy to field list table
                ...
                switch ($data_type) {
                    case 'date': $Params['type'] = 'date';
                        break;

                    case 'time': $Params['type'] = 'time';
                        break;
                    case 'datetime': $Params['type'] = 'date_time';
                        break;
                    case 'timestamp': $Params['type'] = 'time_stamp';
                        break;
                    case 'year': $Params['type'] = 'date';
                        break;
                    ...
                    case 'text':
                        ...
                        if ($data_type == 'tinytext' OR $data_type == 'tinyblob')
                            $Params['length'] = '255';
                        if ($data_type == 'text' OR $data_type == 'blob')
                            $Params['length'] = '65,535';
                        if ($data_type == 'mediumtext' OR $data_type == 'mediumblob')
                            $Params['length'] = '16,777,215';
                        if ($data_type == 'longtext' OR $data_type == 'longblob')
                            $Params['length'] = '4,294,967,295';
                        break;
                    case 'enum':
                    case 'set':
                        ...
                        foreach ($selections as $selection) {
                            ...
                            if ($myrow->fields['Default'] == $selection)
                                $set = 1; else
                                $set = 0;
