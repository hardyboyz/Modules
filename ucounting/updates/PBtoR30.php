...
// +-----------------------------------------------------------------+
//  Path: /modules/ucounting/updates/PBtoR10.php
//

// This script updates/converts UcBooks Release 2.1 to Ucounting Release 3.0
...
if (!db_field_exists(TABLE_USERS_PROFILES, 'dashboard_id')) {
  ...
  $db->Execute("UPDATE " . TABLE_USERS_PROFILES  . " SET module_id = 'ucbooks' WHERE dashboard_id = 'po_status'");
  $db->Execute("UPDATE " . TABLE_USERS_PROFILES  . " SET module_id = 'ucbooks' WHERE dashboard_id = 'so_status'");
  $db->Execute("UPDATE " . TABLE_USERS_PROFILES  . " SET module_id = 'ucbooks' WHERE dashboard_id = 'open_inv'");
  $db->Execute("UPDATE " . TABLE_USERS_PROFILES  . " SET module_id = 'ucbooks' WHERE dashboard_id = 'mini_financial'");
  $db->Execute("UPDATE " . TABLE_USERS_PROFILES  . " SET module_id = 'ucbooks' WHERE dashboard_id = 'todays_orders'");
  $db->Execute("UPDATE " . TABLE_USERS_PROFILES  . " SET module_id = 'ucbooks' WHERE dashboard_id = 'todays_sales'");
  $db->Execute("UPDATE " . TABLE_USERS_PROFILES  . " SET module_id = 'ucounting'   WHERE dashboard_id = 'company_links'");
  $db->Execute("UPDATE " . TABLE_USERS_PROFILES  . " SET module_id = 'ucounting'   WHERE dashboard_id = 'my_notes'");
  $db->Execute("UPDATE " . TABLE_USERS_PROFILES  . " SET module_id = 'ucounting'   WHERE dashboard_id = 'personal_links'");
  $db->Execute("UPDATE " . TABLE_USERS_PROFILES  . " SET module_id = 'ucform'  WHERE dashboard_id = 'favorite_reports'");
}
...
// check for modules installed, set status
                                                       ...
                                                       write_configure('MODULE_UCBOOKS_STATUS', '0.1');
                                                       write_configure('MODULE_UCCRM_STATUS',   '0.1');
                                                       write_configure('MODULE_UCHELP_STATUS',  '0.1');
...
// load the ucform module
gen_pull_language('ucform', 'admin');
require_once (DIR_FS_MODULES . 'ucform/config.php');
require_once (DIR_FS_MODULES . 'ucform/defaults.php');
require_once (DIR_FS_MODULES . 'ucform/classes/install.php');
$mInstall = new ucform_admin;
admin_install_dirs($mInstall->dirlist, DIR_FS_MY_FILES.$_SESSION['company'].'/');
admin_install_tables($mInstall->tables);
write_configure('MODULE_UCFORM_STATUS', constant('MODULE_UCFORM_VERSION'));
foreach ($mInstall->keys as $key => $value) write_configure($key, $value);
$mInstall->load_reports('ucform');
admin_add_reports('ucform');
$mInstall->install('ucform');
...
foreach ($contents as $entry) { // load the configuration files to load version info
  if ($entry <> '.' && $entry <> '..' && is_dir(DIR_FS_MODULES . $entry)) {
    if (defined('MODULE_' . strtoupper($entry) . '_STATUS') && $entry <> 'ucform') { // build the directories
