...
// +-----------------------------------------------------------------+
//  Path: /modules/ucdom/language/en_us/admin.php
//
...
define('HEADING_TITLE', 'Ucounting Login');
...
define('HEADING_TITLE_CRASH_TITLE','Ucounting SQL Error Trace Information');
define('HEADING_TITLE_CRASH_INFORMATION','Ucounting has encountered an unexpected error. Click on the button below to download the debug trace file information to send to the Ucounting Development Team for troubleshooting assistance.');
...
define('GEN_ADM_TOOLS_SEQ_DESC','Changes to the sequencing can be made here.<br />NOTE 1: Ucounting does not allow duplicate sequences, be sure the new starting sequence will not conflict with any currently posted values.<br />Note 2: The next deposit number is generated by the system and uses the current date.<br />Note 3: The next check number can be set at the payment screen prior to posting a payment and will continue from the entered value.');
...
define('GEN_ADM_TOOLS_CLEAN_LOG_DESC','This operation creates a downloaded backup of your audit log database file. This will help keep the database size down and reduce company backup file sizes. Backing up this log is recommended before cleaning out to preserve Ucounting transaction history. <br />INFORMATION: Cleaning out the audit log will leave the current periods data in the database table and remove all other records.');
...
define('GEN_IMPORT_EXPORT_MESSAGE','Importing can be through CSV format. Click on the sample button to download a sample file to use for formatting purposes.');
...
define('CD_08_03_DESC', 'Automatically check for program updates at login to Ucounting.');
...
define('CD_08_10_DESC', 'Limits the length of history values shown in customer/Suppliers accounts for sales/purchases.');
...
define('CD_15_05_DESC', 'When enabled, this option will use ajax to refresh the session timer every 5 minutes preventing the session from expiring and logging out the user. This feature helps prevent dropped posts when Ucounting has been inactive and a post results in a login screen.');
...
define('CD_09_01_DESC', 'Specifies the export preference when exporting reports and forms. Local will save them in the /'.PATH_TO_MY_FILES.'reports directory of the webserver for use with all companies. Download will download the file to your browser to save/print on your local machine.');
...
define('CD_01_09_DESC', 'The country this company is located.');
...
define('GEN_BACKUP_SAVE_LOCAL',' Save a local copy in webserver ('.PATH_TO_MY_FILES.'backups) directory');
...
define('SETUP_CO_MGR_COPY_HDR','Enter the database information for the new company. (Must conform to mysql naming conventions, typically 8-12 alphanumeric characters) This name is used as the database name and will be added to the '.PATH_TO_MY_FILES.' directory to hold company specific data. The database must exist prior to creating the company.');
