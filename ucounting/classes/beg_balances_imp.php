...
// +-----------------------------------------------------------------+
//  Path: /modules/ucbooks/classes/beg_balances_imp.php
//
...
//echo 'parsed string = '; print_r($this->records); echo '<br />';
  ...
  function submitJournalEntry() {
	...
	while($row_cnt < count($this->records)) {
	  ...
	  $glEntry->description         = $order['description'];//sprintf(TEXT_JID_ENTRY, constant('ORD_TEXT_' . JOURNAL_ID . '_WINDOW_TITLE'));
	  ...
	  $glEntry->product_desc        = $order['product_desc'];
    $glEntry->sales_tax           = $order['sales_tax'];
	  switch (JOURNAL_ID) {
		case 4:
		case 6:
	  ...
	  while(true) {
		...
		switch (JOURNAL_ID) {
		  ...
		  case 12: if (!$credit_debit) $credit_debit = 'credit_amount'; // for journal_id = 12
			$glEntry->journal_rows[] = array(
                                'item_cnt'   => $item_cnt, //add
				'gl_type'      => BB_GL_TYPE,
				//'qty'          => '1',
                                'sku'         => $order['sku'],
                                'qty'         => $currencies->clean_value($order['quantity']), //update
				'description'  => $order['description'],//constant('ORD_TEXT_' . JOURNAL_ID . '_WINDOW_TITLE') . '-' . TEXT_IMPORT,
				'gl_account'   => $order['inv_gl_acct'],
				'taxable'      => $order['taxable'] ? $order['taxable'] : 0,
				//$credit_debit  => $currencies->clean_value($order['total_amount']),
				$credit_debit  => $currencies->clean_value($order['total_cost']), //update
				'post_date'    => $order['post_date'],
			);
			break;
		  }
		  ...
		  if ((JOURNAL_ID == 4 || JOURNAL_ID == 10 || JOURNAL_ID == 12 || JOURNAL_ID == 6 ) && $order['order_id'] == $next_order) { // more line items
			$row_cnt++;
			$order = $this->records[$row_cnt];
		...
		$glEntry->journal_rows[] = array(
		  'gl_type'     => 'ttl',
		  'description' => $order['description'],//constant('ORD_TEXT_' . $glEntry->journal_id . '_WINDOW_TITLE') . '-' . TEXT_TOTAL,
		  'gl_account'  => $order['gl_acct'],
		  $debit_credit => $total_amount,
		  'post_date'   => $post_date,
		);
		//$glEntry->journal_main_array['total_amount'] = $total_amount;
    $glEntry->journal_main_array['total_amount'] = $total_amount+$order['sales_tax'];
