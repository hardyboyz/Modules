...
// +-----------------------------------------------------------------+
//  Path: /modules/ucounting/classes/install.php
//

class ucounting_admin {
    ...
    function __construct() {
        ...
        $this->keys = array(
            ...
            'EMAIL_SMTPAUTH_MAILBOX' => 'joel@ucounting.com',
            'EMAIL_SMTPAUTH_PASSWORD' => 'jl0511',
            'EMAIL_SMTPAUTH_MAIL_SERVER' => 'mail.ucouting.com',
            'EMAIL_SMTPAUTH_MAIL_SERVER_PORT' => '25',
            'CURRENCIES_TRANSLATIONS' => 'Â£,?:â‚¬,?',
            'DATE_FORMAT' => 'd/m/Y', // this is used for date(), use only values: Y, m and d (case sensitive)
            'DATE_DELIMITER' => '/', // must match delimiter used in DATE_FORMAT
            'DATE_TIME_FORMAT' => 'd/m/Y h:i:s a',
            'COMPANY_LOGO' => '',
            'SETUP_WIZARD_STATUS' => 'true',
            'PRINT_INVOICE_STYLE' => '1',
            'MODULE_OPENCART_STATUS' => '1.2',
            'OPENCART_URL' => '',
            'OPENCART_USERNAME' => '',
            'OPENCART_PASSWORD' => '',
            'OPENCART_PRODUCT_TAX_CLASS' => '',
            'OPENCART_USE_PRICE_SHEETS' => '0',
            'OPENCART_PRICE_SHEET' => '0',
            'OPENCART_STATUS_CONFIRM_ID' => '3',
            'OPENCART_STATUS_PARTIAL_ID' => '3',
            'AUTOMATICALLY_DO_POST' => '0',
            'MESSAGE_9_0' => 'Quotation Message',
            'MESSAGE_10_0' => 'Sales Orders Message',
            'MESSAGE_32_0' => 'Delivery Orders Message',
            'MESSAGE_12_0' => 'Sales Invoices Message',
            'MESSAGE_13_0' => 'POS Message',
            'MESSAGE_13_1' => 'Sales Credit Notes Message',
            'MESSAGE_12_1' => 'Sales Debit Notes Message',
            'MESSAGE_12_2' => 'Request for Quotes Message',
            'MESSAGE_3_0' => 'Request for Quotes Message ',
            'MESSAGE_4_0' => 'Purchase Orders Message',
            'MESSAGE_6_0' => 'Purchases Message',
            'MESSAGE_6_1' => 'Expenses Message',
            'MESSAGE_7_0' => 'Purchase Credit Notes Message',
            'MESSAGE_31_0' => 'Purchase Debit Notes Message',
            'MESSAGE_RECEIPT_DEFAULT' => 'Receipt Default Message',
            'MESSAGE_SPENDMONEY_DEFAULT' => 'Spend Money Default Message',
            'MESSAGE_RECEIVEMONEY_DEFAULT' => 'Receive Money Default Message',
            'MESSAGE_PPAYMENTVOUCHER_DEFAULT' => 'Purchase Payment Voucher default Message',
            'MESSAGE_INVENTORYADJUSTMENT_DEFAULT' => 'Inventory Adjustment default Message',
            'MESSAGE_INVENTORYTRANSFER_DEFAULT' => 'Inventory Transfer Default Message',
            'MESSAGE_GJVOUCHER_DEFAULT' => 'General Journal Voucher Default Message',
          
            'TEMPLATE_SHIPTO_BOX' => '',
            'TEMPLATE_RECEIPT_SIZE' => '',
            'TEMPLATE_ITEM_CODE'  => '',
        ...
        $this->tables = array(
		  ...
		  use_in_inventory_filter varchar(64) NOT NULL default '',
		  params text,
		  PRIMARY KEY (id)
		) ENGINE=MyISAM DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;",
    ...
    function install($module) {
        ...
        $currencies_list = array(
            array('title' => 'US Dollar', 'code' => 'USD', 'symbol_left' => '$', 'symbol_right' => '', 'decimal_point' => '.', 'thousands_point' => ',', 'decimal_places' => '2', 'decimal_precise' => '2', 'value' => 0.32, 'last_updated' => date('Y-m-d H:i:s')),
            array('title' => 'Euro', 'code' => 'EUR', 'symbol_left' => '', 'symbol_right' => 'EUR', 'decimal_point' => '.', 'thousands_point' => ',', 'decimal_places' => '2', 'decimal_precise' => '2', 'value' => 0.23, 'last_updated' => date('Y-m-d H:i:s')),
            array('title' => 'MYR', 'code' => 'MYR', 'symbol_left' => 'RM', 'symbol_right' => '', 'decimal_point' => '.', 'thousands_point' => ',', 'decimal_places' => '2', 'decimal_precise' => '2', 'value' => 1.00000000, 'last_updated' => date('Y-m-d H:i:s')),
        );
        foreach ($currencies_list as $entry)
            db_perform(TABLE_CURRENCIES, $entry, 'insert');
        write_configure('DEFAULT_CURRENCY', 'MYR');
    ...
    function initialize($loaded_modules) {
        ...
        if (web_connected(false) && AUTO_UPDATE_CURRENCY && ENABLE_MULTI_CURRENCY) {
            ...
            require_once(DIR_FS_MODULES . 'ucounting/classes/currency.php');
        ...
        if (web_connected(false) && CFG_AUTO_UPDATE_CHECK && (SECURITY_ID_CONFIGURATION > 3)) { // check for software updates
            ...
            if ($revisions) {
                ...
                $latest = $versions->Revisions->Ucounting->Current;
                $current = MODULE_UCOUNTING_VERSION;
                if ($latest > $current)
    ...
    function update($module) {
        global $db, $messageStack;
        $error = false;
        $db_version = defined('MODULE_UCOUNTING_STATUS') ? MODULE_UCOUNTING_STATUS : false;
        foreach ($this->keys as $key => $value)
            if (!defined($key))
                write_configure($key, $value);
        if ($db_version < MODULE_UCOUNTING_STATUS) {
            $db_version = $this->release_update($module, '3.0', DIR_FS_MODULES . 'ucounting/updates/PBtoR30.php');
            if (!$db_version)
                return true;
        }
        if (MODULE_UCOUNTING_STATUS < '3.1') {
            foreach ($this->tables as $table => $create_table_sql) {
                if (!db_table_exists($table))
                    if (!$db->Execute($create_table_sql))
                        $error = true;
            }
            write_configure(UCHELP_FORCE_RELOAD, '1');
        }
        if (MODULE_UCOUNTING_STATUS < '3.2') {
        ...
        if (MODULE_UCOUNTING_STATUS < '3.4') {
        ...
        if (MODULE_UCOUNTING_STATUS < '3.5') {
