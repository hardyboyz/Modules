...
// +-----------------------------------------------------------------+
//  Path: /modules/ucounting/pages/main/pre_process.php
//
/* * ************   Check user security   **************************** */
...
if ($_GET['req'] == 'pw_lost_sub')
    $action = 'pw_lost_sub';

if ($action != 'no_permission' && $action != 'logout') {  // codeing by shaheen for checking user's permission
    if (isset($_SESSION['admin_security'])) {
        $security_level = validate_user(constant($sub_menu_select));
    }
...
switch ($action) {
    case 'validate':
        ...
        if (preg_match("/([\w\-]+\@[\w\-]+\.[\w\-]+)/", $admin_name)) {
            $admin_name = strtolower($admin_name);
        }
        ...
        // $sql = "select admin_id, admin_name, inactive, display_name, admin_email, admin_pass, account_id, admin_prefs, admin_security,active_user 
        ...
        } else if (!pw_validate_password($admin_pass, $result->fields['admin_pass'])) {
            $error = true;
            $messageStack->add(ERROR_WRONG_LOGIN, 'error');
        }
//        else if($result->fields['active_user']==1){
//             $error = true;
//            $messageStack->add('You have no permission to access.Because another user use this credential at the same time.', 'error');
//        }
        if (!$error) {
            //$db->Execute("update " . TABLE_USERS . " set active_user = 1 where admin_id = " . $result->fields['admin_id']);
            ...
            require(DIR_FS_MODULES . 'ucounting/classes/install.php');
            $ucounting = new ucounting_admin();
            if (MODULE_UCOUNTING_STATUS <> MODULE_UCOUNTING_VERSION)
                $ucounting->update('ucounting');
            $ucounting->initialize($loaded_modules);
            ...
            //if ($user_array['account_status'] == 'trial' || (($user_array['account_status'] == 'active' && $user_array['plan'] == 1))) {
            if ($user_array['account_status'] == 'trial') {
                $current_date = strtotime(date('d-m-Y'));
                $total_day_time = 24 * 60 * 60 * $user_array['trial_day'];
                $one_day_time = 24 * 60 * 60;
                if (!empty($user_array['trial_modified'])) {
                    $gone_day_time = $current_date - intval($user_array['trial_modified']);
                } else {
                    $gone_day_time = $current_date - intval($user_array['created_on']);
                }

                $remaining_day_time = $total_day_time - $gone_day_time;
                $remaining_day = $remaining_day_time / $one_day_time;
                $_SESSION['trial_account'] = floor($remaining_day);
//                if ($user_array['account_status'] == 'active') {
//                    $_SESSION['account_status'] = 'trial';
//                } else {
                $_SESSION['account_status'] = $user_array['account_status'];
                //}
            } else if ($user_array['account_status'] == 'pre_active') {
                $_SESSION['account_status'] = $user_array['account_status'];
            }
            ...
            if (get_cfg_var('safe_mode'))
                $messageStack->add_session(SAFE_MODE_ERROR, 'error');
    ...
    case 'logout':
        ...
        //  $db->Execute("update " . TABLE_USERS . " set active_user = '0' where admin_id = " . $_SESSION['admin_id']);
...
switch ($action) {
    ...
    case 'pw_lost_req':
        ...
        if ($_POST['company']) { // find default company
            ...
            if ($_COOKIE['pb_company'])
                $default_company = $_COOKIE['pb_company'];
            foreach ($_SESSION['companies'] as $key => $value) {
                if ($value == $default_company)
                    $company_index = $key;
            }
        }
        if ($_POST['language']) { // find default language
            ...
            if ($_COOKIE['pb_language'])
                $default_language = $_COOKIE['pb_language'];
            foreach ($languages as $value) {
                if ($value['id'] == $default_language)
                    $language_index = $value['id'];
            }
        }
    ...
    case 'crash':
        ...
        break;
    case 'no_permission':
        $include_template = 'no_permission.php';
        define('PAGE_TITLE', TITLE);
        break;
    default: // prepare to display templates
        if (!class_exists('queryFactory')) { // Errors will happen here if there was a problem logging in, logout and restart
            session_destroy();
            gen_redirect(html_href_link(FILENAME_DEFAULT, 'module=ucounting&amp;page=main&amp;action=login', 'SSL'));
        }
        if ($sub_menu_select == 'home_dashboard') {
            $dashboard1st = 'dashboard';
        } else if($sub_menu_select == 'Sales_Dashboard'){
            $dashboard1st = 'sales_dashboard';
        }else if($sub_menu_select == 'Purchases_Dashboard'){
              $dashboard1st = 'purchase_dashboard';
        }

        if (!$_SESSION['dashboard_visited']) {
            ...
            if ($status->configuration_value == 'true') {
                ...
                $cp_boxes = $db->Execute("select * from " . TABLE_USERS_PROFILES . " 
                    where user_id = '" . $_SESSION['admin_id'] . "' and menu_id = '$menu_id' ORDER BY CASE WHEN dashboard_id = '".$dashboard1st."' THEN 1 ELSE dashboard_id end");
                $total = $db->Execute("select count(*) from " . TABLE_USERS_PROFILES . " 
                    where user_id = '" . $_SESSION['admin_id'] . "' and menu_id = '$menu_id' ORDER BY CASE WHEN dashboard_id = '".$dashboard1st."' THEN 1 ELSE dashboard_id end");
                $include_template = 'template_main.php';
                define('PAGE_TITLE', COMPANY_NAME . ' - ' . TITLE);
            }
        } else {
          $var = "select * from " . TABLE_USERS_PROFILES . " where user_id = '" . $_SESSION['admin_id'] . "' and menu_id = '$menu_id' ORDER BY CASE WHEN dashboard_id = '".$dashboard1st."' THEN 1 ELSE dashboard_id end";
            $cp_boxes = $db->Execute("select * from " . TABLE_USERS_PROFILES . " 
                    where user_id = '" . $_SESSION['admin_id'] . "' and menu_id = '$menu_id' ORDER BY CASE WHEN dashboard_id = '".$dashboard1st."' THEN 1 ELSE dashboard_id end");
            $total = $db->Execute("select count(*) as total from " . TABLE_USERS_PROFILES . " 
                    where user_id = '" . $_SESSION['admin_id'] . "' and menu_id = '$menu_id' ORDER BY CASE WHEN dashboard_id = '".$dashboard1st."' THEN 1 ELSE dashboard_id end");
            $include_template = 'template_main.php';
            define('PAGE_TITLE', COMPANY_NAME . ' - ' . TITLE);
        }
