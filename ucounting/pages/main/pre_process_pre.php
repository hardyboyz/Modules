    ...
    case 'pw_lost_sub':
	  ...
	  from " . TABLE_USERS . " where admin_email = '" . db_input($admin_email) . "'");
        ...
        if (!$error) {
            ...
            validate_send_mail($result->fields['admin_name'], $result->fields['admin_email'], TEXT_EMAIL_SUBJECT, $html_msg['EMAIL_MESSAGE_HTML'], COMPANY_NAME, 'no-reply@smeucounting.com', $html_msg);
...
switch ($action) {
    ...
    default: // prepare to display templates
        ...
        if (!$_SESSION['dashboard_visited']) {
            ...
            $var = "select * from " . TABLE_USERS_PROFILES . " where user_id = '" . $_SESSION['admin_id'] . "' and menu_id = '$menu_id' order by column_id, row_id";
            $cp_boxes = $db->Execute("select * from " . TABLE_USERS_PROFILES . " 
                    where user_id = '" . $_SESSION['admin_id'] . "' and menu_id = '$menu_id' order by column_id, row_id");
            $include_template = 'template_main.php';
            define('PAGE_TITLE', COMPANY_NAME . ' - ' . TITLE);
