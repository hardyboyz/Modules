...
if ($_GET['action'] == 'first_dashboard') {
    $menu_select = 'home';
    $sub_menu_select = 'setup_wizard';
} else if ($_POST['first_dashboard'] == 'first_dashboard') {
    $menu_select = 'home';
    $sub_menu_select = 'setup_wizard';
    $action = 'save';
} else if ($_GET['action'] == 'fourth_dashboard') {
    $menu_select = 'home';
    $sub_menu_select = 'setup_wizard';
} else {
    $menu_select = 'company';
...
$core_modules = array('ucounting', 'ucform', 'ucbooks', 'contacts', 'inventory', 'uchelp', 'payment'); // ucform first!
...
$install = new ucounting_admin();
$currency = new currency();
// load other module admin information
$page_list = array();
if ($dir = @dir(DIR_FS_MODULES)) {
    while ($file = $dir->read()) {
        if ($file == "bank_transfer") {
            continue;
        }
        if (is_dir(DIR_FS_MODULES . $file) && $file <> '.' && $file <> '..') {
            if (file_exists(DIR_FS_MODULES . $file . '/config.php') && $file <> 'ucounting') {
...
while (!$result->EOF) {
    if ($result->fields['Field'] <> 'id')
        $status_fields[] = $result->fields['Field'];
    $result->MoveNext();
}
$status_values = $db->Execute("select * from " . TABLE_CURRENT_STATUS);


if (isset($_GET['action_type']) && $_GET['action_type'] == 'renew_trial') {

    if (isset($_POST['client_choose']) && $_POST['client_choose'] == 'request_sales_person') {
        ...
        $to_name = 'Ucounting Administrator';
        $to_email = 'apu@4axiz.com';
//    $to_email = 'joel@feradigm.com';
        $message_subject = "Request to meet Sales person for a LIVE demo \n\n";
        $email_text = "Hello, Ucounting Administrator. \n\n";
        $email_text .= "I want to see Live Demo of Ucountng application. Please have a visit at my office \n\n";
        $email_text .= "My Company Information is: \n\n";
        $email_text .= "First Name: " . $user_array['first_name'] . "\n";
        $email_text .= "Last Name: " . $user_array['last_name'] . "\n";
        $email_text .= "Email: " . $user_array['email'] . "\n";
        $email_text .= "Phone Number: " . $user_array['phone_number'] . "\n";
        $email_text .= "Company Name: " . $user_array['company_name'] . "\n";
        $email_text .= "Company Code: " . $user_array['company_code'] . "\n";
        $email_text .= "Link: <a href='http://cheshta_" . $user_array['company_code'] . ".ucounting.com'>cheshta_" . $user_array['company_code'] . ".ucounting.com</a>\n";
        $email_text .= "Country : " . $user_array['country'] . "\n";
        $email_text .= "\n\nThanks\n\n" . $user_array['first_name'] . " " . $user_array['last_name'];
    ...
    } else if (isset($_POST['client_choose']) && $_POST['client_choose'] == 'renew_trial') {
        if (!mysql_select_db(LANDING_DB)) {
            return false;
        }
        $user_qry = "select * from user_account where company_code = '" . COMPANY_CODE . "';";
        ...
        $user_array = mysql_fetch_assoc($user_result);
        $today = time();
        $query = "UPDATE user_account SET modified_on=" . $today . ",trial_modified=" . $today . " WHERE company_code='" . COMPANY_CODE . "'";
    ...
    } else if (isset($_POST['client_choose']) && $_POST['client_choose'] == 'start_company') {
...
switch ($action) {
    ...
    case 'update':
        ...
        if ($action == 'install') {
            if (admin_check_versions($method, $mInstall->prerequisites)) { // Check for version levels
                ...
                if ($demo)
                    $error = $mInstall->load_demo(); // load demo data
                $mInstall->load_reports($method);
                admin_add_reports($method);
            }
            if ($mInstall->install($method))
                $error = true; // install any special stuff
        } else {
            if ($mInstall->update($method))
                $error = true;
        }
        if ($error)
            break;
        if (sizeof($mInstall->notes) > 0)
            foreach ($mInstall->notes as $note)
                $messageStack->add($note, 'caution');
    ...
    case 'save':
        ...
        if (empty($_POST['company_name']))
            $error = $messageStack->add('Please Enter Company Name', 'error');
        if (empty($_POST['company_id']))
            $error = $messageStack->add('Please Enter Company ID', 'error');
        if (empty($_POST['company_address1']))
            $error = $messageStack->add('Please Enter Company Addres Line1 ', 'error');
        if (empty($_POST['company_postal_code']))
            $error = $messageStack->add('Please Enter Postal Code', 'error');
        if (empty($_POST['company_city_town']))
            $error = $messageStack->add('Please Enter Company City', 'error');
        if (empty($_POST['company_zone']))
            $error = $messageStack->add('Please Enter Company State', 'error');
        if (empty($_POST['company_country']))
            $error = $messageStack->add('Please Enter Company Country', 'error');
        if (!empty($error)) {
            gen_redirect(html_href_link(FILENAME_DEFAULT, 'module=ucounting&page=admin&action=first_dashboard', 'SSL'));
        }
        // save general tab
  
        //NEW CONFIG for TEMPLATE
        if(!isset($_POST['template_shipto_box'])){
            $_POST['template_shipto_box']=0;
          }
        
        if(!isset($_POST['template_receipt_size'])){
            $_POST['template_receipt_size']=0;
          }
  
        if(!isset($_POST['template_item_code'])){
            $_POST['template_item_code']=0;
          }
        //END NEW CONFIG
        ...
        foreach ($install->keys as $key => $default) {
            ...
            if (isset($_POST[$field]))
                write_configure($key, db_prepare_input($_POST[$field]));
            if (!empty($_FILES['company_logo']['name'])) {
                if (isset($_FILES[$field]))
                    write_configure($key, db_prepare_input(strtolower($_FILES[$field]['name'])));
            }
            // special case for field COMPANY_NAME to update company config file
            if ($key == 'COMPANY_NAME' && $_POST[$field] <> COMPANY_NAME) {
                if (!install_build_co_config_file($_SESSION['company'], $_SESSION['company'] . '_TITLE', db_prepare_input($_POST[$field])))
                    $error = true;
            }
        }
        if (!empty($_FILES['company_logo']['name'])) {
            $target_path = PATH_TO_MY_FILES . $_SESSION['company'] . '/ucform/images/company_logo';
            if (!is_dir($target_path)) {
                mkdir($target_path, 0777);
            }
            $target_path = $target_path . '/' . strtolower($_FILES['company_logo']['name']);
...
//	gen_redirect(html_href_link(FILENAME_DEFAULT, gen_get_all_get_params(array('action')), 'SSL'));
    ...
    case 'copy_co':
        ...
        if (!$db_name || !$co_name)
            $error = $messageStack->add(SETUP_CO_MGR_ERROR_EMPTY_FIELD, 'error');
        if ($db_name == $_SESSION['company'])
            $error = $messageStack->add(SETUP_CO_MGR_DUP_DB_NAME, 'error');
        ...
        if (!$error) {
            ...
            foreach ($copy_modules as $entry) {
                ...
                if ($entry == 'ucounting')
                    $task = 'data'; // force complete copy of ucounting module
                switch ($task) {
                    case 'core':
                    case 'demo':
                        if (admin_install_dirs($install_mod->dirlist, DIR_FS_MY_FILES . $db_name . '/')) {
                            ...
                            if ($task == 'demo')
                                if ($install_mod->load_demo())
                                    $error = true; // load demo data
                                    if ($entry <> 'ucounting')
                                $install_mod->load_reports($entry);
                            if ($entry == 'ucform') {
                                $temp_mod = new ucounting_admin;
                                $temp_mod->load_reports('ucounting');
                            }
                        }
            ...
            if ($_POST['ucform_action'] <> 'data') { // if=data reports have been copied, else load basic reports
                foreach ($copy_modules as $entry)
                    admin_add_reports($entry, DIR_FS_MY_FILES . $db_name . '/ucform/');
            }
        }
        if (!$error && $_POST['ucbooks_action'] <> 'data') { // install fiscal year if the ucbooks data is not copied
...
//this is for first_dashboard coding by 4axiz(apu) date -18_06_2013
if ($action == 'first_dashboard' || $_POST['first_dashboard'] == 'first_dashboard') {
    $include_header = true;
    $include_footer = true;
    $setupwizard_menu = 'step_1';
    $include_template = 'template_first_dashboard.php';

    define('PAGE_TITLE', BOX_GENERAL_ADMIN);
} else if ($action == 'fourth_dashboard') {
    $include_header = true;
    $include_footer = true;
    $setupwizard_menu = 'step_4';
    $include_template = 'template_fourth_dashboard.php';
    define('PAGE_TITLE', BOX_GENERAL_ADMIN);
} else if ($action == 'fifth_dashboard') {
    $include_header = true;
    $include_footer = true;
    $setupwizard_menu = 'step_5';
    $include_template = 'template_fifth_dashboard.php';
    define('PAGE_TITLE', BOX_GENERAL_ADMIN);
}

if (isset($_POST['wizard'])) {
    if ($_POST['wizard'] == 'false') {
        $update_val = "true";
    } else {
        $update_val = 'false';
    }
    $query = "UPDATE configuration SET configuration_value='" . $update_val . "' WHERE configuration_key = 'SETUP_WIZARD_STATUS' ";
