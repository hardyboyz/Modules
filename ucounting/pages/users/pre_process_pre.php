...
// +-----------------------------------------------------------------+
//  Path: /modules/ucounting/pages/users/pre_process.php
//
...
switch ($action) {
    ...
    case 'fill_role':
        ...
        if ($security_level < 3 && $admin_id)
            $error = $messageStack->add(GEN_ADMIN_CANNOT_CHANGE_ROLES, 'error');
        if ($action == 'fill_role') {
        ...
        if (!$error) {
            ...
            if ($admin_id == $_SESSION['admin_id'])
                $_SESSION['admin_security'] = gen_parse_permissions($admin_security); // update if user is current user
        } elseif ($error) {
            $action = 'edit';
        }
...
switch ($action) {
    ...
    case 'fill_role':
//        if ($action == 'new') {
//            $query = $db->Execute("SELECT plan from " . TABLE_USERS . " where admin_id=1");
//            while (!$query->EOF) {
//                $plan = $query->fields['plan'];
//                if ($plan == 1) {
//                    $permit_user = 1;
//                } elseif ($plan == 2) {
//                    $permit_user = 4;
...
//            $query1 = $db->Execute("SELECT COUNT(*) as total_user from " . TABLE_USERS . "");
//            while (!$query1->EOF) {
//                if ($query1->fields['total_user'] >= $permit_user) {
//                    $error = true;
//                    $messageStack->add('Your permited user account created has finished. please upgrade your account to create more user.', 'error');
//
//                    define('PAGE_TITLE', BOX_HEADING_USERS);
//                    $include_header = true;
//                    $include_footer = true;
//                    $include_template = 'template_main.php';
//                    $ajax_source_datatable = html_href_link(FILENAME_DEFAULT, 'module=ucounting&page=ajax&op=load_users', 'SSL');
//
//                    break;
//                }
//                $query1->MoveNext();
