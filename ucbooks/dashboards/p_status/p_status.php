...
define('DASHBOARD_P_STATUS_VERSION','3.2');

require_once(DIR_FS_MODULES . 'ucbooks/functions/ucbooks.php');

class p_status extends ctl_panel {
  function __construct() {
    $this->max_length = MAX_NUM_P_LIST;
  }
  ...
  function Output($params) {
	...
	$control .= CP_P_STATUS_SORT_ORDER     .'&nbsp'.html_pull_down_menu('p_status_field_1', $list_order, $params['order']).'<br />';
	$control .= CP_P_STATUS_HIDE_FUTURE    .'&nbsp'.html_pull_down_menu('p_status_field_2', $list_limit, $params['limit']);
	...
	$sql = "select id, post_date, purchase_invoice_id, bill_primary_name, total_amount, currencies_code, currencies_value 
	  from " . TABLE_JOURNAL_MAIN . " where journal_id = 6 and closed = '0'";
	...
	return $this->build_div(CP_P_STATUS_TITLE, $contents, $control);
  }
