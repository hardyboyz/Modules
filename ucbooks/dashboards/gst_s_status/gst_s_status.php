...
// +-----------------------------------------------------------------+
//  Path: /modules/ucbooks/dashboards/p_status/p_status.php
//
// Revision history
// 2011-07-01 - Added version number for revision control
define('DASHBOARD_GST_S_STATUS_VERSION','3.2');

class gst_s_status extends ctl_panel {
  function __construct() {
    $this->max_length = MAX_NUM_GST_S_LIST;
  }
  ...
  function Output($params) {
	...
	$control .= TEXT_SHOW.TEXT_SHOW_NO_LIMIT.'&nbsp'.html_pull_down_menu('p_status_field_0', $list_length,$params['num_rows']).'<br />';
	$control .= CP_GST_S_STATUS_SORT_ORDER     .'&nbsp'.html_pull_down_menu('p_status_field_1', $list_order, $params['order']).'<br />';
	$control .= CP_GST_S_STATUS_HIDE_FUTURE    .'&nbsp'.html_pull_down_menu('p_status_field_2', $list_limit, $params['limit']);
	$control .= html_submit_field('sub_p_status', TEXT_SAVE);
	$control .= '  </div>';
	$control .= '</div>';
	// Build content box
	$sql = "select id, post_date, purchase_invoice_id, bill_primary_name, total_amount, currencies_code, currencies_value 
	  from " . TABLE_JOURNAL_MAIN . " where journal_id = 12 and closed = '0' and post_date <= '".date("Y-m-d", strtotime("-6 months", strtotime(date('Y-m-d'))))."'";
	...
	if ($result->RecordCount() < 1) {
	  ...
	  while (!$result->EOF) {
		...
		$contents .= '<div>';
		$contents .= '<a href="' . html_href_link(FILENAME_DEFAULT, 'module=ucbooks&amp;page=orders&amp;oID=' . $result->fields['id'] . '&amp;jID=12&amp;action=edit', 'SSL') . '">';
  ...
  return $this->build_div(CP_GST_S_STATUS_TITLE, $contents, $control);
}

  function Update() {
	global $db;
	$params = array(
	  'num_rows'=> db_prepare_input($_POST['p_status_field_0']),
	  'order'   => db_prepare_input($_POST['p_status_field_1']),
	  'limit'   => db_prepare_input($_POST['p_status_field_2']),
	);
