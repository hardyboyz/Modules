...
define('DASHBOARD_GST_P_STATUS_VERSION','3.2');

class gst_p_status extends ctl_panel {
  function __construct() {
    $this->max_length = MAX_NUM_GST_P_LIST;
  }
  ...
  function Output($params) {
	...
	$control .= CP_GST_P_STATUS_SORT_ORDER     .'&nbsp'.html_pull_down_menu('p_status_field_1', $list_order, $params['order']).'<br />';
	$control .= CP_GST_P_STATUS_HIDE_FUTURE    .'&nbsp'.html_pull_down_menu('p_status_field_2', $list_limit, $params['limit']);
	...
	$sql = "select id, post_date, purchase_invoice_id, bill_primary_name, total_amount, currencies_code, currencies_value 
	  from " . TABLE_JOURNAL_MAIN . " where journal_id = 6 and closed = '0' and post_date <= '".date("Y-m-d", strtotime("-6 months", strtotime(date('Y-m-d'))))."'";
	...
	if ($result->RecordCount() < 1) {
	  ...
	  while (!$result->EOF) {
		...
		$contents .= '<div>';
		$contents .= '<a href="' . html_href_link(FILENAME_DEFAULT, 'module=ucbooks&amp;page=orders&amp;oID=' . $result->fields['id'] . '&amp;jID=6&amp;action=edit', 'SSL') . '">';
  ...
  return $this->build_div(CP_GST_P_STATUS_TITLE, $contents, $control);
}
