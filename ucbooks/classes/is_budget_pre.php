...
class is_budget {
  ...
  function add_income_stmt_data($type, $negate = false) {
	...
	$account_array = array();
	// current period
	$sql = "select c.id, c.description, h.debit_amount - h.credit_amount as balance, budget,h.account_id as account_id   
	...
	$sql = "select (sum(h.debit_amount) - sum(h.credit_amount)) as balance, sum(budget) as budget  ,h.account_id as account_id
		from " . TABLE_CHART_OF_ACCOUNTS . " c inner join " . TABLE_CHART_OF_ACCOUNTS_HISTORY . " h on c.id = h.account_id
...
*/
	...
	while (!$cur_period->EOF) {
		...
		$temp[$cur_period->fields['id']]['account_id']  = $ytd_period->fields['account_id'];
