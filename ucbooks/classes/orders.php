...
// +-----------------------------------------------------------------+
//  Path: /modules/ucbooks/classes/orders.php
//
...
class orders extends journal {
    ...
    function post_ordr($action, $recurring = null) {
        ...
        switch ($this->journal_id) { // THE SEQUENCE IS IMPORTANT!
            ...
            case 31:  // //                        $debit_total += $this->add_total_journal_row('debit'); // put total value into ledger row array
//                        break;
                // }
                // break;purchase debit memo  
            ...
            case 36: // Sales/Invoice Journal (Invoice)
            case 30: // Sales Debit Memo
        ...
        if ($this->ship_add_update) { // shipping address
            if (!$this->ship_acct_id)
                $this->ship_acct_id = $this->bill_acct_id; // set to bill if adding contact and id not set.
            if ($this->bill_address_id == $this->ship_address_id)
                $this->ship_address_id = ''; // force create new ship address, here from copy button
        ...
        if ($this->recur_id > 0) { // if new record, will contain count, if edit will contain recur_id
            ...
            if ($this->id) { // it's an edit, fetch list of affected records to update if roll is enabled
                ...
                for ($i = 0; $i < $this->recur_id; $i++) {
                    ...
                    $this->journal_main_array['period'] = $this->period;
                    $this->journal_main_array['terminal_date'] = $this->terminal_date;
                    if (in_array($this->journal_id, array(4, 10, 12, 36, 19)) && $first_purchase_invoice_id == '') {
            ...
            if (in_array($this->journal_id, array(4, 10, 32, 12, 36, 19)) && $first_purchase_invoice_id == '') { // special case for auto increment 
                $first_purchase_invoice_id = $this->purchase_invoice_id;
            }
            $this->id = $first_id;
            $this->post_date = $first_post_date;
        ...
        switch ($this->journal_id) {
            ...
            case 36: // Sales/Invoice Journal
            case 13: // Customer Credit Memo Journal
            case 19: // Customer Debit Memo Journal
        ...
        if ($recurring == "recurring") {
            $return_recurring_value['journal_main'] = $this->journal_main_array;
            $return_recurring_value['journal_main_item'] = $this->journal_rows;
            return $return_recurring_value;
        } else {
...
//echo 'committed transaction - bailing!'; exit();
        ...
        switch ($this->journal_id) {
            ...
            case 10: // Sales Order Journal
                ...
                if ($result->RecordCount() > 0)
                    return $this->fail_message(constant('GENERAL_JOURNAL_' . $this->journal_id . '_ERROR_6'));
                break;
            case 32: // Delivery Order Journal
            ...
            case 36: // Sales/Invoice Journal
            case 13: // Customer Credit Memo Journal
            case 19: // Customer Credit Memo Journal
            case 30: // Customer Debit Memo Journal
            case 31: // Vendor Debit Memo journal
    ...
    function add_total_journal_row($debit_credit) { // put total value into ledger row array
        if ($debit_credit == 'debit' || $debit_credit == 'credit') {
            $this->journal_rows[] = array(// record for accounts receivable
                ...
                'description' => constant('ORD_TEXT_' . $this->journal_id . '_' . $this->sub_journal_id . '_WINDOW_TITLE') . '-' . TEXT_TOTAL,
    ...
    function add_freight_journal_row($debit_credit) { // put freight into journal row array
        if ($debit_credit == 'debit' || $debit_credit == 'credit') {
            switch ($this->journal_id) {
                ...
                case 6:
                case 31:
                case 7: $freight_tax_id = AP_ADD_SALES_TAX_TO_SHIPPING;
                    break;
                case 9:
                case 10:
                case 32:
                ...
                case 36:
                case 30:
                case 13: $freight_tax_id = AR_ADD_SALES_TAX_TO_SHIPPING;
                case 19: $freight_tax_id = AR_ADD_SALES_TAX_TO_SHIPPING;
                    break;
            }
    ...
    function add_item_journal_rows($debit_credit) { // read in line items and add to journal row array
        if ($debit_credit == 'debit' || $debit_credit == 'credit') {
            ...
            for ($i = 0; $i < count($this->item_rows); $i++) {
                ...
                if ($this->item_rows[$i][$qty_pstd] || $this->is_discount == 1) { // make sure the quantity line is set and not zero
                    $this->journal_rows[] = array(
                        ...
                        'uom' => $this->item_rows[$i]['uom'],
                        'uom_qty' => $this->item_rows[$i]['uom_qty'],
                        'discval' => $this->item_rows[$i]['discval'],
                        'disc_percent' => $this->item_rows[$i]['disc_percent'],
    ...
    //this function for insert invoice from recurring table coding by 4axz(apu) date:17-july-2013
    function create_insert_query($table_name, $query_item) {
        unset($query_item['id']);
        $field_name = "";
        $field_val = "";
        foreach ($query_item as $key => $val) {
            $field_name .= $key . ", ";
            $field_val .= "'" . $val . "', ";
        }
        ...
        $query_str = "INSERT INTO " . $table_name . "(" . $field_name . ") VALUES (" . $field_val . ")";
        return $query_str;
    }
