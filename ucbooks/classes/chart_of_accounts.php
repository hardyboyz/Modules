...
// +-----------------------------------------------------------------+
//  Path: /modules/ucbooks/classes/chart_of_accounts.php
//
...
class chart_of_accounts {
  ...
  function build_main_html() {
	...
	$content['thead'] = array(
	  ...
	  'params' => 'width="100%" cellspacing="0" cellpadding="1"',
       ...
       // $sql = "select id, description, heading_only, primary_acct_id, account_type, account_inactive from " . $this->db_table;
	...
	while (!$result->EOF) {
	  $bkgnd = ($result->fields['account_inactive']) ? 'class="ui-state-error" ' : '';
          $aa = 'COA_' . str_pad($result->fields['account_type'], 2, "0", STR_PAD_LEFT) . '_DESC';
  ...
  function build_form_html($action, $id = '') {
	...
	$output .= '    <td>' . ($action == 'new' ? html_input_field('id', $this->id==0 ? '':$this->id,'required="required"') : $this->id) . '</td>' . "\n";
    $output .= '  </tr>' . "\n";
	$output .= '  <tr>' . "\n";
