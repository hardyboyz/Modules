...
// +-----------------------------------------------------------------+
//  Path: /modules/ucbooks/classes/install.php
//
class ucbooks_admin {
	...
	$this->tables = array(
	  ...
	  TABLE_JOURNAL_ITEM => "CREATE TABLE " . TABLE_JOURNAL_ITEM . " (
		  ...
		  date_1 datetime NOT NULL default '0000-00-00 00:00:00',
                  ...
                  discval double default '0',
                  disc_percent double default '0',
		  PRIMARY KEY  (id),
		  KEY ref_id (ref_id),
	  ...
	  TABLE_JOURNAL_MAIN => "CREATE TABLE " . TABLE_JOURNAL_MAIN . " (
		  ...
		  journal_id int(2) NOT NULL default '0',
		  sub_journal_id int(2) NOT NULL default '0',
		  post_date date NOT NULL default '0000-00-00',
		  ...
		  currencies_code char(3) NOT NULL DEFAULT 'MYR',
		  ...
		  bill_primary_name text,
		  bill_contact varchar(255) default NULL,
		  bill_address1 varchar(255) default NULL,
		  bill_address2 varchar(255) default NULL,
		  bill_city_town varchar(255) default NULL,
		  bill_state_province varchar(255) default NULL,
		  ...
		  ship_primary_name varchar(255) default NULL,
		  ship_contact varchar(255) default NULL,
		  ship_address1 varchar(255) default NULL,
		  ship_address2 varchar(255) default NULL,
		  ship_city_town varchar(255) default NULL,
		  ...
		  drop_ship enum('0','1') NOT NULL default '0',
                  from_recurring tinyint(1) NOT NULL DEFAULT '0',
                  message text COLLATE utf8_unicode_ci NOT NULL,
                  note text COLLATE utf8_unicode_ci NOT NULL,
                  product_desc varchar(250) COLLATE utf8_unicode_ci DEFAULT NULL,
  ...
  function install($module, $demo = false) {
	...
	if (!db_field_exists(TABLE_CURRENT_STATUS, 'next_do_num'))       $db->Execute("ALTER TABLE ".TABLE_CURRENT_STATUS." ADD next_do_num VARCHAR( 16 ) NOT NULL DEFAULT 'DO1000';");
	...
	if (!db_field_exists(TABLE_CURRENT_STATUS, 'next_pur_num'))      $db->Execute("ALTER TABLE ".TABLE_CURRENT_STATUS." ADD next_pur_num VARCHAR( 16 ) NOT NULL DEFAULT '30000';");
	...
	if (!db_field_exists(TABLE_CURRENT_STATUS, 'next_dm_num'))       $db->Execute("ALTER TABLE ".TABLE_CURRENT_STATUS." ADD next_dm_num VARCHAR( 16 ) NOT NULL DEFAULT 'DM1000';");
	...
	if (!db_field_exists(TABLE_CURRENT_STATUS, 'next_vdm_num'))      $db->Execute("ALTER TABLE ".TABLE_CURRENT_STATUS." ADD next_vdm_num VARCHAR( 16 ) NOT NULL DEFAULT 'VDM1000';");
	...
	if (!db_field_exists(TABLE_CURRENT_STATUS, 'bank_transfer')) $db->Execute("ALTER TABLE ".TABLE_CURRENT_STATUS." ADD bank_transfer VARCHAR( 16 ) NOT NULL DEFAULT 'BT-1000';");
	if (!db_field_exists(TABLE_CURRENT_STATUS, 'received_money')) $db->Execute("ALTER TABLE ".TABLE_CURRENT_STATUS." ADD received_money VARCHAR( 16 ) NOT NULL DEFAULT 'RCV-1000';");
	if (!db_field_exists(TABLE_CURRENT_STATUS, 'spend_money')) $db->Execute("ALTER TABLE ".TABLE_CURRENT_STATUS." ADD spend_money VARCHAR( 16 ) NOT NULL DEFAULT 'PV-1000';");
	if (!db_field_exists(TABLE_CURRENT_STATUS, 'next_expenses_num')) $db->Execute("ALTER TABLE ".TABLE_CURRENT_STATUS." ADD next_expenses_num VARCHAR( 16 ) NOT NULL DEFAULT 'EX-1000';");
	if (!db_field_exists(TABLE_CURRENT_STATUS, 'sales_cust_deposit')) $db->Execute("ALTER TABLE ".TABLE_CURRENT_STATUS." ADD sales_cust_deposit VARCHAR( 16 ) NOT NULL DEFAULT 'SCD-1000';");
	// copy standard images to ucform images directory
	$dir_source = DIR_FS_MODULES  . 'ucbooks/images/';
	$dir_dest   = DIR_FS_MY_FILES . $_SESSION['company'] . '/ucform/images/';
	@copy($dir_source . 'ucbooks_logo.jpg', $dir_dest . 'ucbooks_logo.jpg');
	@copy($dir_source . 'ucbooks_logo.png', $dir_dest . 'ucbooks_logo.png');
	$this->notes[] = MODULE_UCBOOKS_NOTES_1;
	$this->notes[] = MODULE_UCBOOKS_NOTES_2;
	$this->notes[] = MODULE_UCBOOKS_NOTES_3;
	$this->notes[] = MODULE_UCBOOKS_NOTES_4;
    return $error;
  }
  ...
  function update($module) {
    global $db, $messageStack;
	$error = false;
	$db_version = defined('MODULE_UCBOOKS_STATUS') ? MODULE_UCBOOKS_STATUS : false;
	if (!$db_version || $db_version < '2.1') { // For UcBooks release 2.1 or lower to update to Ucounting structure
	  require(DIR_FS_MODULES . 'ucbooks/functions/updater.php');
	  $result = $db->Execute("select * from " . TABLE_PROJECT_VERSION . " WHERE project_version_key = 'UcBooks Database'");
	...
	if ($db_version == '2.1') {
	  $db_version = $this->release_update($module, '3.0', DIR_FS_MODULES . 'ucbooks/updates/R21toR30.php');
	...
	if ($db_version == '3.0') {
	  $db_version = $this->release_update($module, '3.1', DIR_FS_MODULES . 'ucbooks/updates/R30toR31.php');
	  if (!$db_version) return true;
	}
	if ($db_version == '3.1') {
	  if (!file_exists($path . $dir)) mkdir(DIR_FS_MY_FILES . $_SESSION['company'] . '/ucbooks/orders/', 0755, true);
	  ...
	  if (is_array(glob(DIR_FS_ADMIN.'UCBOOKS_DIR_MY_ORDERS*.zip'))) {
	  	foreach (glob(DIR_FS_ADMIN.'UCBOOKS_DIR_MY_ORDERS*.zip') as $file) {
	  	  $newfile = str_replace('UCBOOKS_DIR_MY_ORDERS', '', $file);
	  	  $newfile = str_replace(DIR_FS_ADMIN, '', $newfile);
	  	  rename($file,UCBOOKS_DIR_MY_ORDERS.$newfile);
	    }
	  }
	  $db_version = '3.3';
	}
  ...
  function remove($module) {
    ...
    if (db_field_exists(TABLE_CURRENT_STATUS, 'next_do_num'))       $db->Execute("ALTER TABLE ".TABLE_CURRENT_STATUS." DROP next_do_num");
    if (db_field_exists(TABLE_CURRENT_STATUS, 'next_inv_num'))      $db->Execute("ALTER TABLE ".TABLE_CURRENT_STATUS." DROP next_inv_num");
    if (db_field_exists(TABLE_CURRENT_STATUS, 'next_pur_num'))      $db->Execute("ALTER TABLE ".TABLE_CURRENT_STATUS." DROP next_pur_num");
    ...
    if (db_field_exists(TABLE_CURRENT_STATUS, 'next_dm_num'))       $db->Execute("ALTER TABLE ".TABLE_CURRENT_STATUS." DROP next_dm_num");
    if (db_field_exists(TABLE_CURRENT_STATUS, 'next_vcm_num'))      $db->Execute("ALTER TABLE ".TABLE_CURRENT_STATUS." DROP next_vcm_num");
    if (db_field_exists(TABLE_CURRENT_STATUS, 'next_vdm_num'))      $db->Execute("ALTER TABLE ".TABLE_CURRENT_STATUS." DROP next_vdm_num");
