...
class statement_builder {
    ...
    function load_query_results($tableKey = 'id', $tableValue = 0, $open_inv = false) {
...
//  $this->terms_lang    = $result['terms_lang'];
    ...
    function load_query_results1($tableKey = 'id', $tableValue = 0, $open_inv = false) {
        global $db, $FieldListings, $report;
        //if (!$tableValue)
        //   return false;
        $output = array();
...
//  $this->terms_lang    = $result['terms_lang'];
	  ...
	  where m.bill_acct_id = " . $this->bill_acct_id;
        ...
        $sql .= " and m.journal_id in (6, 7, 12, 13) and i.gl_type in ('ttl', 'dsc') order by m.post_date";
        /*
         * 6= purchase,7 = vendor credit memo , 12 = sales invoice , 13 = customer credit memo, 18 = sales receipt, 20 = purchase receipt
         * This sql is to get all journal information according to that customer open or closed ( condition applied)
         */
        ...
        while (!$result->EOF) {
            $r = 0;
            /*
             * This query is to find out any reference(so_po_item_ref_id) of that selected journal entry
             * 
             */
...
//                        }
                    ...
                    if (!in_array($final_result->fields['purchase_invoice_id'], $stack_array)) {
                        ...
                        $this->line_items[] = array(
                            // 'journal_desc' => $ref_only ? 'Order Discount (Ref Only)' : $line_desc,
                            'purchase_invoice_id' => $final_result->fields['purchase_invoice_id'],
                            'purch_order_id' => $final_result->fields['purch_order_id'],
                            'post_date' => $final_result->fields['post_date'],
                            // 'due_date' => $terms_date['net_date'],
                            'credit_amount' => !empty($creditt) ? $creditt : ' ',
                            'debit_amount' => !empty($debitt) ? $debitt : ' ',
                        );
                        if (!$ref_only) {
                            $this->statememt_total += $creditt - $debitt;
                        }
                    }
