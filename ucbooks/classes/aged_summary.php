...
// +-----------------------------------------------------------------+
//  Path: /modules/ucform/custom/classes/aged_summary.php
//
            ...
            if (isset($GrpField)) { // we're checking for group totals, see if this group is complete
                if (($myrow[$GrpField] <> $GrpWorking) && $GrpWorking !== false) { // it's a new group so print totals
                    ...
                    foreach ($Seq as $offset => $TotalCtl) {
                        if ($TotalCtl['total'] == 1) {
                            $OutputArray[$RowCnt][$offset + 1] = !empty($TotalCtl['grptotal']) ? ProcessData($TotalCtl['grptotal'], $TotalCtl['processing']) : 0;
                        } else {
...
//echo 'new myrow = '; print_r($myrow); echo '<br /><br />';
        ...
        if ($GrpWorking !== false) { // if we collected group data show the final group total
            ...
            foreach ($Seq as $TotalCtl) {
                if ($TotalCtl['total'] == 1) {
                    $OutputArray[$RowCnt][$ColCnt] = !empty($TotalCtl['grptotal']) ? ProcessData($TotalCtl['grptotal'], $TotalCtl['processing']) : 0;
                } else {
                    $OutputArray[$RowCnt][$ColCnt] = ProcessData($TotalCtl['grptotal'], $TotalCtl['processing']);
                }
                $ColCnt++;
            }
            $RowCnt++;
        ...
        if ($ShowTotals) {
            ...
            foreach ($Seq as $TotalCtl) {
                if ($TotalCtl['total'] == 1) {
                    $OutputArray[$RowCnt][$ColCnt] = !empty($TotalCtl['rpttotal']) ? ProcessData($TotalCtl['rpttotal'], $TotalCtl['processing']) : 0;
                } else {
...
//	$output[] = array('id' => 'contacts.bill_website',            'text' => RW_AR_BILL_WEBSITE);
    ...
    function calulate_special_fields($acct_id) {
        $new_data['balance_0'] = !empty($this->accounts[$acct_id]['balance_0']) ? $this->accounts[$acct_id]['balance_0'] : 0;
        $new_data['balance_30'] = !empty($this->accounts[$acct_id]['balance_30']) ? $this->accounts[$acct_id]['balance_30'] : 0;
        $new_data['balance_60'] = !empty($this->accounts[$acct_id]['balance_60']) ? $this->accounts[$acct_id]['balance_60'] : 0;
        $new_data['balance_90'] = !empty($this->accounts[$acct_id]['balance_90']) ? $this->accounts[$acct_id]['balance_90'] : 0;
        return $new_data;
    }
