...
// +-----------------------------------------------------------------+
//  Path: /modules/Ucbooks/classes/balance_sheet.php
//

require_once(DIR_FS_MODULES . 'ucbooks/functions/ucbooks.php');
require_once(DIR_FS_MODULES . 'ucbooks/classes/income_statement.php');
...
class balance_sheet {
	...
	function add_bal_sheet_data($the_list, $negate_array, $period) {
		...
		foreach($the_list as $key => $account_type) {
			$sql = "select h.beginning_balance + h.debit_amount - h.credit_amount as balance, c.description,c.id  
			...
			while (!$result->EOF) {
				...
				$this->bal_sheet_data[] = array('d', $result->fields['description'], $temp, '', '',$result->fields['id']);
				$result->MoveNext();
			}
