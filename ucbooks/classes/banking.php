...
// +-----------------------------------------------------------------+
//  Path: /modules/ucbooks/classes/banking.php
//
    ...
    function __construct() {
        ...
        switch ($this->journal_id) {
            case 18:
                ...
                //$result = $db->Execute("select next_deposit_num from " . TABLE_CURRENT_STATUS);
                //$this->purchase_invoice_id = $result->fields['next_deposit_num'];
                break;
            case 20:
                ...
                break;
            default: die('bad journal ID in ucbooks/classes/banking.php!');
        }
    }
    ...
    function post_ordr($action) {
        ...
        switch ($this->journal_id) {
            case 18: // Cash Receipts Journal
                ...
                if ($this->skip == false) {//codeing by shsheen for skip load class if this is cash type account
...
//				if ($processor->pre_confirmation_check()) return false;
    ...
    function bulk_pay() {
        ...
        // ************* POST journal entry *************
        if (!$this->Post('insert'))
            return false; // all bulk pay are new posts, cannot edit
...
// ************* post-POST processing *************
        ...
        if (!$this->increment_purchase_invoice_id($force))
            return false;
        return true;
    }
            ...
            switch ($this->journal_id) {
                case '18':
                    if ($this->skip == false)//codeing by shsheen for skip load class if this is cash type account
                        ...
                        $desc = GEN_ADM_TOOLS_J18 . '-' . TEXT_TOTAL . ':' . $_POST[$_POST['shipper_code'] . '_field_0']; // change by shsheen for add cash account without systemitically
                    break;
                case '20':
                default:
