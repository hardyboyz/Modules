...
// +-----------------------------------------------------------------+
//  Path: /modules/ucbooks/classes/gen_ledger.php
//
    ...
    function journal($id = 0) {
        ...
        if ($id) {
            ...
            if ($result->RecordCount() == 0)
                return $this->fail_message(GL_ERROR_DIED_CREATING_RECORD . $id);
...
// START Post Journal Function
    ...
    function Post($action = 'insert', $skip_balance = false) {
        ...
        if ($action == 'edit') { // unPost the original entry and remove from db
            ...
            if (sizeof($old_gl_entry->repost_ids) > 0) { // rePost any journal entries unPosted to rollback COGS calculation (if edit)
                ...
                while (true) {
                    ...
                    if (!$id)
                        break; // no more to unPost, exit loop
                    if (in_array($id, $this->repost_ids))
                        continue; // already has been unposted, skip
        ...
        for ($i = 0; $i < count($this->journal_rows); $i++) {
            ...
            // if (empty($this->journal_rows[$i]['id']))
            db_perform(TABLE_JOURNAL_ITEM, $this->journal_rows[$i], 'insert');
            //else
            //  db_perform(TABLE_JOURNAL_ITEM, $this->journal_rows[$i], 'update', "id = " . (int) $this->journal_rows[$i]['id']);
        ...
        if (!$this->Post_chart_balances()) {
            return false; // post the chart of account values
        }
    ...
    function unPost($action = 'delete', $skip_balance = false) {
        ...
        if (!$this->check_for_re_post())
            return false; // check for dependent records that will need to be re-posted
        if (!$this->unPost_account_sales_purchases())
            return false; // unPost the customer/vendor history
...
// unPost_chart_balances needs to be unPosted before inventory because inventory may remove journal rows (COGS)
        ...
        if (!$this->unPost_chart_balances())
            return false; // unPost the chart of account values
        ...
        if (!$this->check_for_closed_po_so('unPost'))
            return false; // check to re-open predecessor entry
...
// START re-post Functions
    ...
    function check_for_re_post() {
        ...
        switch ($this->journal_id) {
            ...
            case 19: // POS Journal
            case 30: //customer debit memo/ sales debit memo
            case 31: //purchase debit memo/ purchase debit memo//
            case 36: //purchase debit memo/ purchase debit memo//
...
// START Chart of Accout Functions
    ...
    function Post_chart_balances() {
        ...
        switch ($this->journal_id) {
            ...
            case 20: // Cash Distribution Journal
            case 21: // Inventory Direct Purchase Journal
            case 30:  // sales debit memo / customer debit memo
            case 31:  //purchase debit memo  / vendor debit memo
            case 36:  //purchase debit memo  / vendor debit memo
                $accounts = array();

                if (sizeof($this->journal_rows) > 0)
                    foreach ($this->journal_rows as $value) {
			  ...
			  where account_id = '" . $gl_acct . "' and period = " . $this->period;
                            ...
                            $result = $db->Execute($sql);
                            if ($result->AffectedRows() <> 1) {
                                return $this->fail_message(GL_ERROR_POSTING_CHART_BALANCES . ($gl_acct ? $gl_acct : TEXT_NOT_SPECIFIED));
                            }
                        }
                    }
...
// *********  chart of account support functions  **********
    function update_chart_history_periods($period = CURRENT_ACCOUNTING_PERIOD) {
        ...
        $result = $db->Execute($sql);
        if ($result->EOF)
            return $this->fail_message(GL_ERROR_BAD_ACCT_PERIOD);
        ...
        if ($result->RecordCount() > 0) { // close balances for end of this fiscal year and roll post into next fiscal year
            ...
            if ($result->RecordCount() <> 1)
                $this->fail_message(GL_ERROR_NO_RETAINED_EARNINGS_ACCOUNT);
		...
		where account_id = '" . $retained_earnings_acct . "' and period = " . ($max_period + 1);
            ...
            if (!$this->update_chart_history_periods($max_period + 1))
                return false;
        }
...
// Post the customers/vendors sales/purchases values for the given period
        ...
        switch ($this->journal_id) {
            ...
            case 32:
                if (!$this->bill_acct_id)
                    return $this->fail_message(GL_ERROR_NO_GL_ACCT_NUMBER . 'post_account_sales_purchases.');
                ...
                if ($result->AffectedRows() <> 1)
                    return $this->fail_message(GL_ERROR_UPDATING_ACCOUNT_HISTORY);
    ...
    function unPost_account_sales_purchases() {
        ...
        switch ($this->journal_id) {
            ...
            case 36:
                if (!$this->bill_acct_id)
                    return $this->fail_message(GL_ERROR_NO_GL_ACCT_NUMBER . 'unPost_account_sales_purchases.');
                $result = $db->Execute("delete from " . TABLE_ACCOUNTS_HISTORY . " where ref_id = " . $this->id);
                if ($result->AffectedRows() <> 1)
                    return $this->fail_message(GL_ERROR_DELETING_ACCOUNT_HISTORY);
...
// START Inventory Functions
    ...
    function Post_inventory() {
        ...
        switch ($this->journal_id) { // Pre-posting particulars that are journal dependent
            ...
            case 7:
            case 13:
            case 30:        // here case 30 and case 31 is added by 4axiz with some confusion in which condition would be perfect for sales debit memo and purcahse debit memo
            case 31:        // here case 30 and case 31 is added by 4axiz with some confusion in which condition would be perfect for sales debit memo and purcahse debit memo
            case 14:
            case 16:
            case 21:
                $str_field = 'quantity_on_hand';
        ...
        for ($i = 0; $i < $item_rows_to_process; $i++) {
            if ($this->journal_rows[$i]['sku']) {
                if ($this->journal_rows[$i]['debit_amount'])
                    $price = $this->journal_rows[$i]['debit_amount'] / $this->journal_rows[$i]['qty'];
                if ($this->journal_rows[$i]['credit_amount'])
                    $price = $this->journal_rows[$i]['credit_amount'] / $this->journal_rows[$i]['qty'];
                $inv_list = array(
                ...
                switch ($this->journal_id) {
                    ...
                    case 10:
                        ...
                        if ($this->closed)
                            $adjustment = $this->journal_rows[$i]['qty'];
                        $item_cost = ($this->journal_id == 4) ? $inv_list['price'] : 0;
                        $full_price = ($this->journal_id == 10) ? $inv_list['price'] : 0;
                        if (!$this->update_inventory_status($inv_list['sku'], $str_field, -$adjustment, $item_cost, $inv_list['description'], $full_price))
                            return false;
                        break;
                    case 32:
                    ...
                    case 6:
                    case 21:
                        // not sure enough, probably this condition should exist here.  31-> purchase debit memo    
                        if (isset($this->gl_do_type) && $this->gl_do_type == 'doo') { // This is for skipping sku remainning deduction from system when transaction refers from delivery order.
                            break;
                        }
                        if (!$this->calculate_COGS($inv_list))
                            return false;
                        if ($this->journal_id == 12 && $this->gl_do_type == 'doo') {
                            break;
                        }
                        //if ($get_gl_type)
                        if ($inv_list['so_po_item_ref_id']) { // check for reference to po/so to adjust qty on order/sales order
                            ...
                            if (!$this->update_inventory_status($inv_list['sku'], $so_po_str_field, $adjustment))
                                return false;
                        }
                    ...
                    case 30:    // not sure enough, probably this condition should exist here.  30-> sales credit memo  
                        $inv_list['qty'] = -$inv_list['qty'];
                    case 13: // a customer credit memo, qty stays positive
                    case 16:
                    case 31:    // not sure enough, probably this condition should exist here.  30-> purchase credit memo  
                        if (!$this->calculate_COGS($inv_list))
                            return false;
                        break;
                    default: // nothing
        ...
        for ($i = 0; $i < count($this->journal_rows); $i++) {
            ...
            switch ($this->journal_id) {
                case 4:
                    if (ENABLE_AUTO_ITEM_COST == 'PO' && $this->journal_rows[$i]['qty'])
                        $item_cost = $this->journal_rows[$i]['debit_amount'] / $this->journal_rows[$i]['qty'];
                    break;
                case 6:
                case 21:
                case 31:
                    if (ENABLE_AUTO_ITEM_COST == 'PR' && $this->journal_rows[$i]['qty'])
                        $item_cost = $this->journal_rows[$i]['debit_amount'] / $this->journal_rows[$i]['qty'];
                    break;
                case 12:
                ...
                case 30:
                case 32:
                    if ($this->journal_rows[$i]['qty'])
                        $full_price = $this->journal_rows[$i]['credit_amount'] / $this->journal_rows[$i]['qty'];
                case 32:
                case 7:
                case 19:
            ...
            if ($this->journal_id == 12 && $this->gl_do_type == 'doo') {
                ...
                if (!$this->update_inventory_status($this->journal_rows[$i]['sku'], $str_field, $post_qty, $item_cost, $this->journal_rows[$i]['description'], $full_price))
                    return false;
            }
        }
    ...
    function unPost_inventory() {
        ...
        switch ($this->journal_id) {  // journals that don't affect inventory, return now
            ...
            case 36:
                ...
                if (!$this->rollback_COGS($this->journal_rows[$i]['serialize_number']))
                    return false;
                break;
        ...
        for ($i = 0; $i < count($this->journal_rows); $i++)
                ...
                switch ($this->journal_id) {
                    ...
                    case 10:
                        ...
                        if (!$this->closed && $bal_before_post > 0) {
                            if (!$this->update_inventory_status($this->journal_rows[$i]['sku'], $db_field, -$bal_before_post))
                                return false;
                        }
                        break;
                    case 6:
			...
			where ref_id = " . $this->id . " and sku = '" . $this->journal_rows[$i]['sku'] . "'";
                        ...
                        switch ($this->journal_id) {
                            ...
                            case 36: // customer sales - negate quantity
                            case 19: // customer POS - negate quantity
                            case 30: //customer debit memo - negate qty   
                            case 32: //customer debit memo - negate qty   
                                $qty = -$this->journal_rows[$i]['qty'];
                                break;
                            default:
                                $qty = $this->journal_rows[$i]['qty'];
...
//                            break;
//                        }
                        if (!empty($this->purch_order_id)) { // This is for skipping sku remainning deduction from system when transaction refers from delivery order.
                            $sql = "select journal_id from " . TABLE_JOURNAL_MAIN . " where purchase_invoice_id = '" . $this->purch_order_id . "'";
                            $result = $db->Execute($sql);
                            if ($result->fields['journal_id'] == 32)
                                break;
                        }
                        if (!$this->update_inventory_status($this->journal_rows[$i]['sku'], 'quantity_on_hand', -$qty))
                            return false;
                        ...
                        if ($this->journal_rows[$i]['so_po_item_ref_id']) {
                            ...
                            // do not allow qty on order to go below zero.
                            $adjustment = min($this->journal_rows[$i]['qty'], $bal_before_post);
                            if (!$this->update_inventory_status($this->journal_rows[$i]['sku'], $db_field, $adjustment))
                                return false;
                        }
                        break;
                    default:
...
// *********  inventory support functions  **********
    ...
    function calculate_COGS($item, $return_cogs = false) {
	  ...
	  from " . TABLE_INVENTORY . " where sku = '" . $item['sku'] . "'";
        ...
        if (ENABLE_MULTI_BRANCH)
            $defaults['quantity_on_hand'] = $this->branch_qty_on_hand($item['sku'], $defaults['quantity_on_hand']);
        // catch sku's that are serialized and the quantity is not one, error
        if ($defaults['serialize'] && abs($item['qty']) <> 1)
            return $this->fail_message(GL_ERROR_SERIALIZE_QUANTITY);
        if ($defaults['serialize'] && !$item['serialize_number'])
            return $this->fail_message(GL_ERROR_SERIALIZE_EMPTY);
        ...
        if ($item['qty'] > 0) { // for positive quantities, inventory received, customer credit memos, unbuild assembly
            ...
            switch ($this->journal_id) {
                ...
                case 36: // for negative sales/invoices and customer credit memos the price needs to be the last unit_cost, 
            ...
            if (ENABLE_MULTI_BRANCH)
                $sql .= " and store_id = " . $this->store_id;
            ...
            while (!$result->EOF) {
                ...
                if ($working_qty <= 0)
                    break; // we are finished listing all records that will be affected by this inv receipt
                $result->MoveNext();
            }
		  ...
		  where sku = '" . $item['sku'] . "' and remaining > 0 and serialize_number = '" . $item['serialize_number'] . "'";
                $result = $db->Execute($sql);
                if ($result->RecordCount() <> 0)
                    return $this->fail_message(GL_ERROR_SERIALIZE_COGS);
            ...
            if ($result->AffectedRows() <> 1)
                return $this->fail_message(GL_ERROR_POSTING_INV_HISTORY);
            ...
            if ($defaults['cost_method'] == 'a')
                $avg_cost = $this->fetch_avg_cost($item['sku']);
		  ...
		  where sku = '" . $item['sku'] . "' and remaining > 0 and serialize_number = '" . $item['serialize_number'] . "'";
            ...
            if ($working_qty > 0) {
                if (!ALLOW_NEGATIVE_INVENTORY)
                    return $this->fail_message(GL_ERROR_POSTING_NEGATIVE_INV);
    ...
    function load_so_po_balance($ref_id, $id = '', $post = true) {
        ...
        $item_array = array();
        if ($this->gl_do_type == 'doo') {
            $ref_id = $this->do_ref_id ? $this->do_ref_id : $ref_id;
        }
        if ($ref_id) {
            switch ($this->journal_id) {
                case 4:
		...
		where ref_id = " . $ref_id . " and gl_type = '" . $gl_type . "'";
            ...
            while (!$result->EOF) {
                ...
                if ($this->gl_do_type == 'doo') {
                    foreach ($this->item_rows as $row) {
                        if ($row['gl_type'] == 'sos') {
                            $so_po_arr = explode(',', $row['so_po_item_ref_id']);
                            foreach ($so_po_arr as $so_po) {
                                $check_qty = false;
                                $check_pstd = false;
                                $sql = "select id, sku, qty from " . TABLE_JOURNAL_ITEM . " 
		where id = " . $so_po . " and gl_type = '" . $gl_type . "'";
                                $result1 = $db->Execute($sql);
                                $item_array[$so_po]['ordered'] = $result1->fields['qty'];
                                $item_array[$so_po]['processed'] = $row['pstd'];
		...
		where m.so_po_ref_id = " . $ref_id . " and i.gl_type in( '" . $proc_type . "','".$orType."')";
            if (!$post && $id)
                $sql .= " and m.id <> " . $id; // unposting so don't include current id (journal_id = 6 or 12)
            $result = $db->Execute($sql);
            while (!$result->EOF) {
                if ($result->fields['sku'])
    ...
    function remove_journal_COGS_entries() {
        ...
        for ($i = 0; $i < count($temp_array); $i++) {
            if ($temp_array[$i]['gl_type'] == 'cog')
                continue; // skip row - they are re-calculated later
            if ($temp_array[$i]['gl_type'] == 'asi')
                continue; // skip row - they are re-calculated later
    ...
    function calculate_assembly_list($inv_list) {
        ...
        if ($result->RecordCount() == 0)
            return $this->fail_message(GL_ERROR_BAD_SKU_ENTERED);
	  ...
	  where a.ref_id = " . $sku_id;
        $result = $db->Execute($sql);
        if ($result->RecordCount() == 0)
            return $this->fail_message(GL_ERROR_SKU_NOT_ASSY . $sku);
        ...
        while (!$result->EOF) {
            ...
            if (strpos(COG_ITEM_TYPES, $result->fields['inventory_type']) === false) {
                ...
                if ($qty > 0)
                    $result->fields['price'] = 0; // remove unit_price for builds, leave for unbuilds (to calc delta COGS)
                $item_cost = $this->calculate_COGS($result->fields, true);
            }
            if ($item_cost === false)
                return false; // error in cogs calculation
            $assy_cost += $item_cost;
        ...
        if ($this->calculate_COGS($inv_list, $return_cogs = ($qty < 0) ? false : true) === false)
            return false;
        return true;
    }
...
// START General Functions
    ...
    function build_journal_main_array() { // maps/prepares the fields to the journal_main fields
        ...
        if (isset($this->id))
            if ($this->id)
                $main_record['id'] = $this->id; // retain id if known for re-post references
                if (isset($this->period))
            $main_record['period'] = $this->period;
        if (isset($this->journal_id))
            $main_record['journal_id'] = $this->journal_id;
        if (isset($this->sub_journal_id))
            $main_record['sub_journal_id'] = $this->sub_journal_id;
        if (isset($this->post_date))
            $main_record['post_date'] = $this->post_date;
        if (isset($this->store_id))
            $main_record['store_id'] = $this->store_id;
        ...
        if (isset($this->closed))
            $main_record['closed'] = $this->closed;
        if (isset($this->closed_date))
            $main_record['closed_date'] = $this->closed_date;
        if (isset($this->freight))
            $main_record['freight'] = $this->freight;
        if (isset($this->discount))
            $main_record['discount'] = $this->discount;
        if (isset($this->shipper_code))
            $main_record['shipper_code'] = $this->shipper_code;
        if (isset($this->terms))
            $main_record['terms'] = $this->terms;
        if (isset($this->sales_tax))
            $main_record['sales_tax'] = $this->sales_tax;
        if (isset($this->total_amount))
            $main_record['total_amount'] = $this->total_amount;
        if (isset($this->currencies_code))
            $main_record['currencies_code'] = $this->currencies_code;
        if (isset($this->currencies_value))
            $main_record['currencies_value'] = $this->currencies_value;
        if (isset($this->so_po_ref_id))
            $main_record['so_po_ref_id'] = $this->so_po_ref_id;
        if (isset($this->purchase_invoice_id))
            $main_record['purchase_invoice_id'] = $this->purchase_invoice_id;
        if (isset($this->purch_order_id))
            $main_record['purch_order_id'] = $this->purch_order_id;
        if (isset($this->admin_id))
            $main_record['admin_id'] = $this->admin_id;
        if (isset($this->rep_id))
            $main_record['rep_id'] = $this->rep_id;
        if (isset($this->waiting))
            $main_record['waiting'] = $this->waiting;
        if (isset($this->gl_acct_id))
            $main_record['gl_acct_id'] = $this->gl_acct_id;
        if (isset($this->bill_acct_id))
            $main_record['bill_acct_id'] = $this->bill_acct_id;
        if (isset($this->bill_address_id))
            $main_record['bill_address_id'] = $this->bill_address_id;
        if (isset($this->bill_primary_name))
            $main_record['bill_primary_name'] = $this->bill_primary_name;
        if (isset($this->bill_contact))
            $main_record['bill_contact'] = $this->bill_contact;
        if (isset($this->bill_address1))
            $main_record['bill_address1'] = $this->bill_address1;
        if (isset($this->bill_address2))
            $main_record['bill_address2'] = $this->bill_address2;
        if (isset($this->bill_city_town))
            $main_record['bill_city_town'] = $this->bill_city_town;
        if (isset($this->bill_state_province))
            $main_record['bill_state_province'] = $this->bill_state_province;
        if (isset($this->bill_postal_code))
            $main_record['bill_postal_code'] = $this->bill_postal_code;
        if (isset($this->bill_country_code))
            $main_record['bill_country_code'] = $this->bill_country_code;
        if (isset($this->bill_telephone1))
            $main_record['bill_telephone1'] = $this->bill_telephone1;
        if (isset($this->bill_email))
            $main_record['bill_email'] = $this->bill_email;
        if (isset($this->ship_acct_id))
            $main_record['ship_acct_id'] = $this->ship_acct_id;
        if (isset($this->ship_address_id))
            $main_record['ship_address_id'] = $this->ship_address_id;
        if (isset($this->ship_primary_name))
            $main_record['ship_primary_name'] = $this->ship_primary_name;
        if (isset($this->ship_contact))
            $main_record['ship_contact'] = $this->ship_contact;
        if (isset($this->ship_address1))
            $main_record['ship_address1'] = $this->ship_address1;
        if (isset($this->ship_address2))
            $main_record['ship_address2'] = $this->ship_address2;
        if (isset($this->ship_city_town))
            $main_record['ship_city_town'] = $this->ship_city_town;
        if (isset($this->ship_state_province))
            $main_record['ship_state_province'] = $this->ship_state_province;
        if (isset($this->ship_postal_code))
            $main_record['ship_postal_code'] = $this->ship_postal_code;
        if (isset($this->ship_country_code))
            $main_record['ship_country_code'] = $this->ship_country_code;
        if (isset($this->ship_telephone1))
            $main_record['ship_telephone1'] = $this->ship_telephone1;
        if (isset($this->ship_email))
            $main_record['ship_email'] = $this->ship_email;
        if (isset($this->terminal_date))
            $main_record['terminal_date'] = $this->terminal_date;
        if (isset($this->drop_ship))
            $main_record['drop_ship'] = $this->drop_ship;
        if (isset($this->recur_id))
            $main_record['recur_id'] = $this->recur_id;
        if (isset($this->message))
            $main_record['message'] = $this->message;
        if (isset($this->note))
            $main_record['note'] = $this->note;
        if (isset($this->product_desc))
            $main_record['product_desc'] = $this->product_desc;
        if (isset($this->from_recurring))
            $main_record['from_recurring'] = $this->from_recurring;
    ...
    function remove_cogs_rows() {
        ...
        foreach ($this->journal_rows as $key => $value) {
            if (!in_array($value['gl_type'], $removal_gl_types))
                $temp_rows[] = $value;
        }
            ...
            case 10: if (!$gl_type)
                ...
                if (is_array($this->so_po_balance_array)) {
                    foreach ($this->so_po_balance_array as $counts) {
                        if ($counts['ordered'] > $counts['processed'])
                            $ordr_diff = true;
                    }
                }
                ...
                for ($i = 0; $i < count($this->journal_rows); $i++) {
                    if ($this->journal_rows[$i]['qty'] && $this->journal_rows[$i]['gl_type'] == $gl_type)
                        $item_rows_all_zero = false; // at least one qty is non-zero
                }
                // also close if the 'Close' box was checked
                if (!$ordr_diff || $item_rows_all_zero || $this->closed)
...
//                break;
            ...
            case 21:
            case 32:
                if (!empty($this->so_po_ref_id)) { // make sure there is a reference po/so to check
                    $ordr_diff = false;
                    if (is_array($this->so_po_balance_array)) {
                ...
                } else if ($this->gl_do_type == 'doo' && isset($this->do_ref_id)) {
                    $ordr_diff = false;
                    if (is_array($this->so_po_balance_array)) {
                        foreach ($this->so_po_balance_array as $key => $counts) {
                            if ($counts['ordered'] > $counts['processed'])
                    ...
                    if ($ordr_diff) { // open it, there are still items to be processed
                        $this->close_so_po($this->do_ref_id, false);
                    } else { // close the order
                        $this->close_so_po($this->do_ref_id, true);
                    }
                }
            ...
            case 16:
            case 30:
            case 31: //  case 30 and 31 added by 4axiz but not sure whether this is correct or not    
            default:
        }
        return true;
    }
...
//            }
		...
		where purchase_invoice_id = '" . $this->purchase_invoice_id . "' and journal_id = '" . $this->journal_id . "'";
            ...
            $result = $db->Execute($sql);
            if ($result->RecordCount() > 0)
                return $this->fail_message(sprintf(GL_ERROR_2, constant('ORD_HEADING_NUMBER_' . $this->journal_id)));
            ...
            switch ($this->journal_id) { // select the field to fetch the next number
                case 3: $str_field = 'next_ap_quote_num';
                    break;
                case 4: $str_field = 'next_po_num';
                    break;
                case 6:
                    switch ($this->sub_journal_id) {
                        case 0 :
                            $str_field = 'next_pur_num';
                            break;
                        case 1 :
                            $str_field = 'next_expenses_num';
                            break;
                    }
                    break; //adding by 4axiz(shaheen) date:29-dec-2014
                case 7: $str_field = 'next_vcm_num';
                    break;
                case 9: $str_field = 'next_ar_quote_num';
                    break;
                case 10: $str_field = 'next_so_num';
                    break;
                case 32: $str_field = 'next_do_num';
                    break;
                case 12:
                    switch ($this->sub_journal_id) {
                        case '0' :
                        case '1' :
                            $str_field = 'next_inv_num';
                            break;
                        case '2' :
                            $str_field = "next_dm_num";
                            break;
                    }
                    break;
                case 36:
                ...
                case 18: $str_field = 'next_deposit_num';
                    break;
                case 20:
                case 21: $str_field = 'next_check_num';
                    break;
                case 30: $str_field = "next_dm_num";
                    break;
                case 2:
                    switch ($this->sub_journal_id) {
                        case 1 :
                            $str_field = 'bank_transfer';
                            break;
                        case 2: $str_field = 'received_money';
                            break;
                        case 3: $str_field = 'spend_money';
                            break;
                    }
                    break;
                case 33: $str_field = "bank_transfer";
                    break;
                case 34: $str_field = "received_number";
                    break;
                case 35: $str_field = "spend_number";
                    break;
                case 31: $str_field = "next_vdm_num";
            }
            if ($str_field) {
                ...
                if (!$result)
                    return $this->fail_message(sprintf(GL_ERROR_CANNOT_FIND_NEXT_ID, TABLE_CURRENT_STATUS));
//                switch ($this->journal_id) { // allow for duplicates in the following journals
//                    //case 18:
//                     //   $messageStack->debug(" specified ID and dups allowed, returning OK.");
//                     //   return true; // allow for duplicate deposit ticket ID's
...
//                }
                $sql = "select purchase_invoice_id from " . TABLE_JOURNAL_MAIN . " 
		where purchase_invoice_id = '" . $result->fields[$str_field] . "' and journal_id = '" . $this->journal_id . "'";
                if ($this->id)
                    $sql .= " and id <> " . $this->id;
                $result1 = $db->Execute($sql);
    ...
    function increment_purchase_invoice_id($force = false) {
        ...
        if ($this->purchase_invoice_id == '' || $force) { // increment the po/so/invoice number
            ...
            if ($str_field) {
                ...
                if (!$force)
                    $sql .= " where " . $str_field . " = '" . $this->journal_main_array['purchase_invoice_id'] . "'";
                $result = $db->Execute($sql);
                if ($result->AffectedRows() <> 1)
                    return $this->fail_message(sprintf(GL_ERROR_5, constant('ORD_HEADING_NUMBER_' . $this->journal_id)));
            }
        }
    ...
    function add_account($type, $acct_id = 0, $address_id = 0, $allow_overwrite = false) {
        ...
        switch (substr($type, 1, 1)) {
            case 'b':
            case 'm': $add_type = 'bill';
                break;
            case 's': $add_type = 'ship';
                break;
        ...
        if ($add_type == 'bill' || $this->drop_ship) { // update or insert new account record, else skip to add address
            ...
            if (empty($sql_data_array['dept_rep_id'])) {
                $sql_data_array['dept_rep_id'] = 0;
            }
            /* checking duplicate short name in contacts table */
            $duplicate_short_name = @mysql_query('select short_name,id from contacts where short_name = "' . $short_name . '"', $db->link);

            $obj = new queryFactoryResult;
            $obj->cursor = 0;
            $obj->is_cached = true;
            $obj->resource = $duplicate_short_name;
            if ($obj->RecordCount() > 0) {
                $obj->EOF = false;
                $zp_result_array = @mysql_fetch_array($duplicate_short_name);
            }

            if (!isset($zp_result_array['short_name'])) { // if duplicate short name doesn't exist
                ...
                $acct_id = $zp_result_array['id'];
                $sql_data_array['type'] = $acct_type;
                $sql_data_array['short_name'] = $short_name;
                $sql_data_array['gl_type_account'] = DEF_INV_GL_ACCT;
                $sql_data_array['first_date'] = 'now()';
                db_perform(TABLE_CONTACTS, $sql_data_array, 'update', 'short_name = "' . $short_name . '"');
                // $acct_id = AffectedRows();
