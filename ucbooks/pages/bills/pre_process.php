...
// +-----------------------------------------------------------------+
//  Path: /modules/ucbooks/pages/bills/pre_process.php
//
//this is for menu select coding by 4aixz(apu) date:16_04_2013

if ($_GET['page'] == "bills" && $_GET['jID'] == "18") {
    if ($_GET['type'] == "c") {
        $menu_select = 'banking';
...
} else if ($_GET['page'] == "bills" && $_GET['jID'] == "20") {
    if ($_GET['type'] == "v") {
        $menu_select = 'banking';
...
switch (JOURNAL_ID) {
    case 18: // Cash Receipts Journal
        ...
        $dropdown_payment_modules = load_all_methods('payment');
        ...
        foreach ($cash_chart as $cashAccount) {
            $extra_payment_method[] = array('id' => $cashAccount['id'],
                'page' => $cashAccount['text'],
                'fields' =>
                array(
                    'title' => $cashAccount['text'],
                    'field' => html_input_field($cashAccount['id'] . '_field_0', '', 'size="33" maxlength="32"')
                    ));
            $dropdown_payment_modules[$cashAccount['id']] = array(
                'id' => $cashAccount['id'],
                'text' => $cashAccount['text']
            );
        }
        break;
    case 20: // Cash Disbursements Journal
...
switch ($action) {
    ...
    case 'print':
        ...
        if (JOURNAL_ID == 18) {
            ...
            //codeing by shsheen for skip load class if this is cash type account
            $order->skip = true;
            foreach ($payment_modules as $each_module) {
                if (in_array($payment_module, $each_module)) {
                    $order->skip = false;
                    break;
                }
            }
            if ($order->skip == false) {//codeing by shsheen for skip load class if this is cash type account
        ...
        if (!$error && $post_success = $order->post_ordr($action)) { // Post the order class to the db
            if ($action == 'save') {
                ...
                gen_redirect(html_href_link(FILENAME_DEFAULT, 'module=ucbooks&amp;page=bills&amp;oID=' . $order->id . '&amp;jID=' . $_GET['jID'] . '&amp;type=' . $_GET['type'] . '&amp;action=edit', 'SSL'));
