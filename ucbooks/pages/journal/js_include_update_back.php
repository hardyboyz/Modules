    ...
    function check_form() {
        ...
        function updateBalance() {
            ...
            for (var i = 1; i <= ((document.getElementById('item_table').rows.length) / 2); i++) {
                ...
                if (tax != '0') {
                    var tax_index = document.getElementById('tax_'+i).selectedIndex;
            ...
            if (debit_total != credit_total && document.getElementById('auto_complete').checked == true) { // auto fill only for new entries
                if (debit_total > credit_total || debit_total == credit_total){
                    //		credit = debit_total  - credit_total;
                    //		credit_total += credit;
                    var dt = new String(debit_total);
                    document.getElementById('debit_total').value = formatCurrency(dt);
                    taxable_subtotal = debit_total * (tax_rates[tax_index].rate / 100);
                    document.getElementById('tax_total').value = taxable_subtotal;
                }
                if (debit_total < credit_total || debit_total == credit_total){
                    ...
                    taxable_subtotal = credit_total * (tax_rates[tax_index].rate / 100);
                    document.getElementById('tax_total').value = taxable_subtotal;
                }
                //addGLRow(debit?formatCurrency(debit):'', credit?formatCurrency(credit):'', description);
            }
            balance_total = (debit_total - credit_total) + taxable_subtotal;
