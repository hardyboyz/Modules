...
} else if ($_GET['page'] == "status" && $_GET['jID'] == "13" && (!isset($_GET['sub_jID']) || (isset($_GET['sub_jID']) && $_GET['sub_jID'] == 0))) {
    $menu_select = 'customers';
    $sub_menu_select = 'customer_credit_memos_manager';
} else if ($_GET['page'] == "status" && $_GET['jID'] == "19" && (isset($_GET['sub_jID']) && $_GET['sub_jID'] == 1)) {
    $menu_select = 'customers';
    ...
    case 13: // Customer Credit Memo Journal
        switch (SUB_JOURNAL_ID) {
            ...
            case 1 :
                define('POPUP_FORM_TYPE', 'cust:cm');
                $heading_array['closed'] = TEXT_PAID;
                $page_title = ORD_TEXT_13_1_WINDOW_TITLE;
                break;
        }
        break;
