...
// +-----------------------------------------------------------------+
//  Path: /modules/ucbooks/pages/popup_convert/pre_process.php
//
    ...
    case 'save':
        ...
        switch ($order->journal_id) {
            ...
            case 9:
                ...
                } else if ($selection == 'do') { // delivery order
                    define('JOURNAL_ID', 32);
                    define('SUB_JOURNAL_ID', 0);
                    define('GL_TYPE', 'doo');
                    $search_gl_type = 'doo';
                    $purchase_invoice_id = $do_num;
                } else { // sales order
        ...
        if ($order->journal_main_array['message'])
            $order->journal_main_array['message'] = constant('MESSAGE_' . JOURNAL_ID . '_' . SUB_JOURNAL_ID);
        for ($i = 0; $i < sizeof($order->journal_rows); $i++) {
            ...
            if ($order->journal_rows[$i]['gl_type'] == $search_gl_type) {
                ...
                if ($selection == 'do') {
                    if ($order->journal_rows[$i]['gl_type'] == 'soo')
                        $order->journal_rows[$i]['gl_type'] = 'doo';
                }
            }
        }
        // ***************************** START TRANSACTION *******************************
