...
} else if ($_GET['page'] == "orders" && $_GET['jID'] == "13" && (!isset($_GET['sub_jID']) || (isset($_GET['sub_jID']) && $_GET['sub_jID'] == 0))) {
    $menu_select = 'customers';
    $sub_menu_select = 'customer_credit_memos_manager';
} else if ($_GET['page'] == "orders" && (($_GET['jID'] == "13" || $_GET['jID'] == "19") && $_GET['sub_jID'] == '1')) {
    $menu_select = 'customers';
    ...
    case 13:
        switch (SUB_JOURNAL_ID) {
            case 0 :
                ...
                break;
            case 1 :
                $security_token = SECURITY_ID_SALES_CUSTOMER_DEPOSIT;
                break;
        }
        break;
    case 19: $security_token = SECURITY_ID_SALES_CUSTOMER_DEPOSIT;
        break;
    ...
    case 6:  // Purchase Journal (accounts payable - pay later)
        switch (SUB_JOURNAL_ID) {
            ...
            case '1':
                ...
                define('DEF_GL_ACCT_TITLE', ORD_AP_ACCOUNT);
                define('TEXT_COLUMN_1_TITLE', TEXT_PO_BAL);
                define('TEXT_COLUMN_2_TITLE', 'Quantity');
                define('TEXT_ORDER_CLOSED_FIELD', TEXT_INVOICE_PAID);
                $item_col_1_enable = false;
                $item_col_2_enable = true;
                define('POPUP_FORM_TYPE', '');
    ...
    case 13: // Customer Credit Memo Journal (unpaid invoice returned product from customer)
        switch (SUB_JOURNAL_ID) {
            ...
            case '1' :
                define('ORD_ACCT_ID', GEN_CUSTOMER_ID);
                define('GL_TYPE', 'sos');
                define('DEF_INV_GL_ACCT', AR_DEF_DEP_LIAB_ACCT);
                define('DEF_GL_ACCT', $_SESSION['admin_prefs']['def_ar_acct'] ? $_SESSION['admin_prefs']['def_ar_acct'] : AR_SALES_RECEIPTS_ACCOUNT);
                define('DEF_GL_ACCT_TITLE', ORD_AR_ACCOUNT);
                define('TEXT_COLUMN_1_TITLE', TEXT_SHIPPED);
                define('TEXT_COLUMN_2_TITLE', TEXT_RETURNED);
                define('TEXT_ORDER_CLOSED_FIELD', TEXT_CREDIT_PAID);
            ...
            if (JOURNAL_ID == 13) {
                switch (SUB_JOURNAL_ID) {
                    case '0' :
                        ...
                        break;
                    case '1' :
                        $order->description = 'Customer Deposit Entry Discount';
                        break;
                    default:
                        $order->description = 'Customer Credit Memos Entry Discount';
                        break;
...
//	$order->sales_tax_auths     = db_prepare_input($_POST['sales_tax_auths']);
        ...
        $order->message = db_prepare_input($_POST['message']);
       // $order->message = db_prepare_input(constant('MESSAGE_'.JOURNAL_ID.'_'.SUB_JOURNAL_ID));
        $order->product_desc = db_prepare_input($_POST['product_desc']);
        $order->note = db_prepare_input($_POST['note']);
        if (($_GET['jID'] == 7 || $_GET['jID'] == 13 || $_GET['jID'] == 19 || $_GET['jID'] == 30 || $_GET['jID'] == 31 ) && (isset($_POST['is_discount']) && $_POST['is_discount'] == 1)) {//we need to make item qty 0 for credait memo discount we assign distinct value
            $order->is_discount = 1;
        }
// load item row data
        $x = 1;
...
//if ($_POST['price_' . $x] == "") $error = $messageStack->add(GEN_ERRMSG_NO_DATA . "Price", 'error'); //need to fix bugs.
            $order->item_rows[] = array(
                ...
                'discval' => db_prepare_input($_POST['discval_' . $x]),
                'disc_percent' => isset($_POST['disc_percent_' . $x]) ? db_prepare_input(($_POST['discval_' . $x]/100) * $currencies->clean_value(db_prepare_input($_POST['price_' . $x]), $order->currencies_code) / $order->currencies_value) : 0,
    ...
    case 'delete':
        ...
        if ($id) {
            ...
            if ($delOrd->closed == 0) {
                ...
                if ($delOrd->gl_type == 'doo') {
                    $result = $db->Execute("select purchase_invoice_id from " . TABLE_JOURNAL_MAIN . " 
	    where journal_id = '32' and id = '" . $id . "'");
                    if ($result->RecordCount() > 0) {
                        $purchase_invoice_id = $result->fields['purchase_invoice_id'];

                        $result = $db->Execute("select purch_order_id from " . TABLE_JOURNAL_MAIN . " 
	    where journal_id = '12' and purch_order_id = '" . $purchase_invoice_id . "'");
                        if ($result->RecordCount() > 0) {

                            $messageStack->add('You can not delete this entry. This entry is used for sales invoice', 'error');
                ...
                $messageStack->add('You can not delete this entry', 'error');
                gen_redirect(html_href_link(FILENAME_DEFAULT, 'module=ucbooks&page=orders&oID=' . $_GET['oID'] . '&jID=' . $_GET['jID'] . '&amp;sub_jID=' . SUB_JOURNAL_ID . '&action=edit', 'SSL'));
            }
        } else {
...
//if ((isset($_SESSION['set_security']) && $_SESSION['set_security'] == 'yes') && JOURNAL_ID == '10') {
//    $include_template = 'template_main_sec.php'; // include display template (required)
//} else if ((isset($_SESSION['set_security']) && $_SESSION['set_security'] == 'yes') && JOURNAL_ID == '4') {
//    $include_template = 'template_main_sec_update.php'; // include display template (required)
//} else {
//
//    $include_template = 'template_main.php'; // include display template (required)
//}
  $include_template = 'template_main.php'; // include display template (required)
if (JOURNAL_ID == 13 && SUB_JOURNAL_ID == 1) {
    define('PAGE_TITLE', constant('ORD_TEXT_' . JOURNAL_ID . '_' . SUB_JOURNAL_ID . '_WINDOW_TITLE'));
} else if (JOURNAL_ID == 19 && SUB_JOURNAL_ID == 1) {
    define('PAGE_TITLE', constant('ORD_TEXT_13_1_WINDOW_TITLE'));
} else if (JOURNAL_ID == 6 && SUB_JOURNAL_ID == 1) {
    define('PAGE_TITLE', constant('ORD_TEXT_' . JOURNAL_ID . '_' . SUB_JOURNAL_ID . '_WINDOW_TITLE'));
} 
//else if ((isset($_SESSION['set_security']) && $_SESSION['set_security'] == 'yes') && JOURNAL_ID == '10') {
//    define('PAGE_TITLE', 'Job Sheet');
//} else if ((isset($_SESSION['set_security']) && $_SESSION['set_security'] == 'yes') && JOURNAL_ID == '4') {
//    define('PAGE_TITLE', 'Job sheet update screen');
//} 
else {
