...
<script type="text/javascript">
    ...
    var discval              = '<?php echo DISCVAL; ?>';
    var disc_percent         = '<?php echo DISC_PERCENT ?>';
...
if ($action == 'prc_cust')
    echo '  ajaxOrderData(0, ' . $oID . ', ' . JOURNAL_ID . ', true, false,1);' . "\n";
if ($action == 'prc_doo')
    echo '  ajaxOrderData(0, ' . $oID . ', ' . JOURNAL_ID . ', true, false);' . "\n";
if ($action == 'prc_do')
...
?>
    ...
    $('#search').live('keyup',function(e){
        if(e.keyCode == 13){
            ...
            if (e.keyCode != 40 && e.keyCode != 38) { 
                ...
                // alert(keyword)
                var type='';
                if(journalID==12 || journalID==36 || journalID==32 || journalID==10 || journalID==9 || journalID==13|| journalID==19|| journalID==30){
                    type = 'cm';
    ...
    function setReturnAccount(cID) {
        ...
        if((journalID==13 && sub_journalID==1))
            copyAddress();
        if((journalID==19 && sub_journalID==1))
            copyAddress();
        $('#customer_list').hide();
    }
