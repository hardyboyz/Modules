...
?>
    ...
    $(document).ready(function(){
        ...
        $("#search").tokenInput('index.php?module=ucbooks&page=ajax&op=autocopmplete&type='+type+'&journalID='+journalID+'&action=contact', {
            onAdd: function (item) {
                var func =  item.onclick;
                if(journalID==18 || journalID==20){
                    setReturnEntry(item.id)
                }else{
                    setReturnAccount(item.id)
        ...
        if(action!='edit'){
            $("#token-input-search").focus();
        }else{
            //$("#token-input-search").attr('onfocus','javascript: this.select()');
        }
        if(navigator.userAgent.toLowerCase().indexOf('chrome') > -1){
            $(".token-input-dropdown").css('top','357px');
        }else{
            $(".token-input-dropdown").css('top','323px');
        }
        $("#token-input-search").attr('tabindex','1');
       
         
    });
    $("#ship_email").live('blur',function(){
        $('#token-input-sku_1').focus();
        $("html, body").scrollTop($("#productList").offset().top);
    })
    //    $("#hidden_btn").live('blur',function(){
    ...
    $(document).keydown(function(e) {
        if(e.keyCode == 13) {
            //            if($("#hidden_btn").attr('rel')=='hover'){
            //                $("#currency_area").slideToggle("fast");
            //                var class_name = $("#hidden_btn").attr("class"); 
            //                             
            //                if(class_name == "plus" ){
            //                    $("#hidden_btn").children("img").attr("src","images/minus_icon.png");
            //                    $("#hidden_btn").removeClass('plus').addClass('minus');
            //                    $("#hidden_btn").children("img").removeAttr("title").attr("title","Hide Form");
            //                    $('#terminal_date').focus();   
            //                    $('#item_cnt_1').removeAttr('tabindex');
            //                } else if(class_name == "minus"){
            //                    $("#hidden_btn").children("img").attr("src","images/plus_icon.png");
            //                    $("#hidden_btn").removeClass('minus').addClass('plus');
            //                    $("#hidden_btn").children("img").removeAttr("title").attr("title","Show More Form");
            //                }
            //                $("#hidden_btn").css('border','0 none');
            //                $("#hidden_btn").attr('rel','');
            //                $("html, body").scrollTop($("#file_name").offset().top);
            ...
            if($('#tb_icon_save').hasClass('btn_save')){
                ...
                //var id = document.activeElement//.attr('id');//$('#item_table').find('input[id*="desc_:focused"]').attr('id');
                //alert(id)
                $('#'+id).trigger('click');
            }
        }else if(e.keyCode==9){
            if($("#post_date").is(':focus')==true){
                if(e.shiftKey==true){
                    
                    text = $('.token-input-delete-token').siblings('p').html();
                    $('.token-input-delete-token').parent().parent().remove();
                    ...
                    $("#post_date").focusout(function() {
                        //$("#token-input-search").trigger('focus');
                        //$("#token-input-search").focus();
                        $("#token-input-search").val(text);
                        $("#token-input-search").select();
                        $("#token-input-search").attr('tabindex','1');
                        $("[tabindex=1]").focus();
                        var obj = $("#token-input-search");
                        if (obj != null) {
                           // alert('test')
                          //  obj.focus();
                            //if (obj.select)
        ...
        //    $('#post_date').keypress(function(e){
        //        alert(e.keyCode())
        //        if(e.shiftKey==true){
        //            
        //            $("#token-input-search").focus();
        //            if($('#token-input-search').is(':focus')){
        //                alert('test')
        //            }
        //           
        //        }
        //       
        //    })
        //    $('#post_date').live('keypress',function(){
        //        $("#token-input-search").val(text);
        //        $("#token-input-search").attr('onfocus','javascript: this.select()');
        //        $("#token-input-search").attr('tabindex','1');
        //        $("#token-input-search").focus();
