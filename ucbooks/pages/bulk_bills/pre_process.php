...
// +-----------------------------------------------------------------+
//  Path: /modules/ucbooks/pages/bulk_bills/pre_process.php
//
...
$purchase_invoice_id     = db_prepare_input($_POST['purchase_invoice_id']);	// UcBooks starting check number
...
switch ($action) {
  case 'print':
	...
	foreach ($payment_list as $account => $values) {
	  ...
	  $order->purchase_invoice_id = $purchase_invoice_id;	// ucBooks payment number
	  $order->shipper_code        = '';
