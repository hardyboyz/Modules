...
$sub_jID = db_prepare_input($_GET['sub_jID']);
...
if (sizeof($order->fields) > 0) {
    ...
    if ($so_po) { // opening a SO/PO for Invoice/Receive
        ...
        if (JOURNAL_ID == 30 || JOURNAL_ID == 31 || JOURNAL_ID == 7 || JOURNAL_ID == 13 || JOURNAL_ID == 19) {      // if this journal id belongs to credit memo then purchase_invoice_id is ssaving into purch_order_id field
    ...
    if ($so_po_ref_id) { // then there is a purchase order/sales order to load first
        ...
        if ((JOURNAL_ID == 13 || JOURNAL_ID == 19) && SUB_JOURNAL_ID == 1)
        ...
        while (!$ordr_items->EOF) {
		    ...
		    from " . TABLE_INVENTORY . " where sku = '" . $ordr_items->fields['sku'] . "'");
              ...
              if (UOM_QTY == 1) {//this is for get unit price with uom qty
                     if($ordr_items->fields['disc_percent'] > 0){
                      $unit_price = $ordr_items->fields['qty'] ? ($total / $ordr_items->fields['qty']) : '0';
                      $unit_price = $unit_price / ($ordr_items->fields['uom_qty'] ? $ordr_items->fields['uom_qty'] : 1) + $ordr_items->fields['disc_percent'];
                     }else{
                       $unit_price = $ordr_items->fields['qty'] ? ($total / $ordr_items->fields['qty']) : '0';
                        $unit_price = ($unit_price + $ordr_items->fields['discval']) / ($ordr_items->fields['uom_qty'] ? $ordr_items->fields['uom_qty'] : 1);
                     }
                } else {
                  
                  if($ordr_items->fields['disc_percent'] > 0){
                    $unit_price = $ordr_items->fields['qty'] ? ($total / $ordr_items->fields['qty']) + $ordr_items->fields['disc_percent'] : '0';
                  }else{
                    $unit_price = $ordr_items->fields['qty'] ? ($total + $ordr_items->fields['discval']) / $ordr_items->fields['qty'] : '0';
                  }
                ...
                $item_list[] = array(
                    ...
                    'discval' => $ordr_items->fields['discval'],
                    'disc_percent' => $ordr_items->fields['disc_percent'],
            ...
            if ($so_po_ref_id) {
                for ($i = 0; $i < count($item_list); $i++) {
                    ...
                    if ($ordr_items->fields['so_po_item_ref_id'] && $item_list[$i]['so_po_item_ref_id'] == $ordr_items->fields['so_po_item_ref_id']) {
                        ...
                        $item_list[$i]['discval'] = $ordr_items->fields['discval'];
                        $item_list[$i]['disc_percent'] = $ordr_items->fields['disc_percent'];
