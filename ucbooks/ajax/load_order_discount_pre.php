...
if(isset($_GET['goods_return']) && $_GET['goods_return'] == 0){
    $discount = 1;
}

if ($oID) {
    $order = $db->Execute("select * from " . TABLE_JOURNAL_MAIN . " where id = '" . $oID . "'");
    //if($order->fields['journal_id'] == 13 ){
        $pre_total = $db->Execute("SELECT total_amount - sales_tax as pre_total from " . TABLE_JOURNAL_MAIN . " where id = '" . $order->fields['so_po_ref_id'] . "'");
        if($pre_total){
            $order->fields['pre_total'] = $pre_total->fields['pre_total']; 
        }
    //}
...
if (sizeof($order->fields) > 0) {
    ...
    $order->fields['attach_exist'] = file_exists(UCBOOKS_DIR_MY_ORDERS . 'order_' . $oID . '.zip') ? 1 : 0;
    if ($so_po) { // opening a SO/PO for Invoice/Receive
        ...
        if(JOURNAL_ID == 13){      // if this journal id belongs to credit memo then purchase_invoice_id is ssaving into purch_order_id field
            $order->fields['purch_order_id'] = $order->fields['purchase_invoice_id'];
        }
        if(JOURNAL_ID == 30){      // if this journal id belongs to credit memo then purchase_invoice_id is ssaving into purch_order_id field
            $order->fields['purch_order_id'] = $order->fields['purchase_invoice_id'];
        }
    ...
    if ($so_po_ref_id) { // then there is a purchase order/sales order to load first
        ...
        while (!$ordr_items->EOF) {
		    ...
		    from " . TABLE_INVENTORY . " where sku = '" . $ordr_items->fields['sku'] . "'");
                ...
                $item_list[] = array(
                    ...
                    'full_price' => $ordr_items->fields['full_price'],
        ...
        $sql = "select i.qty, i.sku, i.so_po_item_ref_id, i.debit_amount+i.credit_amount as total,m.total_amount-m.sales_tax as pre_total
        ...
        //if($order->fields['journal_id'] == 13 ){ //to track the original item amount in discounted item table
            $order->fields['pre_total'] -= $posted_items->fields['pre_total'];
        //}
        while (!$posted_items->EOF) {
            for ($i = 0; $i < count($item_list); $i++) {
                if ($item_list[$i]['so_po_item_ref_id'] == $posted_items->fields['so_po_item_ref_id']) {
                    //if($order->fields['journal_id'] == 13 ){ //to track the original item amount in discounted item table
                           // $item_list[$i]['total'] -= $posted_items->fields['total'];
                            $item_list[$i]['total'] = $item_list[$i]['total']; // changed by shaheen on 27-04-2014
                      //  }
            ...
            if ($so_po_ref_id) {
                for ($i = 0; $i < count($item_list); $i++) {
                    ...
                    if ($ordr_items->fields['so_po_item_ref_id'] && $item_list[$i]['so_po_item_ref_id'] == $ordr_items->fields['so_po_item_ref_id']) {
                        //if($order->fields['journal_id'] == 13 ){ //to track the original item amount in discounted item table
                            $item_list[$i]['pre_item_total'] = $item_list[$i]['total'];
                        //}
