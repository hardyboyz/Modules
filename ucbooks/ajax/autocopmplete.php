...
if (!empty($keyword)) {
    if ($_GET['action'] == 'item') {
        $query_raw = "select id,sku,inventory_type,description_short from inventory where (sku like '%" . $keyword . "%' or description_short like '%" . $keyword . "%') and inactive = '0' order by sku ASC";
    } else {
        $type = $_GET['type'];
    ...
    while (!$query_result->EOF) {
        ...
        // add new option
        if ($_GET['action'] == 'item') {
            if ($query_result->fields['inventory_type'] == 'ms' && ($_GET['journalID'] == 4 || $_GET['journalID'] == 6))
                $str .= "<li onclick='setReturnMasterItem(" . $query_result->fields['id'] . "," . $_GET['row'] . ")'>" . $query_result->fields['sku'] . '</li>';
            else
