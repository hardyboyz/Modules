...
while ($row = mysql_fetch_object($result)) {
    ...
    //$row->parent_invoice_link = "<a href='".html_href_link(FILENAME_DEFAULT, 'module=ucbooks&amp;page=orders&amp;oID=' . $row->parent_inv_id . '&amp;jID=' . $row->journal_id . '&amp;action=edit', 'SSL')."'>Edit</a>";
    //$row->edit_delete_link = get_edit_link($row->id,$type)."&nbsp;|&nbsp;".get_delete_link($row->id);
    $row->edit_delete_link = "<img  onclick='window.open(\"".html_href_link(FILENAME_DEFAULT, 'module=ucbooks&amp;page=orders&amp;oID=' . $row->parent_inv_id . '&amp;jID=' . $row->journal_id . '&amp;action=edit&rec_id=' . $row->id, 'SSL')."\",\"_self\")' title='Edit'style=' width:20px; height:20; float:left; cursor:pointer;' src='" . DIR_WS_THEMES . "datatables/images/edit.jpg'>&nbsp;&nbsp;&nbsp;<img style='display:block; float: left; margin-left:5px; cursor:pointer;' onclick='if (confirm(\"Are you sure you want to delete this invoice?\")) deleteRecuring(" . $row->id . ",this)' title='Delete' width='20' height='20' src='" . DIR_WS_THEMES . "datatables/images/delete_red.png'>";
    $output['aaData'][] = $row;
}
