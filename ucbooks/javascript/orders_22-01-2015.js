...
function ajaxOrderData(cID, oID, jID, open_order, ship_only, goods_return,recurring_invoice,allDo ) {
    ...
    if(recurring_invoice != undefined){
        var urls = 'index.php?module=ucbooks&page=ajax&op=load_recurring_invoice_order&cID='+cID+'&oID="'+oID+'"&jID='+jID+'&so_po='+open_so_po+'&ship_only='+only_ship+'&rec_id='+recurring_invoice;
        ...
        var urls = 'index.php?module=ucbooks&page=ajax&op=load_order_discount&cID='+cID+'&oID='+oID+'&jID='+jID+'&so_po='+open_so_po+'&ship_only='+only_ship+'&goods_return=0';
        function_choice = fillOrderDataDiscount; //Select return function for Discount
    }else if(allDo!=undefined){
        var urls = 'index.php?module=ucbooks&page=ajax&op=load_deleveryorder&cID='+allDo+'&oID='+oID+'&jID='+jID+'&so_po='+open_so_po+'&ship_only='+only_ship;
    }else{
        var urls = 'index.php?module=ucbooks&page=ajax&op=load_order&cID='+cID+'&oID='+oID+'&jID='+jID+'&so_po='+open_so_po+'&ship_only='+only_ship;
    }
...
function addInvRow() {
    ...
    cell    = '<input style="" type="text" tabindex="'+index+'" name="qty_'+rowCnt+'" id="qty_'+rowCnt+'"'+(item_col_1_enable == '1' ? " " : " readonly=\"readonly\"")+' size="7" maxlength="6" onchange="updateRowTotal('+rowCnt+', true)" style="text-align:right" />';
    newCell = newRow.insertCell(-1);
    newCell.innerHTML = cell;
    newCell.align  = 'center';
    newCell.style.whiteSpace = 'nowrap';
    ...
    cell    = '<input type="text" tabindex="'+index+'" style="width:30px !important" name="pstd_'+rowCnt+'" id="pstd_'+rowCnt+'"'+(item_col_2_enable == '1' ? " " : " readonly=\"readonly\"")+' size="7" maxlength="6" onchange="updateRowTotal('+rowCnt+', true)" style="text-align:right" />';
    switch (journalID) {
        case  '6':
        case  '7':
        case '12':
    ...
    if (single_line_list != '1') {
        ...
        cell  = '<input type="text" tabindex="'+index+'" name="disc_'+rowCnt+'" id="disc_'+rowCnt+'" readonly="readonly" size="11" maxlength="10" style="text-align:right" />';
        newCell = newRow.insertCell(-1);
        newCell.innerHTML = cell;
        newCell.align  = 'center';
        newCell.style.whiteSpace = 'nowrap'; 
    ...
    if (uom == 1) {
        if(uom_list) buildDropDown('uom_'+rowCnt, uom_list, '3');
    }
...
function updateRowTotal(rowCnt, useAjax) {
    ...
    if(uom_qty == 1){
        ...
        if (isNaN(uom_qty_num)) uom_qty_num = 0; // if blank or a non-numeric value is in the qty field, assume zero
        total_line =  qty * uom_qty_num * unit_price;
    }else{
