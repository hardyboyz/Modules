...
// +-----------------------------------------------------------------+
//  Path: /modules/ucbooks/javascript/banking.js
//
...
//  if (!xml) return;
  if ($(xml).find("result").text() == 'success') {
    ...
    window.open('index.php?module=ucbooks&page=popup_bills_accts&list=1&jID='+journalID+"&type="+account_type+"&search_text="+search_text,"invoices","width=1150px,height=550px,resizable=1,scrollbars=1,top=150,left=100");
  }
}

function OpenOrdrList(currObj) {
  window.open('index.php?module=ucbooks&page=popup_bills&list=1&jID='+journalID+"&type="+account_type,"invoices","width=1150px,height=550px,resizable=1,scrollbars=1,top=150,left=100");
}

function popupWindowCvv() {
  window.open('index.php?module=ucbooks&page=popup_cvv',"popup_payment_cvv","width=1150,height=550,resizable=1,scrollbars=1,top=150,left=100");
}
...
function ajaxBillData(cID, bID, jID) {
  ...
  $.ajax({
    type: "GET",
	url: 'index.php?module=ucbooks&page=ajax&op=load_bill&cID='+cID+'&bID='+bID+'&jID='+jID,
    dataType: ($.browser.msie) ? "text" : "xml",
    error: function(XMLHttpRequest, textStatus, errorThrown) {
      alert ("Ajax Error: " + XMLHttpRequest.responseText + "\nTextStatus: " + textStatus + "\nErrorThrown: " + errorThrown);
    },
...
function loadNewBalance() { // request funtion
  ...
  $.ajax({
    type: "GET",
    url: 'index.php?module=ucbooks&page=ajax&op=acct_balance&gl_acct_id='+gl_acct+'&post_date='+post_date,
    dataType: ($.browser.msie) ? "text" : "xml",
    error: function(XMLHttpRequest, textStatus, errorThrown) {
      alert ("Ajax Error: " + XMLHttpRequest.responseText + "\nTextStatus: " + textStatus + "\nErrorThrown: " + errorThrown);
    },
...
function activateFields() {
  if (payments_installed) {
    ...
    for (var i=0; i<document.getElementById('shipper_code').options.length; i++) {
  	  document.getElementById('pm_'+i).style.display = 'none';
    }
    document.getElementById('pm_'+index).style.display = '';
  }
}
