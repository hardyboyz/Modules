...
switch ($action) {
    case 'save':
        ...
        $close_details = array();
        if (is_array($_POST['rcv_sku']))
            foreach ($_POST['rcv_sku'] as $key => $value) {
                $receive_details[] = array(
        ...
        if (!$error) {
            ...
            $id_array = array();
            if (is_array($cInfo->item_rows))
                foreach ($cInfo->item_rows as $value) {
                    $sql_item_array[] = array(
                        'id' => $value['id'],
                        'qty' => $value['qty'],
                        'sku' => $value['sku'],
        ...
        break;
    case 'edit':
        if (!empty($_SESSION['Rowid'])) {
            $id = db_prepare_input($_SESSION['Rowid']);
            $_SESSION['Rowid'] = '';
        } else {
...
switch ($action) {
    ...
    case 'save':
        $_SESSION['Rowid'] = $id;
        gen_redirect(html_href_link(FILENAME_DEFAULT, 'module=rma&page=main&action=edit', 'SSL'));
        break;
    default:
        define('PAGE_TITLE', BOX_RMA_MAINTAIN);
        $include_header = true;
        ...
        $ajax_source_datatable = html_href_link(FILENAME_DEFAULT, 'module=rma&page=ajax&op=load_rma', 'SSL');
