    ...
    case 'save':
        ...
        if ($adj_lines > 0) {
            ...
            if ($glEntry->Post($glEntry->id ? 'edit' : 'insert')) {
                ...
                gen_redirect(html_href_link(FILENAME_DEFAULT, 'module=inventory&page=adjustments&oID='.$Oid.'&jID='.$_GET['jID'].'&type=c&action=edit', 'SSL'));
            }
            // *************** END TRANSACTION *************************
        } else {
...
switch($action){
    case 'new' : 
    case 'edit' : 
        $include_template = 'template_new.php'; break;
    default:$include_template = 'template_main.php'; break;
}

define('PAGE_TITLE', INV_POPUP_ADJ_WINDOW_TITLE);

$ajax_source_datatable = html_href_link(FILENAME_DEFAULT, 'module=inventory&page=ajax&op=load_adjustment_manager&jID=' . $_GET['jID'], 'SSL');
?>