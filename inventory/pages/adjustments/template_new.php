...
if ($_GET['action'] == 'edit') {
    $toolbar->icon_list['print']['params'] = 'onclick="window.open(\'' . html_href_link(FILENAME_DEFAULT, 'module=ucbooks&amp;page=print_invoice&amp;oID=' . $_GET['oID'] . '&amp;jID=' . $_GET['jID'] . '&amp;action=pdf&amp;pdf_type=adjustment', 'SSL') . '\',\'_blank\')"';
    $toolbar->icon_list['print']['icon'] = 'devices/network-wired.png';
    $toolbar->icon_list['print']['text'] = "Generate Adjustment PDF";
}else{
    $toolbar->icon_list['print']['show'] = false;
}

$toolbar->add_icon('new', 'onclick="location.href = \'' . html_href_link(FILENAME_DEFAULT, gen_get_all_get_params(array('action')), 'SSL') . '\'"');
if ($security_level < 4)
