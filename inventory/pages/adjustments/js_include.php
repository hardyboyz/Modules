...
<?php if ($action == 'edit') echo '  EditAdjustment(' . $oID . ')'; ?>
    ...
    function processSkuStock(sXml) {
        ...
        if (!id) { // new line, fill in all fields
            ...
            if(isInt($(xml).find("branch_qty_in_stock").text())==false)
                document.getElementById('stock_'+rCnt).value    = formatCurrency(parseFloat($(xml).find("branch_qty_in_stock").text()));
            else
            ...
            if(isInt($(xml).find("branch_qty_in_stock").text() - qty)==false)
                document.getElementById('stock_'+rCnt).value    = formatCurrency(parseFloat($(xml).find("branch_qty_in_stock").text()) - qty);
            else
    ...
    function updateBalance() {
        ...
        for (var i=1; i<document.getElementById('item_table').rows.length+1; i++) {
            ...
            if(isInt(stock + adj == false))
                document.getElementById('bal_'+i).value = formatCurrency(stock + adj);
            else
