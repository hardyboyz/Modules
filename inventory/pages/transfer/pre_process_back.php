...
define('SUB_JOURNAL_ID', $_GET['sub_jID'] ? $_GET['sub_jID'] : 0); // Adjustment Journal
define('GL_TYPE', '');
$error = false;
        ...
        if (!$error) {
            ...
            if ($adj_lines > 0) {
                ...
                if ($glEntry->Post($glEntry->id ? 'edit' : 'insert')) {
                    ...
                    $Oid = $_GET['oID'] ? $_GET['oID'] : $glEntry->id;
                    gen_redirect(html_href_link(FILENAME_DEFAULT, 'module=inventory&page=transfer&oID='.$Oid.'&jID='.$_GET['jID'].'&sub_jID=' . SUB_JOURNAL_ID . '&type=c&action=edit', 'SSL'));
                   // gen_redirect(html_href_link(FILENAME_DEFAULT, 'module=inventory&page=transfer&jID=' . JOURNAL_ID . '&sub_jID=' . SUB_JOURNAL_ID . '&list=1', 'SSL'));
                } else {
                    $error = true;
                }
            } else {
...
switch ($action) {
    ...
    case 'edit' :
        ...
        break;
    default:$include_template = 'template_main.php';
        break;
}
define('PAGE_TITLE', BOX_INV_TRANSFER);
$ajax_source_datatable = html_href_link(FILENAME_DEFAULT, 'module=inventory&page=ajax&op=load_transfer_manager&jID=' . $_GET['jID'] . '&sub_jID=' . $_GET['sub_jID'], 'SSL');
?>