...
<?php if ($action == 'edit') echo '  EditAssembly(' . $oID . ')'; ?>
    ...
    $("input[id*='sku_']").live('keyup',function(){
     
        var id = $(this).attr('id').split('_');
        //alert($(this).parent('.input_container1').attr('class'))
        ...
        $.ajax({
            ...
            success: function(data){
                ...
                if(data!=''){
                    $('.input_container1 #item_list').show();
                    $('.input_container1 #item_list').html(data);
                }else{
                    $('#item_list').hide();
                }
            }
    ...
    function InventoryList(rowCnt) {
        ...
        var sku = document.getElementById('sku_1').value;
        window.open("index.php?module=inventory&page=popup_inv&list=1&type=v&f1=as&rowID="+rowCnt+"&storeID="+bID+"&search_text="+sku,"inventory","width=1150,height=550,resizable=1,scrollbars=1,top=150,left=100");
    }

    function OpenAssyList() {
        window.open("index.php?module=inventory&page=popup_assy&list=1","inv_assy_open","width=1150,height=550,resizable=1,scrollbars=1,top=150,left=100");
    }
