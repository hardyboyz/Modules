...
// +-----------------------------------------------------------------+
//  Path: /modules/inventory/pages/status_adj/pre_process.php
//
...
$custom_path = DIR_FS_WORKING . 'custom/pages/status_adj/module/extra_actions.php';
...
if (ENABLE_MULTI_BRANCH && $adj_type == 'xfr') {
  $extras = array(TEXT_FROM_BRANCH, TEXT_DEST_BRANCH,TEXT_ACTION);
} elseif (ENABLE_MULTI_BRANCH) {
  $extras = array(TEXT_BRANCH,TEXT_ACTION);
} else {
...
if ($adj_type == 'xfr'){
$page_title = BOX_INV_TRANSFER;
define('POPUP_FORM_TYPE', 'inv:xfr');
}else{
$page_title = ORD_TEXT_16_WINDOW_TITLE;
define('POPUP_FORM_TYPE', 'inv:adj');
}
