...
// +-----------------------------------------------------------------+
//  Path: /modules/uchelp/functions/uchelp.php
//

function synchronize() {
  global $db;
  $config = $db->Execute('TRUNCATE ' . TABLE_UCHELP);
  ...
  foreach ($file_list as $file_name) {
	...
	$sql      = "insert into " . TABLE_UCHELP . " (doc_url, doc_pos, doc_index, doc_title, doc_text)
  ...
  foreach ($toc as $id => $value) {
    if (strrpos($value, '.') === false) {
	  ...
	  if ($key !== false) { // if no parent found, default to root
	    $db->Execute("update " . TABLE_UCHELP . " set parent_id = " . $key . " where id = " . $id);
	    $db->Execute("update " . TABLE_UCHELP . " set doc_type = '0' where id = " . $key); // set parent to type folder
	  }
    }
  }
}
...
function retrieve_toc() {
  global $db;
  $toc = $db->Execute('select id, parent_id, doc_type, doc_url, doc_title from ' . TABLE_UCHELP . ' 
	...
	for ($j = 0; $j < sizeof($full_array[$index]); $j++) {
	  ...
	  for ($i = 0; $i < $level; $i++) {
	    if (false) {
		  ...
		  $entry_string .= html_icon('ucbooks/cont-end.gif', '', 'small');
		} elseif ($i == $level-1 && $j == sizeof($full_array[$index])-1) {
		  $entry_string .= html_icon('ucbooks/end-end.gif',  '', 'small');
		} elseif (in_array($i, $cont_temp)) {
		  $entry_string .= html_icon('ucbooks/cont.gif',     '', 'small');
		} elseif ($i < $level-1) {
		  $entry_string .= html_icon('ucbooks/blank.gif',    '', 'small');
		}
	  }
...
function retrieve_index() {
  global $db;
  $index = $db->Execute('select id, doc_url, doc_index from ' . TABLE_UCHELP . ' order by doc_index');
  $index_array = array();
  while (!$index->EOF) {
...
function search_results($search_text) {
  ...
  $sql = "select id, doc_url, doc_title, MATCH (doc_title, doc_text) AGAINST ('" . $search_text . "') as score 
    from " . TABLE_UCHELP . " where MATCH (doc_title, doc_text) AGAINST ('" . $search_text . "')";
