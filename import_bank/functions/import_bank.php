	...
	foreach ($sql_array as $table => $count) {
	  foreach ($count as $cnt => $table_array) {
	  	...
	  	if(isset($table_array['debit_amount']) && isset($table_array['credit_amount'])){
			...
			$bankimport = new impbanking($table_array['our_account_number'],$table_array['date'],$table_array['account_number_to'],$table_array['credit_amount'],$table_array['debit_amount'],$table_array['description'].' '. $table_array['anouncement'], $bank_gl_acct);
