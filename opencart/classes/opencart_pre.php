...
class opencart {
  ...
  function submitXML($id, $action = '', $hide_success = false, $inc_image = true) {
	global $messageStack;
	switch ($action) {
	  case 'product_ul':   if (!$this->buildProductUploadXML($id, $inc_image)) return false; break;
	  case 'product_sync': if (!$this->buildProductSyncXML())                  return false; break;
	  case 'confirm':      if (!$this->buildConfirmXML())                      return false; break;
	  default: $messageStack->add(OPENCART_INVALID_ACTION, 'error');           return false;
	}
...
//	foreach ($result->fields as $key => $value) $result->fields[$key] = urlencode($result->fields[$key]);
	...
	if (function_exists('opencart_product_upload')) $request = opencart_product_upload($request, $result->fields);
