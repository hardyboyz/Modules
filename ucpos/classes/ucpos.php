...
// +-----------------------------------------------------------------+
//  Path: /modules/ucpos/classes/ucpos.php
//

class ucpos extends journal {
  var $closed = '0';
  ...
  function ucpos() {
	...
	switch ($this->journal_id) {
	  ...
	  case 21:
		...
		break;
	  default: die ('bad journal ID in ucpos/classes/ucpos.php!');
	}
  ...
  function post_ordr($action) {
	...
	$db->transCommit();
	// ***************************** END TRANSACTION *******************************
	$messageStack->add_session('Successfully posted ' . MENU_HEADING_UCPOS . ' Ref # ' . $this->purchase_invoice_id, 'success');
	return true;
  }
    ...
    if ($dc == 'debit' || $dc == 'credit') {
	  ...
	  for ($i = 0; $i < count($this->pmt_rows); $i++) {
		if ($this->pmt_rows[$i]['pmt']) { // make sure the payment line is set and not zero
		  $desc = MENU_HEADING_UCPOS . '-' . TEXT_TOTAL;
	  ...
	  die('bad parameter passed to add_total_journal_row in class ucpos');
    }
  }
	...
	if ($debit_credit == 'debit' || $debit_credit == 'credit') {
	  if ($this->discount <> 0) {
		$this->journal_rows[] = array(
		  ...
		  'description'             => MENU_HEADING_UCPOS . '-' . TEXT_DISCOUNT,
	  ...
	  die('bad parameter passed to add_discount_journal_row in class ucpos.');
	}
  }
  ...
  function add_item_journal_rows($debit_credit) {	// read in line items and add to journal row array
	if ($debit_credit == 'debit' || $debit_credit == 'credit') {
	  ...
	  die('Bad parameter passed to add_item_journal_rows in class ucpos!');
	}
  }
  ...
  function add_tax_journal_rows($debit_credit) {
    ...
    if ($debit_credit == 'debit' || $debit_credit == 'credit') {
	  ...
	  die('bad parameter passed to add_tax_journal_rows in class ucpos.');
    }
  }
