...
// +-----------------------------------------------------------------+
//  Path: /modules/ucpos/pages/deposit/js_include.php
//
?>
...
<?php if ($post_success && $action == 'print') { ?>
            ClearForm();
            var printWin = window.open("index.php?module=ucform&page=popup_gen&gID=<?php echo POPUP_FORM_TYPE; ?>&date=a&xfld=journal_main.id&xcr=EQUAL&xmin=<?php echo $oID; ?>","popup_gen","width=1150px,height=550px,resizable=1,scrollbars=1,top=150px,left=100px");
            printWin.focus();
<?php } ?>
    }
...
?>
    ...
    $('#search').live('keyup',function(e){
        ...
        if(e.keyCode == 13){
            ...
            $.ajax({
                type: "GET",
                url: 'index.php?module=ucbooks&page=ajax&op=autocopmplete&type='+type+'&action=contact&keyword='+keyword,
                dataType: "text",
    ...
    function billsAcctList() {
        window.open("index.php?module=contacts&page=popup_accts&type="+account_type+"&form=bills_deposit&fill=bill&jID="+journalID+"&search_text="+document.getElementById('search').value,"accounts","width=1150px,height=550px,resizable=1,scrollbars=1,top=150,left=100");
    }
