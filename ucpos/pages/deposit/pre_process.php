...
// +-----------------------------------------------------------------+
//  Path: /modules/ucpos/pages/deposit/pre_process.php
//
...
require_once(DIR_FS_MODULES . 'ucbooks/classes/orders.php');
    ...
    case 'print':
        ...
        switch (JOURNAL_ID) {
            case 19:
                ...
                if ($$payment_module->pre_confirmation_check())
                    $error = true;
        ...
        if (!$order->bill_acct_id)
            $error = $messageStack->add(sprintf(ERROR_NO_CONTACT_SELECTED, TEXT_LC_CUSTOMER, TEXT_LC_CUSTOMER, ORD_ADD_UPDATE), 'error');
        if (!$order->item_rows[0]['total'])
