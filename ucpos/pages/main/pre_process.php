...
// +-----------------------------------------------------------------+
//  Path: /modules/ucpos/pages/main/pre_process.php
//
    ...
    case 'print_return':
        ...
        if (UCPOS_REQUIRE_ADDRESS) {
            if (!$order->bill_acct_id && !$order->bill_add_update) {
                $error = $messageStack->add(POS_ERROR_CONTACT_REQUIRED, 'error');
            } else {
                if ($order->bill_primary_name === false)
                    $error = $messageStack->add(GEN_ERRMSG_NO_DATA . GEN_PRIMARY_NAME, 'error');
                if ($order->bill_contact === false)
                    $error = $messageStack->add(GEN_ERRMSG_NO_DATA . GEN_CONTACT, 'error');
                if ($order->bill_address1 === false)
                    $error = $messageStack->add(GEN_ERRMSG_NO_DATA . GEN_ADDRESS1, 'error');
                if ($order->bill_address2 === false)
                    $error = $messageStack->add(GEN_ERRMSG_NO_DATA . GEN_ADDRESS2, 'error');
                if ($order->bill_city_town === false)
                    $error = $messageStack->add(GEN_ERRMSG_NO_DATA . GEN_CITY_TOWN, 'error');
                if ($order->bill_state_province === false)
                    $error = $messageStack->add(GEN_ERRMSG_NO_DATA . GEN_STATE_PROVINCE, 'error');
                if ($order->bill_postal_code === false)
                    $error = $messageStack->add(GEN_ERRMSG_NO_DATA . GEN_POSTAL_CODE, 'error');
            }
        }
        // Item row errors
        if (!$order->item_rows)
        ...
        if ($tot_paid < $order->total_amount) {
            $error = $messageStack->add('The total payment was not greater than or equal to the order!', 'error');
        }
		...
		where doc_group = '" . POPUP_FORM_TYPE . "' and doc_ext = 'frm'");
            if ($result->RecordCount() > 1) {
                $messageStack->add('More than one form was found for this type (' . POPUP_FORM_TYPE . '). Using the first form found.', 'caution');
            }
...
//this change for include header and footer edit by 4axiz(apu)
$include_header = true;
$include_footer = true;
...
switch ($action) {
    case 'pos_return':
        ...
        define('PAGE_TITLE', BOX_UCPOS_RETURN);
        break;
    default:
        $include_template = 'template_main.php';
        define('PAGE_TITLE', BOX_UCPOS);
        break;
}

//payment modules
$payment_modules = load_all_methods('payment');
foreach ($payment_modules as $mod) {
    $pmt_method = $mod['id'];
    $$pmt_method = new $pmt_method;
}
define('PAYMENT_TITLE', UCPOS_PAYMENT_TITLE);
?>