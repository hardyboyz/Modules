...
class misc_reports extends ctl_panel {
  ...
  function Output($params) {
	...
	$control .= '&nbsp;&nbsp;&nbsp;&nbsp;';
	$control .= html_submit_field('sub_misc_reports', TEXT_ADD);
	$control .= html_hidden_field('misc_reports_rId', '');
	$control .= '</div></div>';
	...
	if (is_array($params)) {
	  ...
	  $contents = CP_MISC_REPORTS_NO_RESULTS;
	}
	return $this->build_div(CP_MISC_REPORTS_TITLE, $contents, $control);
  }
  ...
  function Update() {
	...
	$remove_id   = db_prepare_input($_POST['misc_reports_rId']);
