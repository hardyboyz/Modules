...
class inventory_reports extends ctl_panel {
  ...
  function Output($params) {
	...
	$control .= '&nbsp;&nbsp;&nbsp;&nbsp;';
	$control .= html_submit_field('sub_inventory_reports', TEXT_ADD);
	$control .= html_hidden_field('inventory_reports_rId', '');
	$control .= '</div></div>';
	...
	if (is_array($params)) {
	  ...
	  $contents = CP_INVENTORY_REPORTS_NO_RESULTS;
	}
	return $this->build_div(CP_INVENTORY_REPORTS_TITLE, $contents, $control);
  }
  ...
  function Update() {
	...
	$remove_id   = db_prepare_input($_POST['inventory_reports_rId']);
