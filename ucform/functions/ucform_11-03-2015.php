...
//         require_once(DIR_FS_MODULES . 'ucform/classes/customer_statement_generator.php');
//    } else {
    ...
    for ($i = 0; $i < sizeof($report->fieldlist); $i++) {
        if ($report->fieldlist[$i]->type == 'CDta') {
            ...
            foreach ($report->fieldlist[$i]->boxfield as $entry) {
                if ($report->title == 'Customer Statement' && $entry->fieldname == 'COMPANY_COUNTRY') {
...
// find the single line data from the query for the current form page
        ...
        if ($report->special_class) {
            ...
            if ($report->title == 'Customer Statement') {
                ...
                $FieldValues = $special_form->load_query_results1($report->formbreakfield, $Fvalue, $open_inv);
            } else {
...
function MPDFFile($ReportData, $report) {
    require_once(DIR_FS_ADMIN . '/includes/mpdf/mpdf.php');
    require_once(DIR_FS_MODULES . 'ucform/classes/html_generator.php');
    $mpdf = new mPDF();
    $html = new HTML();
    $output = '';
    $output = $html->ReportTable($ReportData, true);
...
//codeing by shaheen for remove extra fields
    ...
    if ($report->page->heading->show) { // Show the company name
        $CSVOutput .= ',,,' . htmlspecialchars(COMPANY_NAME) . chr(10);
    }
    if ($report->page->title1->show) { // Set title 1 heading
        $CSVOutput .= ',,,' . htmlspecialchars(TextReplace($report->page->title1->text)) . chr(10);
    }
    if ($report->page->title2->show) { // Set Title 2 heading
        if ($report->title == 'General Ledger')
            $CSVOutput .= ',,' . htmlspecialchars(TextReplaceGeneralLedger($report->page->title2->text)) . chr(10);
        else
            $CSVOutput .= ',,' . htmlspecialchars(TextReplace($report->page->title2->text)) . chr(10);
    }

    // Set the filter heading
    if ($report->title != 'General Ledger')
        $CSVOutput .= ',,,' . htmlspecialchars(TextReplace($report->page->filter->text)) . chr(10);
    else
        $CSVOutput .= chr(10);
...
//codeing by shaheen for remove extra fields
        ...
        if ($report->title == "General Ledger") {
            if (count($myrow) >= 10) {
                ...
                if ($myrow[1] !=' ') {
                    $balance += str_replace(",", "", substr($myrow[9], 3));
                    $myrow[9] = 'RM '.number_format($balance, 2);
                }
                unset($myrow[1]);
                unset($myrow[10]);
                $myrow = array_values($myrow);
        ...
        switch ($todo[0]) {
            ...
            case "g": // Group Total
                ...
                $CSVOutput .= $Desc . $todo[1].'('.'Net Amount : RM ' .$balance.')';
...
// Now write the total data like any other data row
            ...
            default:
                ...
                if ($report->title == 'General Ledger' && $first_row == 0) {
                    ...
                    $initialBalance = 0;
                    if (!empty($glAccount)) {
                        $initialBalance = getBalance($_POST['date_from'], $_POST['date_to'], $glAccount);
                        $balance += $initialBalance;
                        $balanceRow[8] = 'RM ' . number_format($balance, 2);
                        $myrow[8] = 'RM ' . number_format($balance, 2);
                    }
                    $a = array();
                    $balanceRow = array(
                        ...
                        6 => '',
                        7 => '',
                        8 => $initialBalance ? 'RM ' . number_format($initialBalance, 2) : 'RM 0'
                    );
                    foreach ($balanceRow as $mycolumn) { // check for embedded commas and enclose in quotes
                        $CSVLine .= (strpos($mycolumn, ',') === false) ? ($mycolumn . ',') : ('"' . $mycolumn . '",');
                    }
                    $CSVLine = substr($CSVLine, 0, -1); // Strip the last comma off
                    $CSVOutput .= $CSVLine . chr(10);
...
function getBalance($fromDate, $toDate, $glAccount) {
    ...
    $lastperiod = getLastperiodEndDate();
