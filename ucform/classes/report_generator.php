...
// +-----------------------------------------------------------------+
//  Path: /modules/ucform/classes/report_generator.php
//

if (PDF_APP == 'TCPDF') {
    define('K_PATH_MAIN', DIR_FS_MODULES . 'ucform/includes/tcpdf/');
    define('K_PATH_URL', DIR_WS_MODULES . 'ucform/includes/tcpdf/');
    require_once (DIR_FS_MODULES . 'ucform/includes/tcpdf/tcpdf.php'); // TCPDF class to generate reports, default
} else {

    require_once (DIR_FS_MODULES . 'ucform/includes/fpdf/fpdf.php'); // FPDF class to generate reports
}
...
//	    $this->AliasNbPages(); // deprecated
    ...
    function Header() {
        ...
        //codeing by shaheen for remove extra column 
    ...
    function ReportTable($Data) {
        ...
        foreach ($Data as $myrow) {
            ...
            // Codeing by shaheen for remove extra column from balance sheet
            ...
            switch ($todo[0]) {
                ...
                case "g": // Group Total
                    // Draw a fill box
                    if ($this->y0 + (2 * $this->MaxRowHt) > $this->pageY)
                ...
                default:
                    ...
                    // Draw a fill box
                    if (($this->y0 + $this->MaxRowHt) > $this->pageY)
                        $this->forcePageBreak($CellXPos[0]);
                    $this->SetLeftMargin($CellXPos[0]);
                    $this->SetX($CellXPos[0]);
                    $this->SetY($this->y0);
                    if ($fill)
                        $this->SetFillColor($FillColor[0], $FillColor[1], $FillColor[2]); else
                        $this->SetFillColor(255);
                    $this->Cell(0, $this->pageY - $this->y0, '', $brdr, 0, 'L', 1);
                    // fill in the data
            ...
            if ($ThisRowHt > $this->MaxRowHt)
                $this->MaxRowHt = $ThisRowHt; // keep that largest row so far to track pagination
