...
// +-----------------------------------------------------------------+
//  Path: /modules/ucform/pages/popup_gen/pre_process.php
//
...
//echo 'rID found report->filterlist = '; print_r($report->filterlist); echo '<br>';
    if (isset($_GET['date']))
        $report->datedefault = $_GET['date'] . ':' . $_GET['df'] . ':' . $_GET['dt'];
    if (is_array($report->filterlist))
        foreach ($report->filterlist as $key => $value) { // fill the overrides
            if (isset($_GET['cr' . $key]))
                $value->default = $_GET['cr' . $key];
            if (isset($_GET['min' . $key]))
                $value->min_val = $_GET['min' . $key];
            if (isset($_GET['max' . $key]))
                $value->max_val = $_GET['max' . $key];
            $report->filterlist[$key] = $value;
        }
    if (isset($_GET['xfld'])) { // check for extra filters
        if (isset($_GET['xfld']))
            $report->xfilterlist[0]->fieldname = $_GET['xfld'];
        if (isset($_GET['xcr']))
            $report->xfilterlist[0]->default = $_GET['xcr'];
        if (isset($_GET['xmin']))
            $report->xfilterlist[0]->min_val = $_GET['xmin'];
        if (isset($_GET['xmax']))
            $report->xfilterlist[0]->max_val = $_GET['xmax'];
    }
    $title = $report->title;
} elseif ($gID) {
    $result = $db->Execute("select id, doc_title from " . TABLE_UCFORM . " 
    ...
    if ($result->RecordCount() == 1) {
        ...
        $frm_grp = $db->Execute("select doc_title from " . TABLE_UCFORM . " 
    ...
    $error = true;
    $messageStack->add(UCFORM_NORPT, 'error');
}
...
$open_inv = $_POST['open_invoice'] ? $_POST['open_invoice'] : '';
if (!$error)
    switch ($action) {
        case 'save':
        case 'save_as':
        ...
        case 'exp_html':
            ...
            if ($report->reporttype == 'rpt') {
                ...
                //codeing by shaheen for add link
                $link = array();
                foreach ($report->fieldlist as $fieldlist) {
                    $link[] = $fieldlist->link;
                }
                ...
                if ($_POST['fld_fld'])
                    foreach ($_POST['fld_fld'] as $key => $value) {
                        $report->fieldlist[] = new objectInfo(array(
                                    ...
                                    'link' => db_prepare_input($link[$key]),
                                ));
                    }
                $i = 0;
                while (true) {
                ...
                if (is_array($report->grouplist))
                    foreach ($report->grouplist as $key => $value) {
                        $report->grouplist[$key]->default = ($_POST['defgroup'] == ($key + 1)) ? 1 : 0;
                    }
            }
            if (isset($_POST['datedefault']))
                $report->datedefault = $_POST['datedefault'] . ':' . $_POST['date_from'] . ':' . $_POST['date_to'];
// BOF - added by UcSoft for UcBooks module
            $report->period = $_POST['period'] ? $_POST['period'] : 0;
            if (isset($_POST['period']))
                $report->datedefault = 'z:' . $report->period;
// EOF - added by UcSoft for UcBooks
            ...
            if (is_array($report->filterlist))
                foreach ($report->filterlist as $key => $value) {
                    if (isset($_POST['defcritsel' . $key]))
                        $value->default = $_POST['defcritsel' . $key];
                    if (isset($_POST['fromvalue' . $key]))
                        $value->min_val = $_POST['fromvalue' . $key];
                    if (isset($_POST['tovalue' . $key]))
                        $value->max_val = $_POST['tovalue' . $key];
                    $report->filterlist[$key] = $value;
                }
            if ($action == 'save' && $report->standard_report <> 's') { // Update the main report record
                ...
                fclose($handle);
                $messageStack->add(TEXT_REPORT . $report->description . UCFORM_WASSAVED . $report->title, 'success');
                break; // we're done
            } elseif ($action == 'save') {
                $messageStack->add(UCFORM_CANNOT_EDIT, 'caution');
                break; // we're done
            } elseif ($action == 'save_as') {
                $result = $db->Execute("select * from " . TABLE_UCFORM . " where id = " . $rID);
                $sql_array = array(
            ...
            switch ($report->reporttype) {
                case 'frm':
                    if ($report->title == 'Customer Statement') {
                        if ($action == 'exp_pdf') {
                            $report->special_class = 'statement_builder';
                        }
                    }

                    if (!$ReportData = BuildForm($report, $delivery_method, '', $open_inv)) {
                        //$messageStack->add(UCFORM_NODATA . ' The sql was: ' . $sql, 'caution');
                        $messageStack->add('No data found');
                        $error = true;
                        break;
                    }
                    if (!$ReportData) {
                        $messageStack->add(UCFORM_NODATA . ' The failing sql= ' . $sql, 'caution');
                        $error = true;
                    } else {
                        if ($action == 'exp_csv')
...
//                        if ($success['description'] == 'Report Filters:Grouped by: SKU Total; ') {
//                            $total_array = array();
//                            foreach ($ReportData as $myrow) {
//                                $Action = array_shift($myrow);
//                                $todo = explode(':', $Action);
//                                if ($todo[0] == 'g') {
//                                    $aa = explode('RM ',$myrow[6]);
//                                    array_push($total_array,  preg_replace('/\,/','',$aa[1]));
//                                }
//                            }
//                        }
//                        $a = array(0=>632,1=>4512316541);
//                        arsort($a);
                        ...
                        if (!$ReportData) {
                            ...
                            if ($action == 'exp_csv') {
                                if ($report->title == 'Income Statement Detail') {
                                    $output = isdGenerateCSVFile($ReportData, $report, $delivery_method);
                                } else {
                                    $output = GenerateCSVFile($ReportData, $report, $delivery_method);
                                }
                            }
                            ...
                            if ($action == 'exp_pdf') {
                                if ($report->title == 'Income Statement Detail') {
                                    $company_name = $db->Execute('SELECT configuration_value from configuration where configuration_key = "COMPANY_NAME"');
                                    $output = isdGeneratePDFFile($ReportData, $report, $company_name->fields['configuration_value']);
                                }else if ($report->title == 'General Ledger' || $report->title == 'Sales By Rep' || $report->title == 'Sales By SKU' || $report->title == 'Sales Report') {
                                    
                                    $output = MPDFFile($ReportData, $report);
                                } else {
                                    $output = GeneratePDFFile($ReportData, $report, $delivery_method);
                                }
                            }
...
foreach ($DateChoices as $key => $value) {
    if (strpos($report->datelist, $key) !== false)
...
if ($report->reporttype == 'rpt') {
    ...
    if (is_array($report->grouplist))
        foreach ($report->grouplist as $group) {
            if ($group->default == '1')
                $group_default = $i;
            if ($group->page_break == '1')
                $group_break = true;
    ...
    if (is_array($report->sortlist))
        foreach ($report->sortlist as $sortitem) {
            if ($sortitem->default == '1')
                $sort_default = $i;
...
if ($report->title == 'Income Statement Detail') {
    $cal_from = array(
        ...
        'default' => date('m/Y', strtotime('-1 years')),
...
$include_header_report = true;
$include_tabs = true;
$include_calendar = true;
$include_template = $IncludePage;
define('PAGE_TITLE', UCFORM_REPORT_GEN);
?>