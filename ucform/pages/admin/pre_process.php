...
// +-----------------------------------------------------------------+
//  Path: /modules/ucform/pages/admin/pre_process.php
//
...
switch ($action) {
  ...
  case 'fix':
	// drop the database
	$db->Execute("truncate ".TABLE_UCFORM);
	// load all the install classes to re-build directory structure
  	$install_mod = new ucform_admin;
	$install_mod->load_reports('ucform');
	...
	foreach ($contents as $entry) { // install each module
	  ...
	  if (!in_array($entry, array('.', '..', 'ucform')) && is_dir(DIR_FS_MODULES . $entry)) {
	...
	foreach ($reports as $report) {
	  ...
	  if ($rpt->UcformReport) $rpt = $rpt->UcformReport; // lose the container
	  if ($rpt->security == 'u:-1;g:-1') $rpt->security = 'u:'.$_SESSION['admin_id'].'g:-1'; // orphaned, set so current user can access
	  $result = $db->Execute("select id from ".TABLE_UCFORM." where doc_group = '".$rpt->groupname."' and doc_type = '0'");
	  if ($result->RecordCount() == 0) { // orphaned put into misc category
	  	$orph_cnt++;
	  	$search_type = $rpt->reporttype=='frm'?'misc:misc':'misc'; // put in misc
	    $result = $db->Execute("select id from ".TABLE_UCFORM." where doc_group = '".$search_type."' and doc_type = '0'");
	  }
	  $sql_array = array(
	...
	gen_add_audit_log(UCFORM_TOOLS_REBUILD_TITLE);
	$messageStack->add(sprintf(UCFORM_TOOLS_REBUILD_SUCCESS, $rpt_cnt, $orph_cnt), 'success');
  	break;

/*** BOF - Added by UcSoft to convert UcBooks reports to ucform format *************/
  // This script transfers stored reports from the reportwriter database used in UcBooks to ucform
  case 'convert':
    ...
    break;
/*** EOF - Added by UcSoft to convert UcBooks reports to ucform format *************/
  default:
}
...
define('PAGE_TITLE', BOX_UCFORM_MODULE_ADM);
