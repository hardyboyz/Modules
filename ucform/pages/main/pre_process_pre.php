...
// +-----------------------------------------------------------------+
//  Path: /modules/ucform/pages/main/pre_process.php
//
...
require(DIR_FS_WORKING . 'functions/ucform.php');
...
switch ($action) {
    ...
    case 'export':
        ...
        if (!class_exists('ZipArchive')) {
            $messageStack->add(UCFORM_NO_ZIP, 'error');
            break;
        }
        $zip = new ZipArchive;
        ...
        if ($res === TRUE) {
            ...
            $messageStack->add(UCFORM_ZIP_ERROR . $dest_dir, 'error');
            break;
        }
    ...
    case 'search':
    case 'search_reset':
    case 'go_page': $action = 'search';
        break;
    default:
}

/* * ***************   prepare to display templates  ************************ */
$result = $db->Execute('select id, parent_id, doc_type, doc_title, doc_group, security from ' . TABLE_UCFORM . ' 
	order by doc_title, id, parent_id');
$toc_array = array();
//$toc_array[-1][] = array('id' => 0, 'doc_type' => '0', 'doc_title' => TEXT_HOME); // home dir
$toc_array[-1][] = array('id' => 0, 'doc_type' => '0', 'doc_title' => "All Reports"); // home dir
while (!$result->EOF) {
...
unset($toc_array[0][1]);
unset($toc_array[0][5]);
$toc_array[0] = array_values($toc_array[0]);
$toggle_list = false;
if ($group) {
    $result = $db->Execute("select id from " . TABLE_UCFORM . " where doc_group = '" . $group . "'");
...
switch ($action) { // figure which detail page to load
    ...
    case 'view':
        ...
        $query_raw = "select " . implode(', ', $field_list) . " from " . TABLE_UCFORM . $search;
