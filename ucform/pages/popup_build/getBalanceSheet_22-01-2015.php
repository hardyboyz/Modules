...
<table width="100%" align="center">
 ...
 if (!empty($str)) {
    ...
    foreach ($group_total_array as $key => $group) {
        if ($balance_sheet->fields['c1'] == $group['period']) {
            $net_balance = ($group['beginning_balance']+$group['debit_amount'])-$group['credit_amount'];
            $str = "<tr style='background-color:#CCCCCC;'><th colspan='2'>Group Total for period : " . $group['period'] . "</th><th colspan='2'>".$account_name."</th><th colspan='2'>(Net Balance : ".'RM ' . number_format(round($net_balance,2),2).")</th><th colspan='1'>" . 'RM ' . number_format(round($group['beginning_balance'],2),2) . "</th><th colspan='1'> " . 'RM ' . number_format(round($group['debit_amount'],2),2) . "</th><th colspan='1'>" . 'RM ' . number_format(round($group['credit_amount'],2),2) . "</th></tr>";
            $period = $group['period'];
