...
// +-----------------------------------------------------------------+
//  Path: /modules/ucform/pages/popup_build/js_include.php
//
...
function init() {
  ...
  if (action == 'preview') {
    ...
    var popupWin = window.open("index.php?module=ucform&page=popup_gen&rID="+rID+"&todo=open&preview=1","popup_gen","width=1150,height=650,resizable=1,scrollbars=1,top=150,left=100");
  }
<?php 
  if ($self_close) {
    echo 'window.opener.location="index.php?module=ucform&page=main";' . chr(10);
...
function validateDB() {
  ...
  if (tableCnt < 2 || !tableCrit) {
    alert('<?php echo UCFORM_JS_TABLE_CHECK_ERROR; ?>');
    return;
  }
  $.ajax({
    type: "GET",
    url: 'index.php?module=ucform&page=ajax&op=validate_db'+fields,
...
function fieldLoad() {
  ...
  $.ajax({
    type: "GET",
    url: 'index.php?module=ucform&page=ajax&op=field_load'+params,
    dataType: ($.browser.msie) ? "text" : "xml",
    error: function(XMLHttpRequest, textStatus, errorThrown) {
      alert ("Ajax Error: " + XMLHttpRequest.responseText + "\nTextStatus: " + textStatus + "\nErrorThrown: " + errorThrown);
    },
...
function boxLoad(type, rowID) {
  ...
  $.ajax({
    type: "GET",
    url: 'index.php?module=ucform&page=ajax&op=box_load&type='+type+'&rID='+rID+'&rowID='+rowID,
    dataType: ($.browser.msie) ? "text" : "xml",
    error: function(XMLHttpRequest, textStatus, errorThrown) {
      alert ("Ajax Error: " + XMLHttpRequest.responseText + "\nTextStatus: " + textStatus + "\nErrorThrown: " + errorThrown);
    },
...
function calculateWidth() {	// total up the columns
	...
	var pageProperties = '<?php echo UCFORM_FLDLIST; ?>';
	pageProperties += ' ('+'<?php echo TEXT_ORIEN; ?>'+': '+orienText;
	pageProperties += ', '+'<?php echo TEXT_WIDTH; ?>'+': '+pageWidth;
	pageProperties += ', '+'<?php echo UCFORM_PGMARGIN_L; ?>'+': '+document.getElementById('marginleft').value;
	pageProperties += ', '+'<?php echo UCFORM_PGMARGIN_R; ?>'+': '+document.getElementById('marginright').value+')';
