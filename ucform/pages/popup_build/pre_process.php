...
// +-----------------------------------------------------------------+
//  Path: /modules/ucform/pages/popup_build/pre_process.php
//
$security_level = validate_user(SECURITY_ID_UCFORM);
/* * ************  include page specific files    ******************** */
require_once(DIR_FS_WORKING . 'defaults.php');
require_once(DIR_FS_WORKING . 'functions/ucform.php');
require_once(DIR_FS_MODULES . 'ucounting/functions/ucounting.php');
/* * *************   hook for custom actions  ************************** */
$custom_path = DIR_FS_MODULES . 'ucform/custom/extra_ucform.php';
...
$js_dir = $db->Execute('select id, parent_id, doc_title from ' . TABLE_UCFORM . ' order by id');
$dir_tree = array();
while (!$js_dir->EOF) {
...
switch ($action) {
    ...
    case 'preview':
        ...
        if (!$report->title)
            $error = $messageStack->add(GEN_ERRMSG_NO_DATA . TEXT_TITLE, 'error');
        ...
        if ($report->reporttype == 'frm') {
            ...
            while (true) {
                    ...
                    if ($_POST['img_sel_' . $key] == 'U') { // upload
                        if (validate_upload('img_upload_' . $key, 'image', array('jpg', 'jpeg', 'png', 'gif'))) {
                            ...
                            if (!@move_uploaded_file($_FILES['img_upload_' . $key]['tmp_name'], PF_DIR_MY_REPORTS . 'images/' . $properties->filename)) {
                                $messageStack->add(sprintf(UCFORM_IMAGE_MOVE_ERROR, PF_DIR_MY_REPORTS . 'images/' . $properties->filename), 'error');
                            }
                        } else {
                            $messageStack->add(UCFORM_IMAGE_UPLOAD_ERROR, 'error');
                        }
                ...
                if (isset($_POST['box_ltype_' . $key])) {
                    ...
                    if ($_POST['box_len_' . $key])
                        $properties->length = $_POST['box_len_' . $key];
                    if ($_POST['box_eabs_' . $key])
                        $properties->endabscissa = $_POST['box_eabs_' . $key];
                    if ($_POST['box_eord_' . $key])
                        $properties->endordinate = $_POST['box_eord_' . $key];
                }
                if ($_POST['box_txt_' . $key])
                    $properties->text = $_POST['box_txt_' . $key];
                if ($_POST['box_trun_' . $key])
                    $properties->truncate = $_POST['box_trun_' . $key];
                if ($_POST['box_last_' . $key])
                    $properties->display = $_POST['box_last_' . $key];
                if ($_POST['box_fnt_' . $key])
                    $properties->font = $_POST['box_fnt_' . $key];
                if ($_POST['box_size_' . $key])
                    $properties->size = $_POST['box_size_' . $key];
                if ($_POST['box_aln_' . $key])
                    $properties->align = $_POST['box_aln_' . $key];
                if ($_POST['box_clr_' . $key])
                    $properties->color = $_POST['box_clr_' . $key];
                if ($_POST['box_bdr_' . $key])
                    $properties->bordershow = $_POST['box_bdr_' . $key] ? '1' : '0';
                if ($_POST['box_bsz_' . $key])
                    $properties->bordersize = $_POST['box_bsz_' . $key];
                if ($_POST['box_bclr_' . $key])
                    $properties->bordercolor = $_POST['box_bclr_' . $key];
                if ($_POST['box_fill_' . $key])
                    $properties->fillshow = $_POST['box_fill_' . $key] ? '1' : '0';
                if ($_POST['box_fclr_' . $key])
                    $properties->fillcolor = $_POST['box_fclr_' . $key];
                if ($_POST['hbox_fnt_' . $key])
                    $properties->hfont = $_POST['hbox_fnt_' . $key];
                if ($_POST['hbox_size_' . $key])
                    $properties->hsize = $_POST['hbox_size_' . $key];
                if ($_POST['hbox_aln_' . $key])
                    $properties->halign = $_POST['hbox_aln_' . $key];
                if ($_POST['hbox_clr_' . $key])
                    $properties->hcolor = $_POST['hbox_clr_' . $key];
                if ($_POST['hbox_bdr_' . $key])
                    $properties->hbordershow = $_POST['hbox_bdr_' . $key] ? '1' : '0';
                if ($_POST['hbox_bsz_' . $key])
                    $properties->hbordersize = $_POST['hbox_bsz_' . $key];
                if ($_POST['hbox_bclr_' . $key])
                    $properties->hbordercolor = $_POST['hbox_bclr_' . $key];
                if ($_POST['hbox_fill_' . $key])
                    $properties->hfillshow = $_POST['hbox_fill_' . $key] ? '1' : '0';
                if ($_POST['hbox_fclr_' . $key])
                    $properties->hfillcolor = $_POST['hbox_fclr_' . $key];
                $boxfield = array();
                for ($j = 0; $j < sizeof($_POST['box_fld_' . $key]); $j++) {
                    $temp = new objectInfo();
                    if ($_POST['box_fld_' . $key])
                        $temp->fieldname = $_POST['box_fld_' . $key][$j];
                    if ($_POST['box_desc_' . $key])
                        $temp->description = $_POST['box_desc_' . $key][$j];
                    if ($_POST['box_proc_' . $key])
                        $temp->processing = $_POST['box_proc_' . $key][$j];
                    if ($_POST['box_fmt_' . $key])
                        $temp->formatting = $_POST['box_fmt_' . $key][$j];
                    if ($_POST['box_fnt_' . $key])
                        $temp->font = $_POST['box_fnt_' . $key][$j];
                    if ($_POST['box_size_' . $key])
                        $temp->size = $_POST['box_size_' . $key][$j];
                    if ($_POST['box_aln_' . $key])
                        $temp->align = $_POST['box_aln_' . $key][$j];
                    if ($_POST['box_clr_' . $key])
                        $temp->color = $_POST['box_clr_' . $key][$j];
                    if ($_POST['box_wid_' . $key])
                        $temp->width = $_POST['box_wid_' . $key][$j];
                    $boxfield[] = $temp;
                }
        ...
        if (isset($_POST['user_all']))
            $users = 'u:0';
        elseif (isset($_POST['users']) && $_POST['users'][0] <> '')
            $users = 'u:' . implode(':', $_POST['users']);
        $groups = 'g:-1';
        if (isset($_POST['group_all']))
            $groups = 'g:0';
        elseif (isset($_POST['users']) && $_POST['groups'][0] <> '')
            $groups = 'g:' . implode(':', $_POST['groups']);
        $report->security = $users . ';' . $groups;
        // test for no access which will hide the report
        if ($report->security == 'u:-1;g:-1') {
            $messageStack->add(UCFORM_NO_ACCESS, 'error');
            $error = true;
        }
        ...
        if ($_POST['grp_fld'])
            foreach ($_POST['grp_fld'] as $key => $value) {
        ...
        if ($_POST['sort_fld'])
            foreach ($_POST['sort_fld'] as $key => $value) {
        ...
        if ($_POST['crit_fld'])
            foreach ($_POST['crit_fld'] as $key => $value) {
    ...
    case 'save_dir':
        ...
        if (!$doc_title) {
            $messageStack->add(UCFORM_FOLDER_BLANK_ERROR, 'error');
            break;
        }
        // check to see if the directory is being moved below itself
        if (!validate_dir_move($dir_tree, $id, $parent_id)) {
            $messageStack->add(UCFORM_DIR_MOVE_ERROR, 'error');
            break;
        }
        $result = $db->Execute("select id from " . TABLE_UCFORM . " where doc_group = '" . $doc_group . "'");
        if ($result->RecordCount() > 0) {
            if ($result->fields['id'] <> $id) {
                $messageStack->add(UCFORM_DIR_GROUP_DUP_ERROR, 'error');
                break;
            }
        }
        // insert/update db
        ...
        if ($id) { // update
            db_perform(TABLE_UCFORM, $sql_array, 'update', 'id = ' . $id);
        } else { // insert
            db_perform(TABLE_UCFORM, $sql_array, 'insert');
        }
        $doc_title = ''; // clear the doc title
        break;
    case 'delete_dir':
        if (!$id) {
            $messageStack->add(UCFORM_DIR_DELETE_ERROR, 'error');
            break;
        }
        // check for directory empty
        $result = $db->Execute("select id from " . TABLE_UCFORM . " where parent_id = " . $id);
        if ($result->RecordCount() > 0) {
            $messageStack->add(UCFORM_DIR_NOT_EMPTY_ERROR, 'error');
            break;
        }
        $db->Execute("delete from " . TABLE_UCFORM . " where id = " . $id);
        $messageStack->add(UCFORM_DIR_DELETE_SUCCESS, 'success');
        break;
    case 'import_one':
        if ($success = ImportReport($_POST['reportname'], $_POST['RptFileName'], $import_path)) {
            $messageStack->add_session(UCFORM_IMPORT_SUCCESS, 'success');
            $self_close = true;
        }
        break;
    case 'import_all':
        ...
        foreach ($output as $file)
            if (!$success = ImportReport(NULL, $file, $import_path))
                $error = true;
        if (!$error) {
            $messageStack->add_session(UCFORM_DIR_IMPORT_SUCCESS, 'success');
            $self_close = true;
        }
        break;
    default:
...
switch ($action) {
    ...
    case 'refresh_dir':
        ...
        foreach ($loaded_modules as $mod)
            $sel_modules[] = array('id' => $mod, 'text' => $mod);
        $sel_language = load_language_dropdown();
        define('PAGE_TITLE', UCFORM_DOC_IMPORT);
    ...
    case 'new_ltr':
        ...
        if ($action == 'new_rpt')
            $report->reporttype = 'rpt';
        if ($action == 'new_frm')
            $report->reporttype = 'frm';
        if ($action == 'new_ltr')
            $report->reporttype = 'ltr';
    case 'save':
    case 'preview':
    case 'balance_sheet':
        $gl_account_id = $_GET['id'];

        $period_id = $_GET['period'];
        if ($_GET['param'] == 'in_stat') {
            $sql = "SELECT journal_main.id AS c0, journal_main.period AS c1, journal_main.post_date AS c2, 
CONCAT(journal_item.gl_account,' - ',chart_of_accounts.description) AS c3, journal_item.description AS c4, journal_item.debit_amount AS c5,
 journal_item.credit_amount AS c6,journal_main.journal_id AS c7,journal_main.purchase_invoice_id as c8,journal_main.sub_journal_id as c9 ,(SELECT chart_of_accounts_history.beginning_balance FROM chart_of_accounts_history WHERE account_id='" . $gl_account_id . "' and period = journal_main.period) as balance FROM journal_main JOIN journal_item ON journal_main.id = journal_item.ref_id 

JOIN chart_of_accounts ON journal_item.gl_account = chart_of_accounts.id 


WHERE journal_main.journal_id in ('2','6','7','12','13','14','16','18','20','33','34','35','36') AND journal_item.gl_account in('" . $gl_account_id . "') AND journal_main.period =" . $period_id . "

ORDER BY CONCAT(journal_item.gl_account,' - ',chart_of_accounts.description), journal_main.post_date";
        } else {
            $sql = "SELECT journal_main.id AS c0, journal_main.period AS c1, journal_main.post_date AS c2, 
CONCAT(journal_item.gl_account,' - ',chart_of_accounts.description) AS c3, journal_item.description AS c4, journal_item.debit_amount AS c5,
 journal_item.credit_amount AS c6,journal_main.journal_id AS c7,journal_main.purchase_invoice_id as c8,journal_main.sub_journal_id as c9 ,(SELECT chart_of_accounts_history.beginning_balance FROM chart_of_accounts_history WHERE account_id='" . $gl_account_id . "' and period = journal_main.period) as balance FROM journal_main JOIN journal_item ON journal_main.id = journal_item.ref_id 
...
ORDER BY CONCAT(journal_item.gl_account,' - ',chart_of_accounts.description), journal_main.post_date";
        ...
        $balance_sheet = $db->Execute($sql);
        $final_balance = $db->Execute('select * from chart_of_accounts_history WHERE period=' . $period_id . ' AND account_id="' . $gl_account_id . '"');
        $group_total = $db->Execute("SELECT journal_main.period as period, sum(journal_item.debit_amount) AS debit_amount,
 sum(journal_item.credit_amount) AS credit_amount, ( SELECT sum(chart_of_accounts_history.beginning_balance) as beginning_balance FROM chart_of_accounts_history WHERE chart_of_accounts_history.account_id=" . $gl_account_id . " AND chart_of_accounts_history.period=journal_main.period) as beginning_balance
FROM journal_main JOIN journal_item ON journal_main.id = journal_item.ref_id 
JOIN chart_of_accounts ON journal_item.gl_account = chart_of_accounts.id 
WHERE journal_main.journal_id in ('2','6','7','12','13','14','16','18','20','33','34','35','36') AND journal_item.gl_account in('" . $gl_account_id . "') AND journal_main.period<=" . $period_id . "
GROUP BY journal_main.period");
        $i=0;
        $group_total_array = array();
        while (!$group_total->EOF) {
            $group_total_array[$i]['period'] = $group_total->fields['period'];
            $group_total_array[$i]['debit_amount'] = $group_total->fields['debit_amount'];
            $group_total_array[$i]['credit_amount'] = $group_total->fields['credit_amount'];
            $group_total_array[$i]['beginning_balance'] = $group_total->fields['beginning_balance'];
           $i++;
            $group_total->MoveNext();
        }
        ...
        $include_tabs = false;
        $include_calendar = false;
        $include_template = 'getBalanceSheet.php';
    ...
    case 'design':
        ...
        foreach ($temp as $value) {
            ...
            if ($member[1] == '-1')
                $member[1] = ''; // make it no access which is null string on pull down
...
// build the groups list
        ...
        define('PAGE_TITLE', UCFORM_DOC_DESIGN);
